...
public class VersamentoExtPrsr {
    ...
    public void parseXML(SyncFakeSessn sessione, RispostaWS rispostaWs, VersamentoExt versamento) {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            verificaUdCollegate(tmpCSVersatore, tpiConfVerificaNumkeyUD, rispostaWs, versamento, rispostaControlli,
                    parsedUnitaDoc);
        }
    ...
    private void elabFlagSipVDB(OrgStrut os, VersamentoExt versamento, UnitaDocumentaria parsedUnitaDoc) {
        //
        ...
        // flags
        versamento.getStrutturaComponenti().setFlagAccettaErroreFmtNumero(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_FMT_NUM_NEG, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));
        //
        versamento.getStrutturaComponenti().setConfigFlagForzaFormatoNumero(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_FMT_NUM, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));
        //
        versamento.getStrutturaComponenti().setFlagProfiloUdObbOggetto(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_OBBL_OGGETTO, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));

        //
        versamento.getStrutturaComponenti()
                .setFlagProfiloUdObbData(configurationHelper
                        .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_OBBL_DATA, idStrut, idAmbiente, idTipoUd)
                        .equals(CostantiDB.Flag.TRUE));
        /*
        ...
        if (isConfigurazioneOnSip && parsedUnitaDoc.getConfigurazione().isForzaCollegamento() != null) {
            forzaCollegamento = parsedUnitaDoc.getConfigurazione().isForzaCollegamento().booleanValue();
        } else {
            forzaCollegamento = configurationHelper
                    .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_COLLEG, idStrut, idAmbiente, idTipoUd)
                    .equals(CostantiDB.Flag.TRUE);
        }
        ...
        //
        versamento.getStrutturaComponenti().setConfigFlagAbilitaContrColleg(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_COLLEG_UD,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        //
        versamento.getStrutturaComponenti().setFlagAbilitaContrFmtNumero(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_FMT_NUM, idStrut,
                        idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        //
        versamento.getStrutturaComponenti().setFlagAccettaContrCollegNeg(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_COLLEG_NEG,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_ABILITA_FORZA_1_5)) {
            ...
            // sip vs db
            if (isConfigurazioneOnSip && parsedUnitaDoc.getConfigurazione().isForzaFormatoNumero() != null) {
            ...
            flForzaHashVers = isConfigurazioneOnSip && parsedUnitaDoc.getConfigurazione().isForzaHash() != null
    ...
    private void verificaUdCollegate(CSVersatore csVersatore, KeySizeUtility.TpiConfig tpiConfVerificaNumkeyUD,
            RispostaWS rispostaWs, VersamentoExt versamento, RispostaControlli rispostaControlli,
            ...
            for (DocumentoVers valueDocVers : versamento.getStrutturaComponenti().getDocumentiAttesi()) {
                ...
                if (valueDocVers.getCategoriaDoc() == CategoriaDocumento.Principale) {
                    ...
                    if (myEsito.getUnitaDocumentaria().getDocumentoPrincipale() == null) {
                        ...
                        documentoPrincipale.setTipoDocumento(valueDocVers.getRifDocumento().getTipoDocumento());
                    ...
                    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                        myDocumentoVersPrsr.parseDocumentoGen(valueDocVers,
                                myEsito.getUnitaDocumentaria().getDocumentoPrincipale(), versamento, rispostaWs);
                    }
                }
                ...
                if (valueDocVers.getCategoriaDoc() == CategoriaDocumento.Allegato) {
                    ...
                    // verifica completa del documento
                    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                        myDocumentoVersPrsr.parseDocumentoGen(valueDocVers, allegato, versamento, rispostaWs);
                    }
                }
                ///////////////////////////////////////////////
                ...
                if (valueDocVers.getCategoriaDoc() == CategoriaDocumento.Annesso) {
                    ...
                    // verifica completa del documento
                    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                        myDocumentoVersPrsr.parseDocumentoGen(valueDocVers, annesso, versamento, rispostaWs);
                    }
                }
                ///////////////////////////////////////////////
                ...
                if (valueDocVers.getCategoriaDoc() == CategoriaDocumento.Annotazione) {
                    ...
                    // verifica completa del documento
                    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                        myDocumentoVersPrsr.parseDocumentoGen(valueDocVers, annotazione, versamento, rispostaWs);
                    }
