...
public class FirmeFormatiVers {
    ...
    /**
     * Il metodo attualmente si occupa di verificare la firma e validare il formato. Il metodo è richiamato da
     ...
     *            lista di marche temporali
     * @param dataDiRiferimento
     *            data di riferimento per la verifica
     * @param idTipoUd
     *            (opzionale)
     * @param versamento
     *            Informazioni relativi al versamento
     *
     * @return risposta controlli
     */
    public RispostaControlli verifica(ComponenteVers componenteVers, List<ComponenteVers> sottoComponentiFirma,
            List<ComponenteVers> sottoComponentiMarca, ZonedDateTime dataDiRiferimento, long idTipoUd,
        ...
        rule.withEseguiVerificaFirmaOnlyCrypto(
                ...
                .withHasMultipleFirmaDetached(
                        CollectionUtils.isNotEmpty(sottoComponentiFirma) && sottoComponentiFirma.size() > 1)
                .withHasMarcaDetached(CollectionUtils.isNotEmpty(sottoComponentiMarca));

        return verifica(componenteVers, sottoComponentiFirma, sottoComponentiMarca, dataDiRiferimento, idTipoUd,
                versamento, rule);
    }

    private RispostaControlli verifica(ComponenteVers componenteVers, List<ComponenteVers> sottoComponentiFirma,
            List<ComponenteVers> sottoComponentiMarca, ZonedDateTime dataDiRiferimento, long idTipoUd,
            AbsVersamentoExt versamento, InvokeVerificaRule rule) {
        RispostaControlli rs = new RispostaControlli();
        rs.setrLong(-1); // default
        ...
        try {
            ...
            // input
            InvokeVerificaInput in = new InvokeVerificaInput(componenteVers, sottoComponentiFirma, sottoComponentiMarca,
                    dataDiRiferimento, rule.getAbilitazioni(), verificaAllaDataDiFirma);

            // call ws
            LOG.debug("Inizio verifica firma");
            VerificaFirmaWrapper wrapper = verificaFirmaWrapperManager.invokeVerifica(rule, in, versamento);
            ...
            tmpUrn = MessaggiWSFormat.formattaBaseUrnDoc(versamento.getStrutturaComponenti().getUrnPartVersatore(),
                    ...
                    componenteVers.getUrnPartComponenteNiOrdDoc(), Costanti.UrnFormatter.URN_DOC_FMT_STRING_V2);
            ...
            LOG.info(
                    ...
                    wrapper.getServiceVersion(), (System.currentTimeMillis() - start), wrapper.getSelfLink(),
                    tmpUrn != null ? tmpUrn : "non calcolabile");
    ...
    private List<VFBustaExt> filterWrapper(String idcomponente, List<VFBusta> busteWrapper) {
        ...
        for (VFBusta busta : busteWrapper) {
            ...
            /**
             * Restituisce una lista di buste se e solo se per l'idcomponente esistono firme e/o marche associate
             */
    ...
    /**
     * Estensione della busta per ricreare il legame tra componente versato ed informazione restituita dal wrapper
     */
    private class VFBustaExt extends VFBusta {
        ...
        public List<VFFirmaCompType> getFirmaComps() {
            return bustaWrapper.getFirmaComps().stream().filter(f -> f.getId().equals(idComponenteVersato))
                    .collect(Collectors.toList());
        }

        @Override
        public List<VFMarcaCompType> getMarcaComps() {
            return bustaWrapper.getMarcaComps().stream().filter(m -> m.getId().equals(idComponenteVersato))
                    .collect(Collectors.toList());
        }
    ...
    /**
     * Estrazione/valutazione risultati verifica formato per il componente principale. A differenza del sottocomponente
     * il tikamime fornito è quello dedotto dal file sbustato in ambito di verifica della firma. Il tika mime, parte dei
     * dati "additional" del wrapper viene verificato secondo due scenari: 1) firma undetached: mimetype rilevato con
     * Tika su unico componente firmato con sbustamento 2) - detached: come sopra ma del componente principale
     * (originale unsigned)
     *
     * @param componenteVers
     * @param idTipoUd
     ...
     */
    private void extractComponenteFormatInfo(ComponenteVers componenteVers, long idTipoUd, VerificaFirmaWrapper wrapper)
            throws VerificaFirmaException {
        extractFormatInfo(componenteVers, idTipoUd, wrapper, wrapper.getAdditionalInfo().getTikaMime());
    }

    /**
     * Estrazione/valutazione risultati verifica formato per il sottocomponente (se esiste, di tipo FIRMA / MARCA). A
     * differenza del componente principale il tikamime non viene fornito ma dedotto all'interno della logica.
     *
     * @param componenteVers
     * @param idTipoUd
     * @param wrapper
    ...
    /**
     *
     * Il metodo decora il componenteVers (che può essere un componente piuttosto che un sottocomponente) con i
     ...
     * @throws VerificaFirmaException
     */
    private void extractFormatInfo(ComponenteVers componenteVers, long idTipoUd, VerificaFirmaWrapper wrapper,
            String tikaMime) throws VerificaFirmaException {
        CompDocMock mock = componenteVers.withAcdEntity();
        OrgStrut struttura = null;
        ...
        /*
         * MEV#18660 : info ricavata dal wrapper Ciò significa che la detection del mime-type, verrà effettuata in fase
         * di VERIFICA e non più come processo a parte
         */
        ...
        if (tikaMime != null) {
            ...
            if (!componenteVers.isFormatoFileVersNonAmmesso()) {
                ...
                formatoStd = controlliPerFirme.getDecFmtFileStandardFromFmtDocs(componenteVers.getIdFormatoFileVers(),
                        componenteVers.getChiaveComp(), formatoVersato.getNmFormatoFileDoc()).get(0);
                ...
                if (!tikaMime.equals(formatoStd.getNmMimetypeFile())) {
                    ...
                    formatoStd = this.getFormatoStdDaVersato(formati, formatoVersato.getNmFormatoFileDoc());
                    appendEsitoNeg = " - Il mimetype del formato dichiarato " + "(\"" + mimeTypeVersato
                            + "\") non coincide con il mimetype calcolato (\"" + tikaMime + "\")";
                ...
                // Calcolo il formato standard corrispondente al mimetype tika
                List<DecFormatoFileStandard> formati = controlliPerFirme.getDecFmtFileStandardFromTikaMimes(tikaMime);
                if (!formati.isEmpty()) {
        ...
        } else if (componenteVers.getTipoUso().equals("FIRMA") || componenteVers.getTipoUso().equals("MARCA")) {
        ...
        if (struttura != null) {
            ...
            if (flAbilitaContrFmt.equals("0")) {
                ...
                // test formato
                boolean formatoCompatibile = testFormati(formatoRappr.toString(), formatoVersato.getNmFormatoFileDoc());
                ...
                if (formatoCompatibile) {
                    ...
                    if (formatoStd.getTiEsitoContrFormato().equals(IdoneitaFormato.DEPRECATO.name())) {
                        mock.setTiEsitoContrFormatoFile(EsitoControlloFormato.WARNING.name());
                        mock.setDsMsgEsitoContrFormato(
                                "Il formato " + formatoStd.getNmFormatoFileStandard() + " è deprecato");
                    } else {
                    ...
                    if (!componenteVers.isFormatoFileVersNonAmmesso()) {
                        ...
                        esitoNegativo.append(formatoVersato.getNmFormatoFileDoc());
                            ...
                            || componenteVers.getDescFormatoFileVers().isEmpty()) {
                        esitoNegativo.append(" e non è stato dichiarato un formato per il versamento ");
                    } else {
    ...
    /**
     * SacerWS accetta il formato <strong>application/xml</strong> mentre, per esempio, EIDAS può restituisce
     * <strong>text/xml</strong>. Aggiungo questa forzatura per poter continuare la verifica dei formati.
     *
     * @param tikaMime
     *            mime type ottenuto da tika
     *
    ...
    /**
     * Il metodo setta all'interno di una busta il formato Standard determinamdo i formati standard ammessi per il
     * formato di firma / marca
     *
     * @param busta
     *            Busta crittografica
     * @param tiFormatoMarca
     *            Formato della firma / marca
     * @param formatoVersato
     *            Formato versato
     */
    private List<DecFormatoFileStandard> getFormatiBusta(VFBusta busta) throws VerificaFirmaException {
     ...
     *
     * <br/>
     * (1) quello coerente al formatoVersato se la lista contiene più di un elemento e questo coincide con una delle
     * estensioni del formato rapp. compatto fornito oppure <br/>
     * (2) uno a caso della lista se la lista contiene più di un elemento e questo NON coincide con una delle estensioni
     * del formato rapp. compatto fornito oppure <br/>
     ...
     *
     * Migliorata l'euristica: nel caso in cui non venga trovato alcun formato viene valutata anche l'estensione
     ...
     * @return
     */
    private DecFormatoFileStandard getFormatoStdDaVersato(List<DecFormatoFileStandard> formati, String formatoVersato) {
        ...
        foundFormat: for (DecFormatoFileStandard f : formati) {
            for (DecEstensioneFile ext : f.getDecEstensioneFiles()) {
                ...
                for (String formaVersItem : formVers) {
                    if (formaVersItem.toUpperCase().contains(ext.getCdEstensioneFile().toUpperCase())) {
                        formato = f;
                        break foundFormat;
                    }
                }
     ...
     *
     * @param formatoVersato
     *            estensione del file indicata nel sip di versamento.
     *
    ...
    /**
     * Testa il formato di rappresentazione compatto calcolato con quello versato. La verificaCrypto si basa sulle
     * estensioni dei formati standard che compongono il formato di rappresentazione.
     *
     * @param formatoRappr
     * @param formatoVers
     *
     ...
     */
    private boolean testFormati(String formatoRappr, String formatoVers) throws VerificaFirmaException {
        ...
        for (int i = 0; i < formRappr.length; i++) {
            if (!formRappr[i].equals(formVers[i])) {
                List<DecEstensioneFile> exts = controlliPerFirme.getDecEstensioneFiles(formRappr[i]);
