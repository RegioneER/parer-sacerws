...
public abstract class UpdVersamentoBasePrsr {
    ...
    protected boolean controllaUnivocitaDocIDs(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
    ...
    protected boolean controllaTipoAggiornamento(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
    ...
    protected boolean controllaVersioneXSD(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
    ...
    protected boolean controllaStrutturaVersatore(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            CSVersatore tagCSVersatore, String descChiaveUD) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        ...
        RispostaControlli rispostaControlli = controlliSemantici.checkIdStrut(tagCSVersatore,
                TipiWSPerControlli.AGGIORNAMENTO_VERSAMENTO, versamento.getStrutturaUpdVers().getDataVersamento());
    ...
    protected boolean controlloChiaveECdKeyNorm(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
    ...
    private boolean controllaChiave(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs, CSChiave tagCSChiave,
            String descChiaveUD) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
    ...
    protected boolean controllaHashSIP(SyncFakeSessn sessione, UpdVersamentoExt versamento,
        ...
        RispostaControlli rispostaControlli = updVersamentoControlli.checkUltimoSIPWithHashBinary(descChiaveUD,
    ...
    protected boolean checkTipoUDAndAbilUpdMeta(CompRapportoUpdVers myEsito, UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, String descChiaveUD) {
        ...
        if (!versamento.getVersamento().getUnitaDocumentaria().getIntestazione().getTipologiaUnitaDocumentaria()
            ...
            RispostaControlli rispostaControlli = updVersamentoControlli.getUpdFlagsFromTipoUDOrgStrut(
    ...
    protected boolean controllaAbilUserRegTipoUD(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        // verifica abilitazione utente per registro/tipo UD
        RispostaControlli rispostaControlli = updVersamentoControlli.checkTipoUDRegIamUserOrganizzazione(
    ...
    protected boolean controllaStatoConservazione(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            String descChiaveUD) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
    ...
    protected boolean controllaCollegamenti(CompRapportoUpdVers myEsito, UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        //
        return updCollegamentiPrsr.eseguiControlli(versamento, rispostaWs, myEsito);
    }

    protected boolean controllaProfiloNormativo(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
        // recupera e controlla fascicolo princ
        RispostaControlli rispostaControlli = updVersamentoControlli.checkProfiloNormativo(versamento,
                TipiEntitaSacer.UNI_DOC, versamento.getStrutturaUpdVers().getIdTipologiaUnitaDocumentaria(),
                versamento.getStrutturaUpdVers().getUrnPartChiaveUd(),
                versamento.getVersamento().getUnitaDocumentaria().getIntestazione().getTipologiaUnitaDocumentaria(),
                versamento.getStrutturaUpdVers().getIdUd());

        if (!rispostaControlli.isrBoolean()) {
            rispostaWs.setSeverity(SeverityEnum.ERROR);
            // esito generale
            rispostaWs.setEsitoWsError(ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_PROFILONORMATIVO),
                    rispostaControlli.getCodErr(), rispostaControlli.getDsErr());
            ...
            if (rispostaControlli.getrLong() != -1) /* se individuato */ {
                versamento.getStrutturaUpdVers().setIdRecUsoXsdProfiloNormativo(rispostaControlli.getrLong());
                // xml canonicalizzato
                versamento.getStrutturaUpdVers().setDatiC14NProfNormXml(rispostaControlli.getrString());
            }
            // aggiunta OK
            versamento.addControlloOkOnControlliUnitaDocumentaria(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_PROFILONORMATIVO));
    ...
    protected boolean controllaProfiloUnitaDocumentaria(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            String descChiaveUD) {
    ...
    protected boolean controllaDatiSpecifici(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            String descChiaveUD) {
        //
        ...
        RispostaControlliAttSpec tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpec(TipiEntitaSacer.UNI_DOC,
                ...
                versamento.getModificatoriWSCalc().contains(Costanti.ModificatoriWS.TAG_DATISPEC_DEL_1_6));
    ...
    protected boolean controllaDatiSpeificiMig(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            String descChiaveUD) {
    ...
    protected boolean controllaFascicoliSecondari(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
        // recupera e controlla fascicolo princ
        RispostaControlli rispostaControlli = updVersamentoControlli.checkFascicoliSecondari(versamento);
    ...
    private boolean controllaCdKeyNormalized(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        boolean prosegui = true;
