...
public class ObjectStorageHelper {
    ...
    private static final String INGEST_NODE = System.getProperty("jboss.node.name");
    ...
    /**
     * Salva il file su disco sull'object storage della configurazione identificandolo con la chiave
     * passata come parametro. Usa {@code RequestBody.fromFile()} che supporta i retry HTTP dell'SDK
     * riaprendo il file senza richiedere mark/reset sullo stream.
     *
     * @param blobPath      path del file da caricare
     ...
     */
    public ObjectStorageResource putS3ObjectFromFile(Path blobPath, final String key,
        ...
        return putS3Object(RequestBody.fromFile(blobPath), key, configuration, metadata, tags,
                base64crc32c);
    }

    /**
     * Implementazione comune per tutte le varianti di put su object storage. Gestisce la
     * costruzione della request, il logging, l'invio e la restituzione della risorsa caricata.
     *
     * @param requestBody   corpo della richiesta HTTP (già costruito dal caller)
     * @param key           chiave dell'oggetto
     * @param configuration configurazione dell'object storage in cui aggiungere l'oggetto
     * @param metadata      eventuali metadati (nel caso non vengano passati vengono utilizzati
     *                      quelli predefiniti)
     ...
     * @throws ObjectStorageException in caso di errore
     */
    private ObjectStorageResource putS3Object(RequestBody requestBody, final String key,
            ObjectStorageBackend configuration, Optional<Map<String, String>> metadata,
            Optional<Set<Tag>> tags, Optional<String> base64crc32c) throws ObjectStorageException {
    ...
    private Map<String, String> defaultMetadata() {
        ...
        defaultMetadata.put(MEATADATA_INGEST_NODE, INGEST_NODE);
     ...
     *
     * La copia è pilotata manualmente: vengono usati il client S3 sorgente (get-object) e il client
     * S3 destinazione (put-object). La trasmissione avviene in streaming puro tramite
     * {@link ContentStreamProvider}: ad ogni tentativo SDK (retry inclusi) il provider riapre
     * autonomamente un nuovo stream verso il bucket sorgente, senza file di appoggio su disco e
     * senza buffering in memoria.
     *
     * Si dovrebbe passare alla metodologia con S3 api di copia MA con un client+SA con permessi di
     * lettura / scrittura su entrambi i bucket coinvolti.
     *
     * @param sourceKey           chiave del bucket sorgente
     * @param sourceConfiguration configurazione del bucket sorgente
     * @param destKey             chiave del bucket destinazione
     * @param destConfiguration   configurazione del bucket di destinazione
     *
     * @return riferimento della risorsa caricata
     *
     * @throws ObjectStorageException in caso di errore
     */
    public ObjectStorageResource getS3ObjectFromSrcAndPutObjectOnDest(String sourceKey,
            ObjectStorageBackend sourceConfiguration, String destKey,
        ...
        try {
            S3Client s3SourceClient = s3Clients.getClient(sourceConfiguration.getAddress(),
                    sourceConfiguration.getAccessKeyId(), sourceConfiguration.getSecretKey());

            // Retrieve content length, content type and optional CRC32c via HeadObject (no data
            // download)
            HeadObjectResponse headResponse = s3SourceClient.headObject(HeadObjectRequest.builder()
                    .bucket(sourceConfiguration.getBucket()).key(sourceKey).build());
            long contentLength = headResponse.contentLength();
            String contentType = StringUtils.defaultIfEmpty(headResponse.contentType(),
                    Mimetype.MIMETYPE_OCTET_STREAM);
            String crc32c = headResponse.checksumCRC32C();

            GetObjectRequest getObjectRequest = GetObjectRequest.builder()
                    .bucket(sourceConfiguration.getBucket()).key(sourceKey).build();

            // ContentStreamProvider: the SDK calls newStream() for each attempt (retries included),
            // opening a fresh stream from the source bucket each time — no temp file, no buffering
            ContentStreamProvider streamProvider = () -> s3SourceClient.getObject(getObjectRequest);

            return putS3Object(
                    RequestBody.fromContentProvider(streamProvider, contentLength, contentType),
                    destKey, destConfiguration, Optional.empty(), Optional.empty(),
            ...
            throw ObjectStorageException.builder().message(
                    ...
                    sourceConfiguration.getBackendName(), sourceConfiguration.getBucket(),
                    sourceKey, destConfiguration.getBucket(), destKey).cause(e).build();
        }
     ...
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkCompDoc(ObjectStorageResource object, String nmBackend,
            long idCompDoc) throws ObjectStorageException {
        try {
    ...
    /**
     * Effettua il salvataggio del collegamento tra i metadati dell'unita documentaria e la chiave
     * sull'object storage
     *
     * @param object        informazioni dell'oggetto salvato
     * @param nmBackend     nome del backend (di tipo OS) su cui è stato salvato
     * @param idAroUnitaDoc id unita documentaria
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipUd(ObjectStorageResource object, String nmBackend,
            long idAroUnitaDoc) throws ObjectStorageException {
        try {
    ...
    /**
     * Effettua il salvataggio del collegamento tra i metadati del documento e la chiave sull'object
     * storage
     *
     * @param object    informazioni dell'oggetto salvato
     * @param nmBackend nome del backend (di tipo OS) su cui è stato salvato
     * @param idAroDoc  id unita documentaria
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipDoc(ObjectStorageResource object, String nmBackend,
            long idAroDoc) throws ObjectStorageException {
        try {
    ...
    // MEV#29276
    /**
     * Effettua il salvataggio del collegamento tra i sip degli aggiornamenti metadati e la chiave
     * sull'object storage
     *
     * @param object        informazioni dell'oggetto salvato
     * @param nmBackend     nome del backend (di tipo OS) su cui è stato salvato
     ...
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipAggMd(ObjectStorageResource object, String nmBackend,
            long idUpdUnitaDoc, BigDecimal idStrut) throws ObjectStorageException {
        try {
    ...
    /**
     * Effettua il salvataggio del collegamento tra i dati specifici degli aggiornamenti metadati e
     * la chiave sull'object storage
     *
     * @param object             informazioni dell'oggetto salvato
     * @param nmBackend          nome del backend (di tipo OS) su cui è stato salvato
     * @param idEntitySacerUpd   id dell'entità sacer associata ai dati specifici dell'aggiornamento
     *                           metadati
     * @param tipoEntitySacerUpd tipo entità sacer associata ai dati specifici dell'aggiornamento
     *                           metadati
     * @param idStrut            id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkUpdDatiSpecAggMd(ObjectStorageResource object,
            String nmBackend, long idEntitySacerUpd,
            TiEntitaAroUpdDatiSpecUnitaDoc tipoEntitySacerUpd, BigDecimal idStrut)
        ...
        try {
            ...
            case UPD_COMP:
                AroUpdCompUnitaDoc aroUpdCompUnitaDoc = entityManager.find(AroUpdCompUnitaDoc.class,
                        idEntitySacerUpd);
                osLink.setAroUpdUnitaDoc(
                        aroUpdCompUnitaDoc.getAroUpdDocUnitaDoc().getAroUpdUnitaDoc());
    ...
    /**
     * Effettua il salvataggio del collegamento tra i dati specifici degli aggiornamenti metadati
     * relativi ai metadati iniziali e la chiave sull'object storage
     *
     * @param object                 informazioni dell'oggetto salvato
     * @param nmBackend              nome del backend (di tipo OS) su cui è stato salvato
     * @param idEntitySacerVersIni   id dell'entità sacer associata ai dati specifici
     *                               dell'aggiornamento metadati relativi ai metadati iniziali
     * @param tipoEntitySacerVersIni tipo entità sacer associata ai dati specifici
     *                               dell'aggiornamento metadati relativi ai metadati iniziali
     * @param idStrut                id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkVersIniDatiSpecAggMd(ObjectStorageResource object,
            String nmBackend, long idEntitySacerVersIni,
        ...
        try {
            ...
            case COMP:
                ...
                osLink.setAroVersIniUnitaDoc(
                        aroVersIniComp.getAroVersIniDoc().getAroVersIniUnitaDoc());
     ...
     * sull'object storage (nel bucket "sessioni-err-ko-agg-md")
     *
     * @param object             informazioni dell'oggetto salvato
     * @param nmBackend          nome del backend (di tipo OS) su cui è stato salvato
     * @param idSesUpdUnitaDocKo id sessione fallita
     * @param idStrut            id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipSessioneKoAggMd(ObjectStorageResource object,
            String nmBackend, long idSesUpdUnitaDocKo, BigDecimal idStrut)
            throws ObjectStorageException {
        try {

            VrsSesUpdUnitaDocKo sesUpdUnitaDocKo = entityManager.find(VrsSesUpdUnitaDocKo.class,
                    idSesUpdUnitaDocKo);
            ...
            linkFalliti.setDecBackend(backendHelper.getBackendEntity(nmBackend));
     ...
     * storage (nel bucket "sessioni-err-ko-agg-md")
     *
     * @param object              informazioni dell'oggetto salvato
     * @param nmBackend           nome del backend (di tipo OS) su cui è stato salvato
     * @param idSesUpdUnitaDocErr id sessione errata
     * @param idStrut             id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipSessioneErrAggMd(ObjectStorageResource object,
            String nmBackend, long idSesUpdUnitaDocErr, BigDecimal idStrut)
            throws ObjectStorageException {
        try {
            ...
            linkErrati.setDecBackend(backendHelper.getBackendEntity(nmBackend));
    ...
    /**
     * Effettua il salvataggio del collegamento tra i sip di versamento (raggruppati dall'id dati
     * sessione vers) e la chiave sull'object storage
     *
     * @param object             informazioni dell'oggetto salvato
     * @param nmBackend          nome del backend (di tipo OS) su cui è stato salvato
     * @param idDatiSessioneVers id dati sessione vers
     * @param idStrut            id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkSipSessione(ObjectStorageResource object, String nmBackend,
        ...
        try {
            ...
            vrsXmlDatiSesObjectStorageKo.setDecBackend(backendHelper.getBackendEntity(nmBackend));
            ...
            vrsXmlDatiSesObjectStorageKo.setDatiSessioneVersKo(
                    entityManager.find(VrsDatiSessioneVersKo.class, idDatiSessioneVers));
    ...
    /**
     * Effettua il salvataggio del collegamento tra i versamenti falliti e la loro chiave
     * sull'object storage (nel bucket di staging)
     *
     * @param stagingResource informazioni dell'oggetto salvato
     * @param nmBackend       nome del backend (di tipo OS) su cui è stato salvato
     * @param idFileSessione  id file sessione
     * @param idStrut         id della struttura versante
     *
     * @throws ObjectStorageException in caso di errore
     */
    public void saveObjectStorageLinkFileSessione(ObjectStorageResource stagingResource,
            String nmBackend, long idFileSessione, BigDecimal idStrut)
            throws ObjectStorageException {
        try {
            VrsFileSessioneKo fileSessioneKo = entityManager.find(VrsFileSessioneKo.class,
                    idFileSessione);
    ...
    /**
     * Effettua la copia dell'oggetto dal bucket di staging a quello definitivo (dei
     * componenti).<strong>Questo metodo funziona solamente se entrambi i bucket sono all'interno
     * del medesimo OS</strong>.
     *
     * <strong>ATTENZIONE</strong> Questo metodo non è mai stato testato né chiamato. Rappresenta,
     * però, una possibile ottimizzazione della fase di versamento.
     *
     *
     * @param sourceKey           chiave dell'oggetto nel bucket di staging
     * @param sourceConfiguration configurazione dell'Object storage sorgente
     * @param destKey             chiave dell'oggetto nel bucket dei componenti
     * @param destConfiguration   configurazione dell'Object storage destinazione
     *
     * @return restituisce oggetto copiato da sorgente a destinazione {@link ObjectStorageResource}
     *
     * @throws ObjectStorageException in caso di eccezione
     *
     */
    public ObjectStorageResource copyS3ObjectFromSrcToDest(String sourceKey,
            ObjectStorageBackend sourceConfiguration, String destKey,
            ObjectStorageBackend destConfiguration) throws ObjectStorageException {
        ...
        if (!sameServiceAccount(sourceConfiguration, destConfiguration)) {
            throw ObjectStorageException.builder().message(
                    "Impossibile effettuare copia source key = {0} bucket {1} / dest key = {2} bucket = {3} per istanze diverse di Object Storage {4} verso {5}",
                    sourceKey, sourceConfiguration.getBucket(), destKey,
                    destConfiguration.getBucket(), sourceConfiguration.getBackendName(),
                    destConfiguration.getBackendName()).build();
        }
        ...
        try {
            ...
            CopyObjectRequest copyReq = CopyObjectRequest.builder()
                    .sourceBucket(sourceConfiguration.getBucket()).sourceKey(sourceKey)
                    .destinationBucket(destConfiguration.getBucket()).destinationKey(destKey)
                    .build();
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        ...
                        sourceKey, sourceConfiguration.getBucket(), destConfiguration.getBucket(),
                        destKey, copyRes.copyObjectResult().eTag());
            }
            final String tenant = getDefaultTenant();
            ...
            throw ObjectStorageException.builder()
                    .message("Impossibile copiare tra staging e componenti").cause(e).build();
        }
    }

    /**
     * Controlla se il service account utilizzato per accedere all'object storage è lo stesso. In
     * questo caso si può procedere all'operazione di copia tra bucket.
     *
     * @param conf1 prima configurazione
     * @param conf2 seconda configurazione
     *
     * @return true se il service account è il medesimo
     */
    private static boolean sameServiceAccount(ObjectStorageBackend conf1,
            ObjectStorageBackend conf2) {
        return conf1.getAddress().equals(conf2.getAddress())
                && conf1.getAccessKeyId().equals(conf2.getAccessKeyId())
    ...
    private String getDefaultTenant() {
        return configurationHelper
                .getValoreParamApplicByApplic(ParametroApplDB.TENANT_OBJECT_STORAGE);
     ...
     *
     *             Effettua il tagging di un oggetto esistente su bucket con chiave e nell'object
     *             storage indicato (su base configurazioni)
     *
     * @param osResource    risorsa salvata su object storage
     * @param configuration configurazione per accedere all'object storage
     * @param tags          tag da impostare sull'oggetto
     *
     * @throws ObjectStorageException eccezzione generica (S3 error code / varie)
     */
    @Deprecated(since = "Integrazione GCP (tagging not implemented)", forRemoval = true)
    public void taggingS3Object(ObjectStorageResource osResource,
            ObjectStorageBackend configuration, Set<Tag> tags) throws ObjectStorageException {
        ...
        log.debug("Sto per taggare con {} nell'os {} la chiave {} sul bucket {}", tags,
                storageAddress, osResource.getKey(), osResource.getBucket());

        try {
            S3Client s3Client = s3Clients.getClient(storageAddress, accessKeyId, secretKey);
            PutObjectTaggingRequest objectRequest = PutObjectTaggingRequest.builder()
                    .bucket(osResource.getBucket()).key(osResource.getKey())
            ...
            if (!response.sdkHttpResponse().isSuccessful()) {
                throw ObjectStorageException.builder()
                        .message("{0}: errore tagging oggetto {1} sul bucket {2}, response {3}",
                                configuration.getBackendName(), osResource.getKey(),
                                osResource.getBucket(), response)
                        .build();
            }

            final long end = System.currentTimeMillis() - start;
            if (log.isDebugEnabled()) {
                log.debug("Tagging oggetto {} sul bucket {} effettuato in {} ms",
                        osResource.getKey(), osResource.getBucket(), end);
            }
        } catch (Exception e) {
            throw ObjectStorageException.builder()
                    .message("{0}: errore imprevisto tagging oggetto {1} sul bucket {2}",
                            configuration.getBackendName(), osResource.getKey(),
                            osResource.getBucket())
                    .cause(e).build();
        }
    }
}
