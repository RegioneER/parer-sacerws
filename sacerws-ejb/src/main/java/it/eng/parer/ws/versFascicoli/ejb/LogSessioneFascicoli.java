...
public class LogSessioneFascicoli {
    ...
    public boolean registraSessioneErrata(RispostaWSFascicolo rispostaWs,
	...
	try {
	    ...
	    tmpRispostaControlli = logSessioneFascicoliHelper.scriviFascicoloErr(rispostaWs,
		    versamento, sessione);
	    if (!tmpRispostaControlli.isrBoolean()) {
	    ...
	    tmpRispostaControlli = logSessioneFascicoliHelper.scriviXmlFascicoloErr(rispostaWs,
		    versamento, sessione, tmpSesFascicoloErr);
	    if (!tmpRispostaControlli.isrBoolean()) {
		// probabilmente è inutile visto che se arrivo qui è per una runtimeexception
		// che forza il rollback in ogni caso, ma meglio andare sul sicuro, dal momento
		// che non posso escludere di terminare male un salvataggio anche senza una
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public boolean registraSessioneFallita(RispostaWSFascicolo rispostaWs,
	    VersFascicoloExt versamento, SyncFakeSessn sessione) {
	// nota l'errore critico di persistenza viene contrassegnato con la lettera P
	// per dare la possibilità all'eventuale chiamante di ripetere il tentativo
	// quando possibile (non è infatti un errore "definitivo" dato dall'input, ma bensì
	...
	try {
		...
		if (tmpRispostaControlli.getrObject() != null) {
			...
			// Mi limito a restituire un errore al chiamante e spero che qualcuno lo
			// legga.
			context.setRollbackOnly();
			this.impostaErrore(rispostaWs, versamento, tmpRispostaControlli);
			return false;
		    ...
		    tmpRispostaControlli = logSessioneFascicoliHelper.scriviFascicoloKo(rispostaWs,
			    versamento, sessione);
		    if (!tmpRispostaControlli.isrBoolean()) {
			context.setRollbackOnly();
			this.impostaErrore(rispostaWs, versamento, tmpRispostaControlli);
	    ...
	    tmpRispostaControlli = logSessioneFascicoliHelper.scriviSessioneFascicoloKo(rispostaWs,
		    versamento, sessione, tmpFascicoloKo, tmpFasFascicolo);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, versamento, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = logSessioneFascicoliHelper.scriviXmlFascicoloKo(rispostaWs,
		    versamento, sessione, tmpSesFascicoloKo);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, versamento, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = logSessioneFascicoliHelper.scriviErroriFascicoloKo(rispostaWs,
		    versamento, tmpSesFascicoloKo);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, versamento, tmpRispostaControlli);
		return false;
