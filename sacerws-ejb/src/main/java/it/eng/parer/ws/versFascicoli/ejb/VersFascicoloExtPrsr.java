...
public class VersFascicoloExtPrsr {
    ...
    public void parseXML(SyncFakeSessn sessione, RispostaWSFascicolo rispostaWs,
	...
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
		...
		tagCSVersatore.setAmbiente(
			parsedIndiceFasc.getIntestazione().getVersatore().getAmbiente());
		tagCSVersatore.setEnte(parsedIndiceFasc.getIntestazione().getVersatore().getEnte());
		tagCSVersatore.setStruttura(
			parsedIndiceFasc.getIntestazione().getVersatore().getStruttura());
		...
		descChiaveFasc = MessaggiWSFormat.formattaChiaveFascicolo(tagCSVersatore,
			tagCSChiave);
		versamento.getStrutturaComponenti().setChiaveNonVerificata(tagCSChiave);
		//
		// sistema =
		// configurationHelper.getParamApplicValue(ParametroApplDB.NM_SISTEMACONSERVAZIONE);
		sistema = configurationHelper
			.getValoreParamApplicByApplic(ParametroApplDB.NM_SISTEMACONSERVAZIONE);
		versamento.getStrutturaComponenti().setUrnPartChiaveFascicolo(MessaggiWSFormat
			.formattaChiaveFascicolo(tagCSVersatore, tagCSChiave, sistema));
		// normalized key
		versamento.getStrutturaComponenti().setUrnPartChiaveFascicoloNormalized(
			MessaggiWSFormat.formattaChiaveFascicolo(tagCSVersatore, tagCSChiave,
				sistema, true));
		...
		if (ex.getMessage() != null && !ex.getMessage().isEmpty()) {
		    ...
		    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.FAS_XSD_001_001,
			    tmpVEventHandler.getMessaggio());
		}
		// se non riesco a convalidare l'XML magari posso provare a leggerlo
		// in modo meno preciso e tentare di riscostruire la struttura, la chiave ed il tipo
		// fascicolo
	...
	// verifico la chiave
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
	    this.controllaChiave(myControlliFascicolo, myEsito, versamento, rispostaWs, tagCSChiave,
		    descChiaveFasc);
	}

	// verifico tipo conservazione
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
	    this.controllaTipoConservazione(myControlliFascicolo, parsedIndiceFasc, rispostaWs,
		    descChiaveFasc);
	}
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    ...
	    boolean prosegui = this.controllaProfiloArchivistico(myEsito, versamento,
		    descChiaveFasc);

	    if (prosegui) {
		prosegui = this.controllaProfiloSpecifico(myEsito, versamento, parsedIndiceFasc,
			descChiaveFasc);// TODO:
					// al
					// momento
					// non
	    ...
	    if (prosegui) {
		prosegui = this.controllaSoggettoProd(myEsito, versamento, parsedIndiceFasc,
			descChiaveFasc);
	    }
	    // viene sempre eseguito (dati già controllati in precedenza)
	    if (prosegui) {
		prosegui = this.controllaFlagStruttura(myEsito, rispostaWs, versamento,
			descChiaveFasc);
	    }

	    if (prosegui) {
		prosegui = this.controllaClassificazione(myEsito, versamento, parsedIndiceFasc,
			descChiaveFasc);
	    }

	    if (prosegui) {
		prosegui = this.controllaFormatoNumero(myEsito, versamento, parsedIndiceFasc,
			descChiaveFasc, tagCSChiave, sistema);
	    }
    ...
    private void controllaTipoConservazione(EsitoControlliFascicolo myControlliFascicolo,
	    IndiceSIPFascicolo parsedIndiceFasc, RispostaWSFascicolo rispostaWs,
	    String descChiaveFasc) {
    ...
    private boolean controllaFlagStruttura(CompRapportoVersFascicolo myEsito,
	    RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento, String descChiaveFasc) {
	...
	if (rispostaControlli.isrBoolean()) {
	    ...
	    versamento.addError(descChiaveFasc, rispostaControlli.getCodErr(),
		    rispostaControlli.getDsErr());
	    prosegui = false;// TODO: non è stato impostato un codice di errore (non esiste da
			     // specifica perché non
    ...
    private boolean controllaClassificazione(CompRapportoVersFascicolo myEsito,
	    VersFascicoloExt versamento, IndiceSIPFascicolo parsedIndiceFasc,
	    String descChiaveFasc) {
	...
	if (svf.getDatiXmlProfiloGenerale() != null
	    ...
	    // se nullo ma esiste su SIP di versamento lo setto ... altrimenti se presente, setto
	    // quello da
	    // configurazione
		...
		if (!rispostaControlli.isrBoolean()) {
		    ...
		    versamento.getStrutturaComponenti()
			    .setIdVoceTitol(rispostaControlli.getrLong());
    ...
    private boolean controllaFormatoNumero(CompRapportoVersFascicolo myEsito,
	    VersFascicoloExt versamento, IndiceSIPFascicolo parsedIndiceFasc, String descChiaveFasc,
	    CSChiaveFasc tagCSChiave, String sistema) {
	ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo = myEsito.getFascicolo()
		.getEsitoControlliFascicolo();
	...
	// se nullo ma esiste su SIP di versamento lo setto ... altrimenti se presente, setto quello
	// da configurazione
	...
	// Note : se il profilo generale non esiste si passa un "null" al fine di far fallire il
	// check gestendo comunque
	...
	if (rispostaControlli.isrBoolean()) {
	    ...
	    KeyOrdFascUtility.KeyOrdResult keyOrdResult = (KeyOrdFascUtility.KeyOrdResult) rc
		    .getrObject();
	    svf.setKeyOrdCalcolata(keyOrdResult.getKeyOrdCalcolata());
	    svf.setProgressivoCalcolato(keyOrdResult.getProgressivoCalcolato());
	}
    ...
    private boolean controllaCollegamenti(CompRapportoVersFascicolo myEsito,
	    VersFascicoloExt versamento, IndiceSIPFascicolo parsedIndiceFasc) {
	ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo = myEsito.getFascicolo()
		.getEsitoControlliFascicolo();
	...
	boolean prosegui = true, flForzaContrColl = false, linkHasBuilt = true;// verifica non
									       // bloccante per i
									       // controlli
									       // successivi
									       // (qualunque esito)
	...
	if (svf.getDatiXmlProfiloArchivistico() != null
	    ...
	    RispostaControlli rispostaControlli = controlliCollFascicolo
	    ...
	    List<FascicoloLink> fascicoliToBeLinked = (List<FascicoloLink>) rispostaControlli
		    .getrObject();
	    ...
	    // - 3 : / il codice di errore non è FASC_006_002 (collegamenti con stessa chiave)
	    ...
	    if (!isFlAbilitaContrColleg && (!linkHasBuilt || rispostaControlli.getCodErr() == null
		    || !rispostaControlli.getCodErr().equals(
		...
		if (linkHasBuilt) {
			...
			rispostaControlli = controlliFascicoli.checkChiave(csChiaveFascColl,
				"dummy", svf.getIdStruttura(),
				ControlliFascicoli.TipiGestioneFascAnnullati.CARICA);// indipendentemente
										     // lo stato,
										     // interessa
										     // capire SE
										     // esiste su
			...
			if (rispostaControlli.isrBoolean()) {
			    ...
			    toBeLinked.setIdLinkFasc(rispostaControlli.getrLong());// fk fascicolo
										   // trovato
			}
			...
			if (toBeLinked.getDescCollegamento()
			    ...
			    flForzaContrColl = false; // in questo caso anche se l'errore sarebbe
						      // normalmente gestito
						      // come WARNING "forzo" ad ERRORE (non sarà
						      // mai possibile
	...
	if (rispostaControlli.getrLong() < 1) {
		...
		versamento.getStrutturaComponenti()
			.setIdStruttura(rispostaControlli.getrLongExtended());
		// salvo idUtente
	...
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
	    ...
	    rispostaControlli = controlliEjb.checkAuthWS(versamento.getUtente(),
		    versamento.getDescrizione(), TipiWSPerControlli.VERSAMENTO_FASCICOLO);
	    if (!rispostaControlli.isrBoolean()) {
		myControlliFascicolo.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
		myControlliFascicolo.setIdentificazioneVersatore(ECEsitoPosNegType.NEGATIVO);
		rispostaWs.setSeverity(SeverityEnum.ERROR);
    ...
    private void controllaTipoFascUserOrg(EsitoControlliFascicolo myControlliFascicolo,
	    VersFascicoloExt versamento, RispostaWSFascicolo rispostaWs, String descChiaveFasc) {
	// verifico il tipo fascicolo abilitato per utente/org - in caso di errore la sessione è
	// DUBBIA
	//
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	RispostaControlli rispostaControlli = controlliFascicoli
	...
	if (!rispostaControlli.isrBoolean()) {
	    ...
	    // tentare di riscostruire la chiave ed il tipo fascicolo
	    // e trasformare la sessione errata in una sessione fallita
	    rispostaWs.setStatoSessioneVersamento(IRispostaWS.StatiSessioneVersEnum.DUBBIA);// TODO:
											    // corretto????
	} else {
	...
	//
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	RispostaControlli rispostaControlli = controlliFascicoli.checkTipoFascicoloAnno(
		descChiaveFasc, svf.getTipoFascicoloNonverificato(),
    ...
    private void controllaChiave(ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo,
	    ...
	    RispostaWSFascicolo rispostaWs, CSChiaveFasc tagCSChiave, String descChiaveFasc) {
	RispostaControlli rispostaControlli = controlliFascicoli.checkChiave(tagCSChiave,
	...
	RispostaControlli rispostaControlli = controlliProfiliFascicolo
	...
	if (prosegui) {
	    ...
	    rispostaControlli = controlliProfiliFascicolo.verificaProfiloGenerale(versamento,
		    myEsito.getFascicolo());
	    if (!rispostaControlli.isrBoolean()) {
		myControlliFascicolo.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
    ...
    private boolean controllaSoggettoProd(CompRapportoVersFascicolo myEsito,
	    VersFascicoloExt versamento, IndiceSIPFascicolo parsedIndiceFasc,
	    String descChiaveFasc) {
	ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo = myEsito.getFascicolo()
		.getEsitoControlliFascicolo();
	...
	// TODO : per il momento commentato (codice testato e funzionante da attivare nella prossima
	// versione del WS)
	/*
	 * if (parsedIndiceFasc.getIntestazione() .getSoggettoProduttore() != null) {
	 *
	 * String code = parsedIndiceFasc. getIntestazione(). getSoggettoProduttore(). getCodice();
	 * String den = parsedIndiceFasc. getIntestazione(). getSoggettoProduttore().
	 * getDenominazione();
	 *
	 ...
	 * } }//codice //verifica per denominazione if(StringUtils.isNotBlank(den)) {
	 *
	 ...
	 * myControlliFascicolo.setIdentificazioneSoggettoProduttore(ECEsitoPosNegWarType.POSITIVO);
	 *
	 * } }//denominazione //Nota: non indicati codice/denominazione -> si restituiscono entrambi
	 * gli errori? if(StringUtils.isBlank(code) && StringUtils.isBlank(den)) {
	 *
	 * myControlliFascicolo.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
	 * myControlliFascicolo.setIdentificazioneSoggettoProduttore(ECEsitoPosNegWarType.NEGATIVO);
	 * // aggiungo l'errore restituito alla lista errori versamento.addError(descChiaveFasc,
	 * MessaggiWSBundle.FAS_CONFIG_004_001,
	 * MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_004_001, "N/A")); // aggiungo
	 * l'errore restituito alla lista errori versamento.addError(descChiaveFasc,
	 * MessaggiWSBundle.FAS_CONFIG_004_002,
	 ...
	 * myControlliFascicolo.setIdentificazioneSoggettoProduttore(ECEsitoPosNegWarType.
	 * NON_ATTIVATO); }
	 */
	...
	if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_URN_SIP_FASC_1_1)) {
	    ...
	    String urnSip = MessaggiWSFormat.formattaUrnSip(tmpUrn,
		    Costanti.UrnFormatter.URN_SIP_FASC);
