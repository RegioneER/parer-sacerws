...
public class SalvataggioFascicoliHelper {
	...
	try {
	    ...
	    tmpFascicolo.setTiConservazione(
		    it.eng.parer.entity.constraint.FasFascicolo.TiConservazione.valueOf(versamento
			    .getVersamento().getParametri().getTipoConservazione().name()));
	    // TODO IL sistema di migraziobe va gestito
	    //
	    tmpFascicolo.setFlForzaContrClassif(
		    booleanToFlag(svf.getFlControlliFasc().isFlForzaContrFlassif()));
	    tmpFascicolo.setFlForzaContrColleg(
		    booleanToFlag(svf.getFlControlliFasc().isFlForzaContrColleg()));
	    tmpFascicolo.setFlForzaContrNumero(
		    booleanToFlag(svf.getFlControlliFasc().isFlForzaContrNumero()));
	    tmpFascicolo.setFlUpdAnnulUnitaDoc("0"); // fixed
	    //
	    if (svf.getDatiXmlProfiloArchivistico() != null) {
		tmpFascicolo.setCdIndiceClassif(
		...
		if (StringUtils.isNotBlank(dxpg.getOggettoFascicolo())) {
		    tmpFascicolo.setDsOggettoFascicolo(LogSessioneUtils.getStringAtMaxLen(
			    dxpg.getOggettoFascicolo(), DS_OGGETTO_FASC_MAX_LEN));
		}
		...
		if (dxpg.getIdPrimoDocumento() != null) {
		    tmpFascicolo.setAroUnitaDocFirst(
			    entityManager.find(AroUnitaDoc.class, dxpg.getIdPrimoDocumento()));
		}
		if (dxpg.getIdUltimoDocumento() != null) {
		    tmpFascicolo.setAroUnitaDocLast(
			    entityManager.find(AroUnitaDoc.class, dxpg.getIdUltimoDocumento()));
		}
		...
		if (StringUtils.isNotBlank(dxpg.getNoteFascicolo())) {
		    tmpFascicolo.setDsNota(LogSessioneUtils
			    .getStringAtMaxLen(dxpg.getNoteFascicolo(), DS_NOTA_MAX_LEN));
		}
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviAmmPartecipanti(VersFascicoloExt versamento,
	...
	try {
		...
		for (DXPGAmminPartecipante ammPart : dxpg.getAmmPartecipanti()) {
		    ...
		    fasAmminPartec.setTiCodiceAmminPartec(ammPart.getTipoCodice());// TODO: è
										   // codificato?
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviSoggCoinvolti(VersFascicoloExt versamento,
	    FasFascicolo fascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	// salvo amm. partecipanti del fascicolo
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviResponsabili(VersFascicoloExt versamento,
	    FasFascicolo fascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	// salvo amm. partecipanti del fascicolo
	...
	try {
		...
		for (DXPGRespFascicolo resp : dxpg.getResponsabili()) {
		     ...
		     */
		    fasRespFascicolo.setTiOggResp(
			    it.eng.parer.entity.constraint.FasRespFascicolo.TiOggResp.FASCICOLO);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviUoOrgResponsabili(VersFascicoloExt versamento,
	    FasFascicolo fascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	// salvo amm. partecipanti del fascicolo
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviRequestResponseFascicolo(RispostaWSFascicolo rispostaWs,
	...
	try {
	    ...
	    tmpXmlVersFascicolo
		    .setBlXmlVers(sessione.getDatiDaSalvareIndiceSip().length() == 0 ? "--"
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviProfiliXMLFascicolo(VersFascicoloExt versamento,
	...
	try {
	    ...
	    String xmlProfilo = this
		    .generaXmlProfilo(versamento.getVersamento().getProfiloGenerale().getAny());
		...
		xmlProfilo = this.generaXmlProfilo(
			versamento.getVersamento().getProfiloArchivistico().getValue().getAny());
		fasXmlFascicolo.setBlXml(xmlProfilo);
		fasXmlFascicolo.setIdStrut(new BigDecimal(svf.getIdStruttura()));
		fasXmlFascicolo.setDtVersFascicolo(convert(sessione.getTmApertura()));
		entityManager.persist(fasXmlFascicolo);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviUnitaDocFascicolo(VersFascicoloExt versamento,
	    FasFascicolo fascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	...
	try {
		...
		for (Long tmpId : svf.getUnitaDocElencate()) {
		    ...
		    fasUnitaDocFascicolo
			    .setAroUnitaDoc(entityManager.find(AroUnitaDoc.class, tmpId));
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviLinkFascicolo(VersFascicoloExt versamento,
	    FasFascicolo tmpFasFascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
	// salvo i riferimenti alle unità documentarie del fascicolo
	tmpRispostaControlli.setrBoolean(false);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviWarningFascicolo(VersFascicoloExt versamento,
	    FasFascicolo fascicolo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli ereditaVersamentiKoFascicolo(RispostaWSFascicolo rispostaWs,
	    VersFascicoloExt versamento, SyncFakeSessn sessione, FasFascicolo fascicolo,
	    VrsFascicoloKo fascicoloKo) {
	RispostaControlli tmpRispostaControlli = new RispostaControlli();
	//
