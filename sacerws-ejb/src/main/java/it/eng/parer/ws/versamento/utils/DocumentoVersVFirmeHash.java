...
public class DocumentoVersVFirmeHash {
    ...
    public DocumentoVersVFirmeHash(AbsVersamentoExt abstractVersamento, IRispostaWS iRisposta,
            ECConfigurazioneType configurazione) {
        versamento = abstractVersamento;
        rispostaWs = iRisposta;
        esitoConfigurazione = configurazione;

        // istanzia la risposta controlli
        rispostaControlli = new RispostaControlli();

        // Recupera l'ejb delle verifiche firma, se possibile - altrimenti segnala
        // errore
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            try {
                firmeFormatiEjb = (FirmeFormatiVers) new InitialContext().lookup("java:module/FirmeFormatiVers");
            } catch (NamingException ex) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB delle verifiche firma   " + ex.getMessage());
                log.error("Errore nel recupero dell'EJB delle verifiche firma ", ex);
            }
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            try {
                ...
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB dei controlli per firme   " + ex.getMessage());
    ...
    public boolean verificaDocumentoGenFirma(DocumentoVers documento, long idTipoUd) {
        ...
        for (ComponenteVers tmpCV : documento.getFileAttesi()) {
            ...
            if (tmpCV.getMySottoComponente() == null && tmpCV.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE
                ...
                if (proseguiElaborazione) {
                    ...
                    rispostaControlli = this.elabCompDocMockWithVerificaFirmaAndFormatoResult(tmpCV,
                            sottoComponentiFirma, sottoComponentiMarca, sottoComponentiAltri, idTipoUd);
                ...
                if (proseguiElaborazione) {
                    ...
                    if (statoCompCorrente.componenteFirmato) {
                        ...
                        if (statoCompCorrente.compEsitoVerFirmeHasErr) {
                            tmpCV.withAcdEntity().setTiEsitoVerifFirme(EsitoControllo.NEGATIVO.name());
                            tmpCV.withAcdEntity().setDsMsgEsitoVerifFirme(EsitoControllo.NEGATIVO.message());
                            ...
                            tmpCV.withAcdEntity().setTiEsitoVerifFirme(EsitoControllo.WARNING.name());
                            tmpCV.withAcdEntity().setDsMsgEsitoVerifFirme(EsitoControllo.WARNING.message());
                            tmpCV.getRifComponenteResp().getEsitoComponente()
                                    .setVerificaFirmeComponente(ECEsitoPosNegWarType.WARNING);
                            statoDocumento.verFirmeCompCodiceEsitoWar = true;
                        } else {
                            tmpCV.withAcdEntity().setTiEsitoVerifFirme(EsitoControllo.POSITIVO.name());
                            tmpCV.withAcdEntity().setDsMsgEsitoVerifFirme(EsitoControllo.POSITIVO.message());
                            tmpCV.getRifComponenteResp().getEsitoComponente()
                    ...
                    if (statoCompCorrente.ctrlFirmeCertificatoScad) {
        ...
        if (proseguiElaborazione) {
            // MEV#18660
            rispostaControlli = this.verificaFormatiCompDoc(documento, statoDocumento);
        ...
        if (proseguiElaborazione) {
            ...
            if (esitoDoc.isFirmatoDigitalmente().booleanValue()) {
                if (statoDocumento.verFirmeCompCodiceEsitoNeg) {
                    documento.setTiEsitoVerifFirme(EsitoControllo.NEGATIVO.name());
                    documento.setDsMsgEsitoVerifica(EsitoControllo.NEGATIVO.message());
                    ...
                    documento.setTiEsitoVerifFirme(EsitoControllo.WARNING.name());
                    documento.setDsMsgEsitoVerifica(EsitoControllo.WARNING.message());
                    esitoDoc.getEsitoDocumento().setVerificaFirmeDocumento(ECEsitoPosNegWarType.WARNING);
                } else {
                    documento.setTiEsitoVerifFirme(EsitoControllo.POSITIVO.name());
                    documento.setDsMsgEsitoVerifica(EsitoControllo.POSITIVO.message());
    ...
    ///
    public void verificaDocumentoGenHash(DocumentoVers documento) {
    ...
    private RispostaControlli verificaFormatiCompDoc(DocumentoVers documento, StatoDocumento statoDoc) {
        ...
        for (ComponenteVers tmpCV : documento.getFileAttesi()) {
            if (tmpCV.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE) {
                ...
                // verifico per errori
                if ((mock.getTiEsitoContrFormatoFile().equals(EsitoControllo.POSITIVO.name())
                        || mock.getTiEsitoContrFormatoFile().equals(EsitoControllo.WARNING.name()))
                    ...
                    if (mock.getTiEsitoContrFormatoFile().equals(EsitoControllo.WARNING.name())) {
                ...
                } else if (mock.getTiEsitoContrFormatoFile().equals(EsitoControllo.NEGATIVO.name())) {
                    // caso negativo
                ...
                } else if (mock.getTiEsitoContrFormatoFile().equals(EsitoControllo.DISABILITATO.name())) {
                    // caso disattivato
    ...
    private RispostaControlli elabCompDocMockWithVerificaFirmaAndFormatoResult(ComponenteVers componente,
            ...
            List<ComponenteVers> sottoComponentiAltri, long idTipoUd) {
        ...
        if (componente.getRifComponenteMM() != null && componente.getRifComponenteMM().isForzaFirmeFormati()) {
            ...
            if (sottoComponentiFirma.isEmpty() && sottoComponentiMarca.isEmpty() && sottoComponentiAltri.isEmpty()) {
                ...
                mock.setTiEsitoContrFormatoFile(EsitoControllo.DISABILITATO.name());
                ...
                if (tmpComponente.getForzaFormato() != null) {
                    ...
                    if (rs.getrLong() != -1) { // i controlli del formato potrebbero aver reso un'eccezione "666"
                        ...
                        if (rs.isrBoolean()) {
                            mock.setTiEsitoContrFormatoFile(EsitoControllo.POSITIVO.name());
                            mock.setDsMsgEsitoContrFormato("Controllo OK");
                        } else {
                            mock.setTiEsitoContrFormatoFile(EsitoControllo.NEGATIVO.name());
