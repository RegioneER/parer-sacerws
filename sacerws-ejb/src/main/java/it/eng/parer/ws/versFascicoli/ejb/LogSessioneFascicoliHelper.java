...
public class LogSessioneFascicoliHelper {
    ...
    public RispostaControlli verificaPartizioneFascicoloByAaStrutKo(RispostaWSFascicolo rispostaWs,
        ...
        try {
            String queryStr = "select count(t) from OrgVChkPartitionFascByAa t "
                    + "where t.idStrut = :idStrut and t.anno = :anno " + "and t.flPartFasckoOk = :flPartFasckoOk "
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviXmlFascicoloErr(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
            SyncFakeSessn sessione, VrsSesFascicoloErr fascicoloErr) {
        RispostaControlli tmpRispostaControlli = new RispostaControlli();
    ...
    public RispostaControlli cercaFascicoloKo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
        ...
        try {
            String queryStr = "select ud " + "from VrsFascicoloKo ud " + "where ud.orgStrut.idStrut = :idStrutIn "
                    + " and ud.cdKeyFascicolo = :cdKeyFascicolo " + " and ud.aaFascicolo = :aaFascicolo ";
            javax.persistence.Query query = entityManager.createQuery(queryStr, VrsFascicoloKo.class);
    ...
    public RispostaControlli scriviFascicoloKo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
        ...
        try {
            ...
            tmpFascicoloKo.setTsIniLastSes(convert(sessione.getTmApertura())); // coincidono: c'è una sola sessione
    ...
    public RispostaControlli scriviSessioneFascicoloKo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
            SyncFakeSessn sessione, VrsFascicoloKo fascicoloKo, FasFascicolo fascicoloOk) {
        RispostaControlli tmpRispostaControlli = new RispostaControlli();
        tmpRispostaControlli.setrBoolean(false);
        StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
        CompRapportoVersFascicolo esito = rispostaWs.getCompRapportoVersFascicolo();
            ...
            if (fascicoloKo != null) {
                ...
                if (updFasKo != null) {
                    if (updFasKo.getIdErrSacerPrinc() != null) {
                        fascicoloKo.setDecErrSacer(
                                entityManager.find(DecErrSacer.class, updFasKo.getIdErrSacerPrinc().longValue()));
                        fascicoloKo.setDsErrPrinc(LogSessioneUtils.getDsErrAtMaxLen(updFasKo.getDsErrPrinc()));
                    } else {
                        fascicoloKo.setDecErrSacer(null);
                        fascicoloKo.setDsErrPrinc(LogSessioneUtils.getDsErrAtMaxLen(CD_DS_ERR_DIVERSI));
                    }
                ...
                fascicoloKo.setIdSesFascicoloKoLast(new BigDecimal(tmpSesFascicoloKo.getIdSesFascicoloKo()));
    ...
    private void aggiornaConteggioMonContaFasKo(VrsFascicoloKo fascicoloKo) {
        ...
        javax.persistence.Query query = entityManager.createQuery(queryStr, MonContaFascicoliKo.class);
        query.setParameter("idStrutIn", fascicoloKo.getOrgStrut().getIdStrut());
        query.setParameter("dtRifConta", LogSessioneUtils.getDatePart(fascicoloKo.getTsIniLastSes()));
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli scriviXmlFascicoloKo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
