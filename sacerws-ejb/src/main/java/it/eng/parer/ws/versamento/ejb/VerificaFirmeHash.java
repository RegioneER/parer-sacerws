...
public class VerificaFirmeHash {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            for (DocumentoVers valueDocVers : strutV.getDocumentiAttesi()) {
                ...
                if (versamento.getStrutturaComponenti().effettuaVerificaFirma()
                        && myDocumentoVersVFirme.verificaDocumentoGenFirma(valueDocVers,
                                versamento.getStrutturaComponenti().getIdTipologiaUnitaDocumentaria(), versamento,
                                rispostaWs, myEsito.getConfigurazione())) {
            ...
            if (!versamento.getStrutturaComponenti().effettuaVerificaFirma()) {
                ...
                if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_FIRMA_1_5)) {
                    ...
                    versamento.getStrutturaComponenti().setTiEsitoVerifFirme(SacerIndication.DISABILITATO.name());
                    versamento.getStrutturaComponenti().setDsMsgEsitoVerifica(SacerIndication.DISABILITATO.message());
                ...
                if (bool != null && bool) {
                    if (verFirmeDocCodiceEsitoNeg) {
                        versamento.getStrutturaComponenti().setTiEsitoVerifFirme(SacerIndication.NEGATIVO.name());
                        versamento.getStrutturaComponenti().setDsMsgEsitoVerifica(SacerIndication.NEGATIVO.message());
                        myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
                                .setVerificaFirmeUnitaDocumentaria(ECEsitoPosNegWarDisType.NEGATIVO);
                    } else if (verFirmeDocCodiceEsitoWar) {
                        versamento.getStrutturaComponenti().setTiEsitoVerifFirme(SacerIndication.WARNING.name());
                        versamento.getStrutturaComponenti().setDsMsgEsitoVerifica(SacerIndication.WARNING.message());
                        myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
                                .setVerificaFirmeUnitaDocumentaria(ECEsitoPosNegWarDisType.WARNING);
                    } else {
                        versamento.getStrutturaComponenti().setTiEsitoVerifFirme(SacerIndication.POSITIVO.name());
                        versamento.getStrutturaComponenti().setDsMsgEsitoVerifica(SacerIndication.POSITIVO.message());
                        myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
    ...
    private void elabFlagSipVDB(VersamentoExt versamento, OrgStrut os) {
        //
        boolean isConfigurazioneOnSip = !Objects.isNull(versamento.getVersamento().getConfigurazione());
        //
        ...
        //
        versamento.getStrutturaComponenti().setFlagAbilitaVerificaFirma(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_VERIFICA_FIRMA, idStrut,
                        idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        //
        versamento.getStrutturaComponenti()
                .setFlagAbilitaVerificaFirmaSoloCrypto(configurationHelper
                        .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_VERIFICA_FIRMA_SOLO_CRYPTO,
        ...
        versamento.getStrutturaComponenti().setFlagAbilitaConservazioneNonFirmati(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_NON_FIRMATI,
        ...
        versamento.getStrutturaComponenti().setFlagAccettaConservazioneNonFirmatiNeg(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_NON_FIRMATI_NEG,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        /*
         * MAC #23544: La gestione del parametro FORZA_CONSERVAZIONE è opzionale sul SIP. Nel caso non siano configurati
        ...
        if (isConfigurazioneOnSip && versamento.getVersamento().getConfigurazione().isForzaConservazione() != null) {
            forzaCons = versamento.getVersamento().getConfigurazione().isForzaConservazione();
        } else {
            forzaCons = configurationHelper
                    .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_NON_FIRMATI_NEG, idStrut,
                            idAmbiente, idTipoUd)
                    .equals(CostantiDB.Flag.TRUE);
        }
        ...
        // forzature controlli
        versamento.getStrutturaComponenti().setConfigFlagForzaControlloRevoca(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_REVOCA_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setConfigFlagForzaControlloTrust(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_TRUST_VERS, idStrut,
                        idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setConfigFlagForzaControlloCrittografico(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_CRITTOG_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setConfigFlagForzaControlloCertificato(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_CERTIF_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setConfigFlagForzaControlloNonConformita(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_CONTR_NOCONF, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));

        // revoca
        versamento.getStrutturaComponenti().setFlagAbilitaControlloRevoca(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_REVOCA_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        // ocsp
        versamento.getStrutturaComponenti().setFlagAccettaControlloOCSPNegativo(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_OCSP_NEG, idStrut,
                        idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setFlagAccettaControlloOCSPNoScaric(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_OCSP_NOSCAR,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setFlagAccettaControlloOCSPNoValido(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_OCSP_NOVAL,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setGenerazioneRerortVerificaFirma(
                configurationHelper.getValoreParamApplicByTipoUd(ParametroApplFl.GENERAZIONE_REPORT_VERIFICA_FIRMA,
                        idStrut, idAmbiente, idTipoUd));

    }

    private void buildFlagsOnEsito(VersamentoExt versamento, EsitoVersamento myEsito, long idStrut, long idAmbiente,
            long idTipoUd) {
        ...
        myEsito.getConfigurazione().setAbilitaControlloCertificato(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_CERTIF_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAbilitaControlloCrittografico(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_CRITTOG_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAbilitaControlloTrust(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_TRUST_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCRLNegativo(configurationHelper.getValoreParamApplicByTipoUdAsFl(
                ParametroApplFl.FL_ACCETTA_CONTR_CRL_NEG, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCRLNoScaric(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CRL_NOSCAR,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCRLNoValida(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CRL_NOVAL,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCRLScaduta(configurationHelper.getValoreParamApplicByTipoUdAsFl(
                ParametroApplFl.FL_ACCETTA_CONTR_CRL_SCAD, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCertificatoNoFirma(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CERTIF_NOCERT,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCertificatoNoValido(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CERTIF_NOVAL,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCertificatoScaduto(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CERTIF_SCAD,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloCrittograficoNegativo(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_CRITTOG_NEG,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaControlloTrustNegativo(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_TRUST_NEG,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaFirmaNoDelibera45(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_FIRMA_GIUGNO_2011,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaFirmaNonConforme(configurationHelper.getValoreParamApplicByTipoUdAsFl(
                ParametroApplFl.FL_ACCETTA_FIRMA_NOCONF, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        myEsito.getConfigurazione().setAccettaFirmaSconosciuta(configurationHelper.getValoreParamApplicByTipoUdAsFl(
                ParametroApplFl.FL_ACCETTA_FIRMA_NOCONOS, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_VERIFICA_FORMATI_1_25)) {
            // Imposto da DB i valori di configurazione prettamente legati alla verifica
            // formati
            myEsito.getConfigurazione().setAbilitaControlloFormato(configurationHelper.getValoreParamApplicByTipoUdAsFl(
                    ParametroApplFl.FL_ABILITA_CONTR_FMT, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
            myEsito.getConfigurazione().setAccettaControlloFormatoNegativo(
                    configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_CONTR_FMT_NEG,
                            idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
            myEsito.getConfigurazione().setAccettaMarcaSconosciuta(
                    configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_MARCA_NOCONOS,
                            idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));
        }
        //
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_FIRMA_1_5)) {
