...
public class VerificaFirmeHash {
    ...
    public VerificaFirmeHash(VersamentoExt vers, RispostaWS risp) {
        versamento = vers;
        rispostaWs = risp;
    ...
    public void controlloFirmeMarcheHash() {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            elabFlagSipVDB(os);
            //
            buildFlagsOnEsito(myEsito, os.getIdStrut(), os.getOrgEnte().getOrgAmbiente().getIdAmbiente(),
            ...
            if (!versamento.getStrutturaComponenti().effettuaVerificaFirma()) {
                ...
                if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_FIRMA_1_5)) {
                    ...
                    versamento.getStrutturaComponenti()
                            .setTiEsitoVerifFirme(VerificaFirmaEnums.EsitoControllo.DISABILITATO.name());
                    versamento.getStrutturaComponenti()
                            .setDsMsgEsitoVerifica(VerificaFirmaEnums.EsitoControllo.DISABILITATO.message());
    ...
    private void elabFlagSipVDB(OrgStrut os) {
         ...
         */
        ConfigType configurazioneXML = versamento.getVersamento().getConfigurazione();
        // SIP vs DB
    ...
    private void buildFlagsOnEsito(EsitoVersamento myEsito, long idStrut, long idAmbiente, long idTipoUd) {
