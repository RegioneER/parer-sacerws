...
public class VersamentoExtAggAllPrsr {
    ...
    public void parseXML(SyncFakeSessn sessione, VersamentoExtAggAll versamento, RispostaWSAggAll rispostaWs) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            try {
                ...
                Unmarshaller tmpUnmarshaller = xmlVersCache.getVersReqCtxforUDAggAllegati().createUnmarshaller();
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            chiaveType.setAnno(Long.toString(parsedUnitaDoc.getIntestazione().getChiave().getAnno()));
            chiaveType.setNumero(parsedUnitaDoc.getIntestazione().getChiave().getNumero());
            chiaveType.setTipoRegistro(parsedUnitaDoc.getIntestazione().getChiave().getTipoRegistro());
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            if (parsedUnitaDoc.getConfigurazione() != null) {
                if (parsedUnitaDoc.getConfigurazione().isSimulaSalvataggioDatiInDB() != null) {
                    versamento.setSimulaScrittura(parsedUnitaDoc.getConfigurazione().isSimulaSalvataggioDatiInDB());
         ...
         */
        if (rispostaWs.getSeverity() == SeverityEnum.OK
                && !parsedUnitaDoc.getIntestazione().getVersione().equals(versamento.getVersioneWsChiamata())) {
            rispostaWs.setSeverity(SeverityEnum.ERROR);
            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.UD_001_013, parsedUnitaDoc.getIntestazione().getVersione());
            myEsito.getEsitoChiamataWS().setVersioneWSCorretta(ECEsitoPosNegType.NEGATIVO);
        }

        if (rispostaWs.getSeverity() == SeverityEnum.OK
                && !parsedUnitaDoc.getIntestazione().getVersatore().getUserID().equals(versamento.getLoginName())) {
            rispostaWs.setSeverity(SeverityEnum.ERROR);
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (rispostaWs.getSeverity() == SeverityEnum.OK
                ...
                myEsito.getEsitoXSD().setCodiceEsito(ECEsitoPosNegType.NEGATIVO);
                myEsito.getEsitoXSD()
                        .setUnivocitaIDComponenti("Errore: ci sono degli ID documento duplicati oppure un ID è nullo.");
            }
            ...
            if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                ...
                versamento.getStrutturaComponenti().setDataVersamento(
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (parsedUnitaDoc.getConfigurazione() != null) {
                ...
                if (parsedUnitaDoc.getConfigurazione().getTipoConservazione() != null
                        && parsedUnitaDoc.getConfigurazione().getTipoConservazione() == TipoConservazioneType.FISCALE) {
                    tmpConfigurazione.setTipoConservazione(TipoConservazioneType.FISCALE.name());
                }

                if (parsedUnitaDoc.getConfigurazione().getTipoConservazione() != null && parsedUnitaDoc
                        .getConfigurazione().getTipoConservazione() == TipoConservazioneType.MIGRAZIONE) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            myEsito.getUnitaDocumentaria().setEsitoUnitaDocumentaria(new ECUnitaDocAggAllType.EsitoUnitaDocumentaria());
        }
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (rispostaControlli.isrBoolean() || rispostaControlli.getrLong() == -1) {
                ...
                /*
                 * salvo il tipo di salvataggio (blob in tabella o filesystem) nel caso del versamento, questa
                 * informazione viene recuperata durante la fase dei controlli di versamento
                 */
                versamento.getStrutturaComponenti()
                ...
                // memorizzo tipo ud
                versamento.getStrutturaComponenti().setIdTipologiaUnitaDocumentaria(
                        (long) rispostaControlli.getrMap().get(RispostaControlli.ValuesOnrMap.ID_TIPO_UD.name()));

                // normalized cd_key
                versamento.getStrutturaComponenti().setNumeroUdNormalized((String) rispostaControlli.getrMap()
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (rispostaControlli.getCodErr() != null) {
                ...
                versamento.getStrutturaComponenti().setDtInizioCalcoloNewUrn(rispostaControlli.getrDate());
            }
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR && myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
            ...
            boolean flForzaAggiuntaDocumento = parsedUnitaDoc.getConfigurazione() != null
                    && parsedUnitaDoc.getConfigurazione().isForzaAggiuntaDocumento() != null
                            ? parsedUnitaDoc.getConfigurazione().isForzaAggiuntaDocumento().booleanValue()
            ...
            if (scud == CostantiDB.StatoConservazioneUnitaDoc.IN_ARCHIVIO
                ...
                if (versamento.getStrutturaComponenti().isFlagAbilitaForzaDocAgg() && isWarn) {
                    ...
                    // add on list
                    versamento.listErrAddWarningUd(descChiaveUD, MessaggiWSBundle.UD_013_001, descChiaveUD);
                } else {
                    setRispostaWsError(rispostaWs, rispostaControlli);
                    myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
                            .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
                    ...
                    // add on list
                    versamento.listErrAddErrorUd(descChiaveUD, MessaggiWSBundle.UD_013_001, descChiaveUD);
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            // aggiunto
            DocumentoVers valueDocVers = versamento.getStrutturaComponenti().getDocumentiAttesi().get(0);

            rispostaControlli = controlliSemantici.trovaNuovoProgDocumento(
                    versamento.getStrutturaComponenti().getIdUnitaDoc(), valueDocVers.getCategoriaDoc().getValoreDb());
            if (rispostaControlli.getrLong() != -1) {
            ...
            /*
             * verifica che il documento che si intende inserire non sia già presente. (in pratica verifica che
             * all'interno della stessa UD non sia presente un documento con IDDocumento uguale)
             */
            if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                rispostaControlli.reset();
                String tmpIdDocumento;
                tmpIdDocumento = versamento.getStrutturaComponenti().getDocumentiAttesi().get(0).getRifDocumento()
                        .getIDDocumento();
                ...
                rispostaControlli = controlliSemantici.checkDocumentoInUd(
                        versamento.getStrutturaComponenti().getIdUnitaDoc(), tmpIdDocumento, tmpDescUd);
                if (!rispostaControlli.isrBoolean()) {
                    setRispostaWsError(rispostaWs, rispostaControlli);
                    myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
                            .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
            ...
            if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                ...
                tmpDocumento.getEsitoDocumento().setUnivocitaOrdinePresentazione(ECEsitoPosNegType.POSITIVO);
                tmpDocumento.getEsitoDocumento().setVerificaTipoDocumento(ECEsitoPosNegType.POSITIVO);
                ...
                myDocumentoVersPrsr.parseDocumentoGen(valueDocVers, tmpDocumento, versamento, rispostaWs);
            }
            myAvanzamentoWs.resetFase().setFase("verifica semantica documento - fine").logAvanzamento();
        }
    ...
    private void elabFlagSipVDB(EsitoVersAggAllegati myEsito, OrgStrut os, VersamentoExtAggAll versamento,
            UnitaDocAggAllegati parsedUnitaDoc) {
        ...
        boolean isForzaAccettazione = parsedUnitaDoc.getConfigurazione() != null
                ...
                && parsedUnitaDoc.getConfigurazione().isForzaAccettazione().booleanValue();
        // flags
        versamento.getStrutturaComponenti().setFlagVerificaHash(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_CONTR_HASH_VERS,
                        idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        boolean flForzaHashVers = configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_HASH_VERS, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE);
        boolean flAccettaContrHashNeg = configurationHelper.getValoreParamApplicByTipoUdAsFl(
                ParametroApplFl.FL_ACCETTA_CONTR_HASH_NEG, idStrut, idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE);
        ...
        //
        versamento.getStrutturaComponenti().setFlagAbilitaForzaDocAgg(
                configurationHelper.getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ABILITA_FORZA_DOC_AGG, idStrut,
                        idAmbiente, idTipoUd).equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setFlagForzaDocAgg(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_DOC_AGG, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti().setFlagAccettaDocNeg(configurationHelper
                .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_ACCETTA_DOC_AGG_NEG, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));

        // default pre 1.5
        versamento.getStrutturaComponenti()
                .setConfigFlagForzaFormato(configurationHelper
                        .getValoreParamApplicByTipoUdAsFl(ParametroApplFl.FL_FORZA_FMT, idStrut, idAmbiente, idTipoUd)
                        .equals(CostantiDB.Flag.TRUE));
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_ABILITA_FORZA_1_5)) {
            ...
            if (parsedUnitaDoc.getConfigurazione() != null) {
                if (parsedUnitaDoc.getConfigurazione().isForzaAggiuntaDocumento() != null) {
                    forzaAggDoc = parsedUnitaDoc.getConfigurazione().isForzaAggiuntaDocumento().booleanValue();
                } else {
                ...
                flForzaHashVers = parsedUnitaDoc.getConfigurazione().isForzaHash() != null
                        ? parsedUnitaDoc.getConfigurazione().isForzaHash().booleanValue() : flForzaHashVers;// se non da
                                                                                                            // SIP
            }
            ...
            myEsito.getConfigurazione()
                    .setAbilitaForzaAggiuntaDocumento(versamento.getStrutturaComponenti().isFlagAbilitaForzaDocAgg());
            //
            ...
            boolean flForzaFmt = parsedUnitaDoc.getConfigurazione() != null
                    && parsedUnitaDoc.getConfigurazione().isForzaFormatoFile() != null
                            ...
                            : versamento.getStrutturaComponenti().isConfigFlagForzaFormato();// se non da SIP è immutato
                                                                                             // (vedi
     ...
     *
     * @param myEsito
     *            esito versamento {@link EsitoVersAggAllegati} con flag impostati
     */
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_URN_SIP_1_5)) {
            ...
            String tmpUrn = MessaggiWSFormat
                    .formattaBaseUrnDoc(versamento.getStrutturaComponenti().getUrnPartVersatore(),
                            versamento.getStrutturaComponenti().getUrnPartChiaveUd(), versamento
                                    .getStrutturaComponenti().getDocumentiAttesi().get(0).getUrnPartDocumentoNiOrdDoc(),
                            Costanti.UrnFormatter.URN_DOC_FMT_STRING_V2);

            // calcolo URN del SIP del documento
            String urnSip = MessaggiWSFormat.formattaUrnSip(tmpUrn, Costanti.UrnFormatter.URN_SIP_DOC);
    ...
    private void setRispostaWsError(RispostaWSAggAll rispostaWs, RispostaControlli rispostaControlli) {
        rispostaWs.setSeverity(SeverityEnum.ERROR);
