...
public class SalvataggioUpdVersamento {
	...
	try {
	    ...
	    AroUnitaDoc tmpAroUnitaDoc = entityManager.find(AroUnitaDoc.class,
		    strutturaUpdVers.getIdUd(), LockModeType.PESSIMISTIC_WRITE, properties);
	    ...
	    tmpRispostaControlli = salvataggioPregVersamentoAroHelper
		    .salvaCdKeyNormUnitaDocumentaria(tmpAroUnitaDoc, strutturaUpdVers);
	    // errore query
	    ...
	    tmpRispostaControlli = salvataggioPregVersamentoAroHelper
		    .aggiornaPregCompDocUrnSip(tmpAroUnitaDoc, strutturaUpdVers);
	    // errore query
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		...
		// salvo dati iniziali
		tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniUnitaDoc(
			rispostaWs, versamento, sessione, tmpAroUnitaDoc, strutturaUpdVers);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		    return false;
		...
		tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniArchivSec(
			rispostaWs, versamento, sessione, tmpAroUnitaDoc, tmpAroVersIniUnitaDoc,
			strutturaUpdVers);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		...
		tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniLinkUnitaDoc(
			rispostaWs, versamento, sessione, tmpAroUnitaDoc, tmpAroVersIniUnitaDoc,
			strutturaUpdVers);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		...
		// è presente ...
		if (tmpRispostaControlli.getrLong() != -1) {
		    AroUsoXsdDatiSpec tmpUsoXsdDatiSpec = (AroUsoXsdDatiSpec) tmpRispostaControlli
			    .getrObject();

		    // per ogni record in ARO_VALORE_ATTRIB_DATI_SPEC relativo al xsd usato
		    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniDatiSpecUd(
			    sessione, tmpAroVersIniUnitaDoc, tmpUsoXsdDatiSpec, backendMetadata,
		...
		tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.getAroVersIniDoc(
			...
			updDocumentoVers.getIdRecDocumentoDB());
		...
		if (tmpRispostaControlli.getrLong() == -1) {
		    //
		    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniDoc(
			    rispostaWs, versamento, sessione, tmpAroVersIniUnitaDoc,
			    updDocumentoVers.getIdRecDocumentoDB(), strutturaUpdVers);
		    if (!tmpRispostaControlli.isrBoolean()) {
			context.setRollbackOnly();
			this.impostaErrore(rispostaWs, tmpRispostaControlli);
			return false;
		    ...
		    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper
			    .checkUsoXsdDatiSpecifici(updDocumentoVers.getIdRecDocumentoDB(),
				    CostantiDB.TipiUsoDatiSpec.VERS,
			...
			// per ogni record in ARO_VALORE_ATTRIB_DATI_SPEC relativo al xsd usato
			tmpRispostaControlli = salvataggioUpdIniVersamentoHelper
				.scriviAroIniDatiSpecDoc(sessione, tmpAroVersIniUnitaDoc,
					tmpAroVersIniDoc, tmpUsoXsdDatiSpec, backendMetadata,
					versIniDatiSpecBlob, strutturaUpdVers);
			if (!tmpRispostaControlli.isrBoolean()) {
			    context.setRollbackOnly();
			    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		    ...
		    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper
			    .checkUsoXsdDatiSpecifici(updDocumentoVers.getIdRecDocumentoDB(),
				    CostantiDB.TipiUsoDatiSpec.MIGRAZ,
				    CostantiDB.TipiEntitaSacer.DOC);
		...
		for (UpdComponenteVers updComponenteVers : updDocumentoVers
			.getUpdComponentiAttesi()) {

		    AroVersIniComp tmpAroVersIniComp = null;
		    // se per il versamento iniziale dell’unita doc non e’ presente il versamento
		    // iniziale del documento
		    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.getAroVersIniComp(
			    tmpAroVersIniDoc.getIdVersIniDoc(), updComponenteVers.getIdRecDB());
		    ...
		    if (tmpRispostaControlli.getrLong() == -1) {
			//
			tmpRispostaControlli = salvataggioUpdIniVersamentoHelper.scriviAroIniComp(
				rispostaWs, versamento, sessione, tmpAroVersIniDoc,
				updComponenteVers.getIdRecDB(), strutturaUpdVers);
			if (!tmpRispostaControlli.isrBoolean()) {
			    context.setRollbackOnly();
			    this.impostaErrore(rispostaWs, tmpRispostaControlli);
			    return false;
			...
			tmpRispostaControlli = salvataggioUpdIniVersamentoHelper
				.checkUsoXsdDatiSpecifici(updComponenteVers.getIdRecDB(),
					CostantiDB.TipiUsoDatiSpec.VERS,
			...
			if (tmpRispostaControlli.getrLong() != -1) {
			    ...
			    // per ogni record in ARO_VALORE_ATTRIB_DATI_SPEC relativo al xsd usato
			    tmpRispostaControlli = salvataggioUpdIniVersamentoHelper
				    .scriviAroIniDatiSpecComp(sessione, tmpAroVersIniUnitaDoc,
					    tmpAroVersIniDoc, tmpAroVersIniComp, tmpUsoXsdDatiSpec,
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdUnitaDoc(versamento,
		    sessione, tmpAroUnitaDoc, pgAroUpdUnitaDoc, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroXmlUpdUnitaDoc(
		    rispostaWs, versamento, sessione, tmpAroUnitaDoc, tmpAroUpdUnitaDoc,
		    backendMetadata, sipBlob, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper
		    .scriviAroWarnUpdUnitaDocForUD(versamento, tmpAroUpdUnitaDoc);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroWarnUpdUnitaDocForDoc(
		    versamento, tmpAroUpdUnitaDoc, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroWarnUpdUnitaDocForComp(
		    versamento, tmpAroUpdUnitaDoc, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdArchivSec(rispostaWs,
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdLinkUnitaDoc(
		    versamento, tmpAroUnitaDoc, tmpAroUpdUnitaDoc, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdDatiSpecUnitaDoc(
		    versamento, sessione, tmpAroUnitaDoc, tmpAroUpdUnitaDoc,
		    TiUsoXsdAroUpdDatiSpecUnitaDoc.VERS, TiEntitaAroUpdDatiSpecUnitaDoc.UPD_UNI_DOC,
		    backendMetadata, updDatiSpecBlob, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdDatiSpecUnitaDoc(
		    ...
		    TiEntitaAroUpdDatiSpecUnitaDoc.UPD_UNI_DOC, backendMetadata, updDatiSpecBlob,
		    strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviElvUpdUdDaElabElenco(
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper
		    .scriviMonKeyTotalUd(tmpAroUnitaDoc, tmpAroUpdUnitaDoc, strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdVersamentoHelper.scriviAroUpdDocUnitaDoc(
		    versamento, sessione, tmpAroUpdUnitaDoc, backendMetadata, updDatiSpecBlob,
		    strutturaUpdVers);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
	    ...
	    String userid = versamento.getVersamento().getUnitaDocumentaria().getIntestazione()
	    ...
	    tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
		    .aggiornaStatoConservazione(tmpAroUnitaDoc, strutturaUpdVers, nomeAgente);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
		...
		tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
			.aggiornaProfiloArchivistico(versamento, tmpAroUnitaDoc, tmpAroUpdUnitaDoc);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		    return false;
		...
		if (strutturaUpdVers.getIdRecUsoXsdProfiloNormativo() != null) {
		    tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
			    .aggiornaProfiloNormativo(tmpAroUnitaDoc, strutturaUpdVers);
		}
		...
		tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
			.aggiornaDocumentiCollegati(tmpAroUnitaDoc, tmpAroUpdUnitaDoc);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		    return false;
		...
		tmpRispostaControlli = salvataggioUpdAroVersamentoHelper.aggiornaDatiSpecificiUd(
		...
		tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
			.aggiornaDocumento(tmpAroUpdUnitaDoc, strutturaUpdVers);
		if (!tmpRispostaControlli.isrBoolean()) {
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		    return false;
	    ...
	    tmpRispostaControlli = salvataggioUpdAroVersamentoHelper
		    .aggiornaAggregazioni(tmpAroUnitaDoc);
	    if (!tmpRispostaControlli.isrBoolean()) {
		context.setRollbackOnly();
		this.impostaErrore(rispostaWs, tmpRispostaControlli);
		return false;
		...
		tmpRispostaControlli = updLogSessioneHelper
			.lockAndGetMonContaSesUpdUdKo(tmpUpdUnitaDocKo);
		if (!tmpRispostaControlli.isrBoolean()) {
		    // errore su db
		    context.setRollbackOnly();
		    this.impostaErrore(rispostaWs, tmpRispostaControlli);
		...
		if (tmpMonKeyTotalUdKo != null) {
		    tmpRispostaControlli = salvataggioUpdVersamentoHelper
			    .ereditaVrsSesUpdUnitaDocKoRisolte(tmpAroUpdUnitaDoc, tmpUpdUnitaDocKo,
				    tmpMonKeyTotalUdKo);
		    if (!tmpRispostaControlli.isrBoolean()) {
			context.setRollbackOnly();
			this.impostaErrore(rispostaWs, tmpRispostaControlli);
		...
		ObjectStorageResource res = objectStorageService.createResourcesInSipAggMd(
			backendMetadata.getBackendName(), sipBlob, versamento.getStrutturaUpdVers(),
			tmpAroUpdUnitaDoc, getIdStrut(versamento));
		...
		for (Map.Entry<DatiSpecLinkOsKeyMap, Map<String, String>> versIniDatiSpecBlobEntry : versIniDatiSpecBlob
			.entrySet()) {
		    res = objectStorageService.createResourcesInVersIniDatiSpecAggMd(
			    backendMetadata.getBackendName(), versIniDatiSpecBlobEntry.getValue(),
			    versIniDatiSpecBlobEntry.getKey().getIdEntitySacer(),
			    TiEntitaSacerAroVersIniDatiSpec.valueOf(
				    versIniDatiSpecBlobEntry.getKey().getTipiEntitaSacer()),
			    versamento.getStrutturaUpdVers(), tmpAroUpdUnitaDoc,
			    getIdStrut(versamento));
		    log.debug(
			    "Salvati i Dati Specifici relativi ai metadati iniziali nel bucket {} con chiave {} ",
		...
		for (Map.Entry<DatiSpecLinkOsKeyMap, Map<String, String>> updDatiSpecBlobEntry : updDatiSpecBlob
			.entrySet()) {
		    res = objectStorageService.createResourcesInUpdDatiSpecAggMd(
			    backendMetadata.getBackendName(), updDatiSpecBlobEntry.getValue(),
			    updDatiSpecBlobEntry.getKey().getIdEntitySacer(),
			    TiEntitaAroUpdDatiSpecUnitaDoc
				    .valueOf(updDatiSpecBlobEntry.getKey().getTipiEntitaSacer()),
			    versamento.getStrutturaUpdVers(), tmpAroUpdUnitaDoc,
			    getIdStrut(versamento));
		    log.debug(
			    "Salvati i Dati Specifici dell'aggiornamento metadati nel bucket {} con chiave {} ",
			    res.getBucket(), res.getKey());
		}
	    }
	    // end MEV#29276
		...
		pl.setAaKeyUnitaDoc(versamento.getVersamento().getUnitaDocumentaria()
			.getIntestazione().getChiave().getAnno());
