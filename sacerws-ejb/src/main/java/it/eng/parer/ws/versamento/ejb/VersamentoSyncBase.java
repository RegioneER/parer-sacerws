...
import it.eng.parer.ws.versamento.dto.SyncFakeSessn.TipiSessioneVersamento;
import it.eng.parer.ws.versamento.dto.VersamentoExt;
import it.eng.parer.ws.versamento.utils.VerificaFirmeHash;
import it.eng.parer.ws.versamento.utils.VersamentoExtPrsr;
...
public abstract class VersamentoSyncBase {
    ...
    public void init(RispostaWS rispostaWs, AvanzamentoWs avanzamento, VersamentoExt versamento,
        ...
        if (!rs.isrBoolean()) {
            ...
            myEsito.getEsitoXSD().setCorrispondenzaAnnotazioniDichiarate(ECEsitoPosNegType.POSITIVO);
    ...
    public void verificaCredenziali(String loginName, String password, String indirizzoIp, RispostaWS rispostaWs,
    ...
    public void parseXML(SyncFakeSessn sessione, RispostaWS rispostaWs, VersamentoExt versamento) {
        ...
        try {
            VersamentoExtPrsr tmpPrsr = new VersamentoExtPrsr(versamento, rispostaWs);
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            RispostaControlli tmpRispostaControlli;
            tmpRispostaControlli = myControlliSubStrut.calcolaSubStrut(versamento.getStrutturaComponenti());
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.ERROR) {
            if (rispostaWs.isErroreElementoDoppio()) {
                ...
                tmpRispostaControlli = myRapportoVersBuilder.produciEsitoVecchioRapportoVers(versamento,
                        rispostaWs.getIdElementoDoppio(), myEsito);
                if (tmpRispostaControlli.getCodErr() != null) {
                    rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
                    rispostaWs.setEsitoWsError(tmpRispostaControlli.getCodErr(), tmpRispostaControlli.getDsErr());
                    logger.error("Eccezione nella fase di produzione Rapporto di versamento UD Doppia "
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            if (versamento.getStrutturaComponenti().getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE
                ...
                try {
                    ...
                    logger.error("Eccezione nella fase di controllo dello spazio libero sul file system TPI ", e);
                }
            }
        }
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.ERROR) {
    ...
    public void controllaFirmeAndHash(RispostaWS rispostaWs, VersamentoExt versamento) {
        ...
        if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
            try {
                VerificaFirmeHash veriFirme = new VerificaFirmeHash(versamento, rispostaWs);
        ...
        // solo dopo la effettiva memorizzazione in Tivoli
        {
            if (versamento.getStrutturaComponenti().getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE
                    && versamento.getStrutturaComponenti().isTpiAbilitato()) {
                myEsito.getUnitaDocumentaria().setStatoConservazione(ECStatoConsType.IN_ATTESA_MEMORIZZAZIONE);
            } else {
                myEsito.getUnitaDocumentaria().setStatoConservazione(ECStatoConsType.IN_ATTESA_SCHED);
            }
        }
    }

    public void salvaTutto(SyncFakeSessn sessione, RispostaWS rispostaWs, VersamentoExt versamento) {
        ...
        if (prosegui && !versamento.isSimulaScrittura()) {
            ...
            if (prosegui && rispostaWs.getErrorType() != IRispostaWS.ErrorTypeEnum.DB_FATAL) {
                // DEBUG: togliere il commento!
                this.commit(rispostaWs);
                // DEBUG: commentare la riga sotto!!
                // this.rollback(rispostaWs);
            }
        }
    }
    ...
    protected void commit(IRispostaWS rispostaWs) {
        try {
            ...
            rispostaWs.setErrorType(IRispostaWS.ErrorTypeEnum.DB_FATAL);
            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                    "Errore nella fase di commit transazione db del EJB " + e.getMessage());
    ...
    protected void rollback(IRispostaWS rispostaWs) {
        try {
            ...
            rispostaWs.setErrorType(IRispostaWS.ErrorTypeEnum.DB_FATAL);
            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                    "Errore nella fase di rollack transazione db del EJB " + e.getMessage());
