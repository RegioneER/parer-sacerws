...
public class DocumentoVersPrsr {
    ...
    public void parseDocumentoGen(DocumentoVers documento, ECDocumentoType esitoDoc,
	    AbsVersamentoExt versamento, IRispostaWS rispostaWs) {
	//
	...
	documento.setUrnPartDocumento(MessaggiWSFormat
		.formattaUrnPartDocumento(documento.getCategoriaDoc(), documento.getProgressivo()));

	// urn v2
	// DOC<progressivo doc>
	documento.setUrnPartDocumentoNiOrdDoc(MessaggiWSFormat.formattaUrnPartDocumento(
		CategoriaDocumento.Documento, documento.getNiOrdDoc(), true,
		Costanti.UrnFormatter.DOC_FMT_STRING_V2, Costanti.UrnFormatter.PAD5DIGITS_FMT));

	// verifica il tipo documento
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    this.verificaTipoDoc(documento, esitoDoc, versamento, rispostaWs, rispostaControlli);
	}

	// verifico la presenza dei DATI FISCALI - non devono essere presenti in alcun caso.
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    this.parseDatiFisc(documento, esitoDoc, rispostaWs, rispostaControlli);
	}

	// verifico la corrispondenza dei DATI SPECIFICI del Documento
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    this.parseDatiSpecDoc(documento, esitoDoc, versamento, rispostaWs);
	}

	// verifico tipo struttura
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    this.verificaTipoStruttura(documento, esitoDoc, versamento, rispostaWs,
		    rispostaControlli);
	}
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    this.verificaComponenti(documento, esitoDoc, versamento, rispostaWs, rispostaControlli);
	}
    }
    ...
    private void parseDatiFisc(DocumentoVers documento, ECDocumentoType esitoDoc,
	    IRispostaWS rispostaWs, RispostaControlli rispostaControlli) {
	if (documento.getRifDocumento().getDatiFiscali() != null) {
	    ...
	    // Documento {0}: L''utilizzo del Tag &lt;DatiFiscali&gt; è DEPRECATO dal 17 Ottobre
	    // 2013
	...
	for (ComponenteVers valueCompVers : documento.getFileAttesi()) {
	    if (rispostaWs.getSeverity() != SeverityEnum.ERROR
		    && valueCompVers.getMySottoComponente() == null) {
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    ...
		    //
		    valueCompVers.setUrnPartComponenteNiOrdDoc(MessaggiWSFormat
			    .formattaUrnPartComponente(documento.getUrnPartDocumentoNiOrdDoc(),
				    valueCompVers.getMyComponente().getOrdinePresentazione(),
				    Costanti.UrnFormatter.COMP_FMT_STRING_V2,
				    Costanti.UrnFormatter.PAD5DIGITS_FMT));
		...
		if (versamento.getModificatoriWSCalc()
		    ...
		    // nome componente
		    componenteResp
			    .setNomeComponente(valueCompVers.getMyComponente().getNomeComponente());
		    // urn versato
		    ...
		    // id componente
		    componenteResp.setIDComponenteVersato(
			    valueCompVers.getMyComponente().getIDComponenteVersato());
		}

		// se necessario, crea e verifica la validità del nome del file sul file system di
		// destinazione
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR
		    ...
		    String tmpFilePath = MessaggiWSFormat.formattaFilePathArk(
			    ...
			    versamento.getStrutturaComponenti().getSubPathUnitaDocArk(),
			    tmpNomeFile);
			...
			// il componente {0} non può essere archiviato su filesystem perchè il suo
			// pathname supera i 255
			// caratteri
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.FILESYS_002_001,
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			...
			componenteResp.getEsitoComponente()
				.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			esitoDoc.getEsitoDocumento().setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			esitoDoc.getEsitoDocumento()
				.setUnivocitaOrdinePresentazione(ECEsitoPosNegType.NEGATIVO);
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    // salvo il controlloUnivocitaOrdinePresentazione
		    if (!ordiniDiPresentazioneUnici.contains(Long
			    .valueOf(valueCompVers.getMyComponente().getOrdinePresentazione()))) {
			ordiniDiPresentazioneUnici.add(Long
				.valueOf(valueCompVers.getMyComponente().getOrdinePresentazione()));
			// salvo i parametri dell'esito
			// ordinePresentazione
			componenteResp.setOrdinePresentazione(BigInteger
				.valueOf(valueCompVers.getMyComponente().getOrdinePresentazione()));
		    } else {
			// sollevo l'errore di connessione UnivocitaOrdinePresentazione
			setRispostaWsError(rispostaWs, rispostaControlli);
			...
			esitoDoc.getEsitoDocumento()
				.setUnivocitaOrdinePresentazione(ECEsitoPosNegType.NEGATIVO);
			// "Documento {0}: <OrdinePresentazione>{1}</OrdinePresentazione> del
			// componente <ID>{2}</ID>
			// non sequenziale"
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    ...
		    rispostaControlli = controlliSemantici.checkTipoComponente(
			    valueCompVers.getMyComponente().getTipoComponente(),
			    documento.getIdTipoStrutturaDB(), tipologiaComponente,
			    valueCompVers.getChiaveComp());
		    if (!rispostaControlli.isrBoolean()) {
			setRispostaWsError(rispostaWs, rispostaControlli);
			componenteResp.getEsitoComponente()
			...
			// tipoComponete
			componenteResp.setTipoComponente(
				valueCompVers.getMyComponente().getTipoComponente());
			// urn
			componenteResp.setURN(valueCompVers.getMyComponente().getUrnVersato());
			// memorizzo in ComponenteVers il riferimento alla classe di esito relativo
			// a questo componente
			valueCompVers.setRifComponenteResp(componenteResp);
		    }
		}

		// Errore: se il tag "TipoSupportoComponente" del XML di versamento è diverso da
		// RIFERIMENTO, ed è
		// presente il tag "Riferimento"
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    if ((valueCompVers.isPresenteRifMeta()) && (valueCompVers
			    .getTipoSupporto() != ComponenteVers.TipiSupporto.RIFERIMENTO)) {
			...
			componenteResp.getEsitoComponente()
				.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			componenteResp.getEsitoComponente()
				.setVerificaTipoSupportoComponente("NEGATIVO");
			// "Il tag Riferimento non deve essere valorizzato per
			// TipoSupportoComponente diverso da
			// RIFERIMENTO"
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_002_001,
				valueCompVers.getChiaveComp());
		    } else {
			componenteResp.getEsitoComponente()
				.setVerificaTipoSupportoComponente("POSITIVO");
		    }
		}

		// Errore: se il tag "TipoSupportoComponente" del XML di versamento è RIFERIMENTO, e
		// non è presente il
		// tag "Riferimento".
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    if ((!valueCompVers.isPresenteRifMeta()) && (valueCompVers
			...
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_002_002,
				valueCompVers.getChiaveComp());
		    } else {
			componenteResp.getEsitoComponente()
				.setVerificaTipoSupportoComponente("POSITIVO");
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			...
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_002_004,
				valueCompVers.getChiaveComp());
		    }
		}
		...
		if ((rispostaWs.getSeverity() != SeverityEnum.ERROR)
			&& (valueCompVers.getMyComponente()
				.getTipoRappresentazioneComponente() != null)
			...
			DecTipoRapprComp decTipoRapprComp = (DecTipoRapprComp) rispostaControlli
				.getrObject();
			if (decTipoRapprComp.getTiOutputRappr() != null && decTipoRapprComp
				.getTiOutputRappr().equals(CostantiDB.TipiOutputRappr.DIP.name())) {
			    trovatoTipoRappComp = true;
			    // salvo l'id del tipo rappresentazione
			    valueCompVers.setIdTipoRappresentazioneComponente(
				    decTipoRapprComp.getIdTipoRapprComp());
			    valueCompVers.setIdFormatoFileDocCont(decTipoRapprComp
				    .getDecFormatoFileDocCont().getIdFormatoFileDoc());
			    if (decTipoRapprComp.getDecFormatoFileDocConv() != null) {
				valueCompVers.setIdFormatoFileDocConv(decTipoRapprComp
					.getDecFormatoFileDocConv().getIdFormatoFileDoc());
			    }
				...
				valueCompVers.setAlgoritmoRappr(CostantiDB.TipoAlgoritmoRappr
					.fromString(decTipoRapprComp.getTiAlgoRappr()));
			    }
			    // salvo l'esito positivo
			    componenteResp.getEsitoComponente()
			    ...
			    componenteResp.getEsitoComponente()
				    .setVerificaTipoRappresentazione(ECEsitoPosNegType.NEGATIVO);
			    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_003_002,
				    valueCompVers.getChiaveComp(), valueCompVers.getMyComponente()
					    .getTipoRappresentazioneComponente());
			}
		    }
		}

		// se presente tipo Rappresentazione Componente, verifico che il salvataggio avvenga
		// su BLOB
		if ((rispostaWs.getSeverity() != SeverityEnum.ERROR && trovatoTipoRappComp)) {
			...
			componenteResp.getEsitoComponente()
				.setVerificaTipoRappresentazione(ECEsitoPosNegType.NEGATIVO);
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_003_004,
				valueCompVers.getChiaveComp());
		    }
		}

		// se presente tipo Rappresentazione Componente, verifico che il tipo supporto sia
		// FILE
		if ((rispostaWs.getSeverity() != SeverityEnum.ERROR && trovatoTipoRappComp)) {
		    if (valueCompVers.getTipoSupporto() != ComponenteVers.TipiSupporto.FILE) {
			setRispostaWsError(rispostaWs, rispostaControlli);
		...
		// se presente tipo Rappresentazione Componente, verifico che il tipo uso componente
		// sia CONTENUTO
		if ((rispostaWs.getSeverity() != SeverityEnum.ERROR && trovatoTipoRappComp)) {
		    if (!valueCompVers.getTipoUso()
			...
			componenteResp.getEsitoComponente()
				.setVerificaTipoRappresentazione(ECEsitoPosNegType.NEGATIVO);
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_003_005,
				valueCompVers.getChiaveComp());
		    }
		}
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			...
			if (valueCompVers.getMyComponente().getNomeComponente() == null
			    ...
			    // Componente {0}: Il tag <NomeComponente> deve essere valorizzato (il
			    // componente ha un tipo
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR
		    ...
		    chiave.setAnno(Long
			    .valueOf(valueCompVers.getMyComponente().getRiferimento().getAnno()));
		    chiave.setNumero(valueCompVers.getMyComponente().getRiferimento().getNumero());
		    chiave.setTipoRegistro(
			    valueCompVers.getMyComponente().getRiferimento().getTipoRegistro());
			...
			rispostaWs.setErrorMessage(
				"Tag Riferimento del componente definito, ma U.D. non presente");

			componenteResp.getEsitoComponente()
				.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			componenteResp.getEsitoComponente().setVerificaRiferimentoUnitaDocumentaria(
				ECEsitoPosNegType.NEGATIVO);
			// Componente {0}: la UD identificata dal Riferimento {1} non è presente nel
			// sistema
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_008_003,
				valueCompVers.getChiaveComp(),
				MessaggiWSFormat.formattaUrnPartUnitaDoc(chiave));
		    } else {
			// verifico che l'unità doc riferita sia composta da un solo componente e
			// che questo sia di tipo
			// CONVERTITORE
			// che sia stata salvata su BLOB e che abbia tipo supporto uguale a FILE
			rispostaControlli.getrLong();
			rispostaControlli = controlliSemantici.checkRiferimentoUD(chiave,
				versamento.getStrutturaComponenti().getIdStruttura(),
				ControlliSemantici.TipologieComponente.COMPONENTE,
				valueCompVers.getChiaveComp());
			if (!rispostaControlli.isrBoolean()) {
			    setRispostaWsError(rispostaWs, rispostaControlli);
			    componenteResp.getEsitoComponente()
				    .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			    componenteResp.getEsitoComponente()
				    .setVerificaRiferimentoUnitaDocumentaria(
					    ECEsitoPosNegType.NEGATIVO);
			    rispostaWs.setEsitoWsError(rispostaControlli.getCodErr(),
				    rispostaControlli.getDsErr());
			} else {
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR
		    ...
		    String descFormatoFileVers = valueCompVers.getMyComponente()
			    .getFormatoFileVersato();
		    if (descFormatoFileVers != null && (descFormatoFileVers.equals(
			...
			if (!valueCompVers.getTipoUso().equals(CostantiDB.TipoUsoComponente.FIRMA)
				&& !valueCompVers.getTipoUso()
					.equals(CostantiDB.TipoUsoComponente.MARCA)) {
			    // se il componenete che sto verificando NON è una firma o una
			    // marca, provo a farlo versare comunque:
			    // verifico se è ammissibile il formato file "sconosciuto"
			    // (parer/unkonwn)
			    rispostaControlli = controlliSemantici.checkFormatoFileVersato(
				    versamento.getXmlDefaults().get(ParametroApplDB.FMT_UNKNOWN),
				    versamento.getStrutturaComponenti().getIdStruttura(),
			    ...
			    // se non è ammissibile neanche il formato "sconosciuto" la risposta del
			    // ws resta ERRORE
			}
		    }
			...
			// verifico che il formato del file versato coincida con quello atteso dal
			// convertitore
			if (trovatoTipoRappComp
				&& valueCompVers.getIdFormatoFileVers() != valueCompVers
					.getIdFormatoFileDocCont()) {
			    setRispostaWsError(rispostaWs, rispostaControlli);
			    componenteResp.getEsitoComponente()
				    .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			    componenteResp.getEsitoComponente()
				    .setVerificaAmmissibilitaFormato(ECEsitoPosNegType.NEGATIVO);
			    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_006_002,
			...
			rispostaWs.setErrorMessage(
				"Formato file non ammesso per struttura versante e tipo componente");
			componenteResp.getEsitoComponente()
				.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			componenteResp.getEsitoComponente()
				.setVerificaAmmissibilitaFormato(ECEsitoPosNegType.NEGATIVO);
			// Componente {0}: il Formato {1} non è presente nel DB per la struttura
			// versante, il tipo
			// struttura e il tipo componente
			rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_006_001,
				valueCompVers.getChiaveComp(), descFormatoFileVers);
		    }
		}

		// se è un componente di tipo FILE, verifica eventualmente il formato dell'hash
		// versato
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR
			...
			this.controllaHash(valueCompVers,
				valueCompVers.getMyComponente().getHashVersato());
			if (valueCompVers
				.getHashVersatoEncoding() == CostantiDB.TipiEncBinari.SCONOSCIUTO) {
			    tmpCdErr = MessaggiWSBundle.HASH_002_001;
			    tmpDsErr = MessaggiWSBundle.getString(MessaggiWSBundle.HASH_002_001,
				    valueCompVers.getChiaveComp());
			} else if (valueCompVers
				.getHashVersatoAlgoritmo() == CostantiDB.TipiHash.SCONOSCIUTO) {
			...
			if (versamento.getStrutturaComponenti().isFlagAccettaHashErrato()) {
			    versamento.addWarningDoc(documento, valueCompVers.getChiaveComp(),
				    tmpCdErr, tmpDsErr);
			} else {
			    versamento.addErrorDoc(documento, valueCompVers.getChiaveComp(),
				    tmpCdErr, tmpDsErr);
			}
		    }
			...
			if (tmpVdE.getCodiceEsito() == VoceDiErrore.TipiEsitoErrore.NEGATIVO) {
			    componenteResp.getEsitoComponente()
				    .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			    rispostaWs.setEsitoWsError(tmpVdE.getErrorCode(),
				    tmpVdE.getErrorMessage());
			} else if (tmpVdE
				.getCodiceEsito() == VoceDiErrore.TipiEsitoErrore.WARNING) {
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    this.parseDatiSpecComp(valueCompVers, componenteResp, versamento, rispostaWs);
		}
		 ...
		 */
		if ((rispostaWs.getSeverity() != SeverityEnum.ERROR)) {
		    this.verificaSottoComponentiComp(valueCompVers, documento, esitoDoc, versamento,
			    rispostaWs, rispostaControlli, ordiniDiPresentazioneUnici);
		}
	    }
	} // fine loop sui componenti
    }

    private void verificaSottoComponentiComp(ComponenteVers componentePadre,
	    DocumentoVers documento, ECDocumentoType esitoDoc, AbsVersamentoExt versamento,
	    IRispostaWS rispostaWs, RispostaControlli rispostaControlli,
	...
	if (componentePadre.getMyComponente().getSottoComponenti() != null) {
		...
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR
		    ...
		    componenteResp.getSottoComponenti().getSottoComponente()
			    .add(sottoComponenteResp);
			...
			valueCompVers.setUrnPartComponenteNiOrdDoc(
				MessaggiWSFormat.formattaUrnPartComponente(
					componentePadre.getUrnPartComponenteNiOrdDoc(),
					valueCompVers.getMySottoComponente()
						.getOrdinePresentazione(),
					Costanti.UrnFormatter.COMP_FMT_STRING_V2,
					Costanti.UrnFormatter.PAD5DIGITS_FMT));
		    }
		    ...
		    if (versamento.getModificatoriWSCalc()
			    .contains(Costanti.ModificatoriWS.TAG_RAPPORTO_VERS_1_5)) {
			// nome componente
			componenteResp.setNomeComponente(
				valueCompVers.getMySottoComponente().getNomeComponente());
			// urn versato
			componenteResp.setUrnVersato(
				valueCompVers.getMySottoComponente().getUrnVersato());
			// id componente
			componenteResp.setIDComponenteVersato(
				valueCompVers.getMySottoComponente().getIDComponenteVersato());
		    }

		    // se necessario, crea e verifica la validità del nome del file sul file system
		    // di destinazione
		    if (rispostaWs.getSeverity() != SeverityEnum.ERROR && versamento
			    .getStrutturaComponenti()
			    .getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE) {
			String tmpNomeFile = MessaggiWSFormat
				.formattaNomeFileArk(valueCompVers.getUrnPartComponenteNiOrdDoc());
			valueCompVers.setNomeFileArk(tmpNomeFile);
			...
			if (!fileServUtils.controllaPath(tmpFilePath)) {
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
			    // il sottocomponente {0} non può essere archiviato su filesystem perchè
			    // il suo pathname
		    ...
		    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			// salvo il controlloUnivocitaOrdinePresentazione
			if (!ordiniDiPresentazioneUnici.contains(Long.valueOf(
				valueCompVers.getMySottoComponente().getOrdinePresentazione()))) {
			    ordiniDiPresentazioneUnici.add(Long.valueOf(
				    valueCompVers.getMySottoComponente().getOrdinePresentazione()));
			    // salvo i parametri dell'esito
			    // ordinePresentazione
			    sottoComponenteResp.setOrdinePresentazione(BigInteger.valueOf(
				    valueCompVers.getMySottoComponente().getOrdinePresentazione()));
			} else {
			    // sollevo l'errore di connessione UnivocitaOrdinePresentazione
			    setRispostaWsError(rispostaWs, rispostaControlli);
			...
			if (rispostaControlli.isrBoolean() == false) {
			    ...
			    valueCompVers.setTipoUso(rispostaControlli.getrString());
			    // tipoComponente
			    sottoComponenteResp.setTipoComponente(
				    valueCompVers.getMySottoComponente().getTipoComponente());
			    // urn
			    sottoComponenteResp
				    .setURN(valueCompVers.getMySottoComponente().getUrnVersato());
			    //
		    ...
		    // Errore: se il tag "TipoSupportoComponente" del XML di versamento è
		    // RIFERIMENTO, e non è presente
		    // il tag "Riferimento".
		    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			if ((!valueCompVers.isPresenteRifMeta()) && (valueCompVers
				.getTipoSupporto() == ComponenteVers.TipiSupporto.RIFERIMENTO)) {
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setVerificaTipoSupportoComponente("NEGATIVO");
			    // " SottoComponente {0}: tag <Riferimento> obbligatorio e non
			    // valorizzato (il tag
			    // <TipoSupportoComponente> = "RIFERIMENTO")
			...
			if ((valueCompVers.getTipoUso()
				.equals(CostantiDB.TipoUsoComponente.CONVERTITORE))
				&& (valueCompVers
					.getTipoSupporto() != ComponenteVers.TipiSupporto.FILE)) {
			    rispostaWs.setSeverity(SeverityEnum.ERROR);
			    rispostaWs.setErrorCode("SUBCOMP-002");
			    rispostaWs.setErrorMessage(
			...
			if ((!valueCompVers.getTipoUso()
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setVerificaTipoSupportoComponente("NEGATIVO");
			    // SottoComponente {0}: tag <TipoSupportoComponente> valorizzato con
			    // "RIFERIMENTO" e tipo
			...
			if ((valueCompVers.getTipoUso()
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setVerificaTipoSupportoComponente("NEGATIVO");
			    // SottoComponente {0}: tag <TipoSupportoComponente> diverso da
			    // "RIFERIMENTO" e tipo uso
			...
			if (valueCompVers.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE) {
				...
				sottoComponenteResp.getEsitoSottoComponente()
					.setVerificaNomeComponente(ECEsitoPosNegType.NEGATIVO);
				// SottoComponente {0}: Il tag <NomeComponente> deve essere
				// valorizzato (il componente
			...
			rispostaControlli.reset();
			CSChiave chiave = new CSChiave();
			chiave.setAnno(Long.valueOf(
				valueCompVers.getMySottoComponente().getRiferimento().getAnno()));
			chiave.setNumero(
				valueCompVers.getMySottoComponente().getRiferimento().getNumero());
			chiave.setTipoRegistro(valueCompVers.getMySottoComponente().getRiferimento()
				.getTipoRegistro());
			...
			if (rispostaControlli.isrBoolean() || rispostaControlli.getrLong() == -1) {
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setVerificaRiferimentoUnitaDocumentaria(
					    ECEsitoPosNegType.NEGATIVO);
			    // SottoComponente {0}: la UD identificata dal Riferimento {1} non è
			    // presente nel sistema
			    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.SUBCOMP_006_003,
				    valueCompVers.getChiaveComp(),
				    MessaggiWSFormat.formattaUrnPartUnitaDoc(chiave));
			} else {
			    // verifico che l'unità doc riferita sia composta da un solo componente
			    // e che questo sia di
			    // tipo CONVERTITORE,
			    // che sia stata salvata su BLOB e che abbia tipo supporto uguale a FILE
				...
				rispostaWs.setEsitoWsError(rispostaControlli.getCodErr(),
				    ...
				    * se sto esaminando un sottocomponente di tipo uso CONV o RAPP,
				    * se è definito un formato di file atteso per il convertitore,
				    * verifico che il formato del file versato coincida
				    */ if (trovatoConvRapp && verificaFormatoConvertitore
				    && rispostaControlli.getrLong() != componentePadre
					    .getIdFormatoFileDocConv()) {
				setRispostaWsError(rispostaWs, rispostaControlli);
				sottoComponenteResp.getEsitoSottoComponente()
					.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
				sottoComponenteResp.getEsitoSottoComponente()
		    ...
		    // verifico l'ammissibilità del formato per il tipo supporto = FILE
		    // e successiva verifica del riconoscimento formato
		    if (rispostaWs.getSeverity() != SeverityEnum.ERROR && valueCompVers
			    .getTipoSupporto() == ComponenteVers.TipiSupporto.FILE) {
			// nel caso il formato versato fosse "sconosciuto" (parer/unkonwn)
			// lo tratto come se fosse nullo
			String descFormatoFileVers = valueCompVers.getMySottoComponente()
				.getFormatoFileVersato();
			if (descFormatoFileVers != null && (descFormatoFileVers.equals(
				versamento.getXmlDefaults().get(ParametroApplDB.FMT_UNKNOWN)))) {
			    descFormatoFileVers = null;
			...
			if (rispostaControlli.isrBoolean()) {
			    ...
			    valueCompVers.setFormatoFileVersNonAmmesso(true);
			    if (!valueCompVers.getTipoUso()
				    .equals(CostantiDB.TipoUsoComponente.FIRMA)
				    && !valueCompVers.getTipoUso()
					    .equals(CostantiDB.TipoUsoComponente.MARCA)) {
				...
				// (parer/unkonwn)
				rispostaControlli = controlliSemantici.checkFormatoFileVersato(
					versamento.getXmlDefaults()
						.get(ParametroApplDB.FMT_UNKNOWN),
					versamento.getStrutturaComponenti().getIdStruttura(),
					valueCompVers.getIdTipoComponente());
				if (rispostaControlli.isrBoolean()) {
				    // se lo è, registro questo formato come se fosse stato versato
				    valueCompVers
					    .setIdFormatoFileVers(rispostaControlli.getrLong());
				    // e correggo lo stato della risposta: non è più in errore
				    rispostaWs.setSeverity(SeverityEnum.WARNING);
				}
				// se non è ammissibile neanche il formato "sconosciuto" la risposta
				// del ws resta ERRORE
			    }
			}
			...
			// se il precedente test di ammissibilità è andato (in un qualche modo)
			// bene...
			if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			    /*
			     * se sto esaminando un sottocomponente di tipo uso CONV o RAPP, se è
			     * definito un formato di file atteso per il convertitore, verifico che
			     * il formato del file versato coincida
			     */
			    if (trovatoConvRapp && verificaFormatoConvertitore
				    && valueCompVers.getIdFormatoFileVers() != componentePadre
					    .getIdFormatoFileDocConv()) {
				setRispostaWsError(rispostaWs, rispostaControlli);
				sottoComponenteResp.getEsitoSottoComponente()
					.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
				sottoComponenteResp.getEsitoSottoComponente()
					.setVerificaAmmissibilitaFormato(
						ECEsitoPosNegType.NEGATIVO);
			    ...
			    sottoComponenteResp.getEsitoSottoComponente()
				    .setVerificaAmmissibilitaFormato(ECEsitoPosNegType.NEGATIVO);
			    // SottoComponente {0}: il Formato {1} non è presente nel DB per la
			    // struttura versante, il
		    ...
		    // verifico gli eventuali dati specifici e di migrazione
		    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
			this.parseDatiSpecSottoComp(valueCompVers, sottoComponenteResp, versamento,
				rispostaWs);
		    }
	...
	/*
	 * COMP: Se non è valorizzato Tipo Rappresentazione Componente non devono essere presenti
	 * sottocomponenti aventi tipo uso CONVERTITORE o RAPPRESENTAZIONE
	 */
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR && (!trovatoTipoRappComp)
		&& contaConvRapp > 0) {
	    setRispostaWsError(rispostaWs, rispostaControlli);
	    ...
	    componenteResp.getEsitoComponente()
		    .setVerificaSottoComponenteRappresentazione("NEGATIVO");
	    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_004_001,
		    componentePadre.getChiaveComp());
	}
	/*
	 * COMP: Se è valorizzato Tipo Rappresentazione Componente deve essere presente uno ed uno
	 * solo sottocomponente avente tipo uso CONVERTITORE o RAPPRESENTAZIONE
	 */
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR && trovatoTipoRappComp
		&& verificaFormatoConvertitore && contaConvRapp != 1) {
	    setRispostaWsError(rispostaWs, rispostaControlli);
	    componenteResp.getEsitoComponente().setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
	    componenteResp.getEsitoComponente()
		    .setVerificaSottoComponenteRappresentazione("NEGATIVO");
	    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.COMP_004_002,
		    componentePadre.getChiaveComp());
	}
    }
	...
	if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_MIGRAZIONE)
	    ...
	    tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpecMig(TipiEntitaSacer.DOC,
		    tmpDatiSpecificiMigrazione, esitoDoc.getChiaveDoc(),
		    documento.getRifDocumento().getTipoDocumento(),
		    versamento.getStrutturaComponenti().getSistemaDiMigrazione(),
		    versamento.getStrutturaComponenti().getIdStruttura(), rispostaWs);
	    if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	...
	if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_DATISPEC_EXT)) {
	    ...
	    tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpec(TipiEntitaSacer.COMP,
	    ...
	    if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_MIGRAZIONE)
		    && rispostaWs.getSeverity() != SeverityEnum.ERROR
		    && versamento.getStrutturaComponenti().getSistemaDiMigrazione() != null) {
		tmpDatiSpecificiMigrazione = componente.getMyComponente()
			.getDatiSpecificiMigrazione();
		tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpecMig(TipiEntitaSacer.COMP,
			tmpDatiSpecificiMigrazione, componente.getChiaveComp(),
			componente.getMyComponente().getTipoComponente(),
			versamento.getStrutturaComponenti().getSistemaDiMigrazione(),
			versamento.getStrutturaComponenti().getIdStruttura(), rispostaWs);
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    componente.setDatiSpecificiMigrazione(tmpControlliAttSpec.getDatiSpecifici());
		    componente.setIdRecXsdDatiSpecMigrazione(
			    tmpControlliAttSpec.getIdRecXsdDatiSpec());
		    esitoComp.getEsitoComponente().setCodiceEsito(ECEsitoPosNegWarType.POSITIVO);
		    esitoComp.getEsitoComponente()
			    .setCorrispondenzaDatiSpecifici(ECEsitoPosNegType.POSITIVO);
		} else {
	...
	if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_DATISPEC_EXT)) {
	    ...
	    tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpec(TipiEntitaSacer.SUB_COMP,
		    tmpDatiSpecifici, componente.getIdTipoComponente(), componente.getChiaveComp(),
	    ...
	    if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_MIGRAZIONE)
		    && rispostaWs.getSeverity() != SeverityEnum.ERROR
		    && versamento.getStrutturaComponenti().getSistemaDiMigrazione() != null) {
		tmpDatiSpecificiMigrazione = componente.getMySottoComponente()
			.getDatiSpecificiMigrazione();
		tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpecMig(TipiEntitaSacer.SUB_COMP,
			tmpDatiSpecificiMigrazione, componente.getChiaveComp(),
			componente.getMySottoComponente().getTipoComponente(),
			versamento.getStrutturaComponenti().getSistemaDiMigrazione(),
			versamento.getStrutturaComponenti().getIdStruttura(), rispostaWs);
		if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
		    ...
		    componente.setIdRecXsdDatiSpecMigrazione(
			    tmpControlliAttSpec.getIdRecXsdDatiSpec());
		    esitoComp.getEsitoSottoComponente()
			    .setCodiceEsito(ECEsitoPosNegWarType.POSITIVO);
		    esitoComp.getEsitoSottoComponente().setCorrispondenzaDatiSpecifici("POSITIVO");
		} else {
		    esitoComp.getEsitoSottoComponente()
    ...
    private void setRispostaWsError(IRispostaWS rispostaWs, RispostaControlli rispostaControlli) {
	rispostaWs.setSeverity(SeverityEnum.ERROR);
