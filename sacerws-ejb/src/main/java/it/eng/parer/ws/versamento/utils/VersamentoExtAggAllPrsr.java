...
public class VersamentoExtAggAllPrsr {
    ...
    public VersamentoExtAggAll getVersamento() {
        return versamento;
    }

    /**
     * @return dto con risposta al ws {@link RispostaWSAggAll}
     */
    public RispostaWSAggAll getRispostaWs() {
        return rispostaWs;
    }
     ...
     *
     * @param vers
     *            dto versamento su servizio aggiunta documento
     * @param risp
     *            risposta di tipo {@link RispostaWSAggAll}
     */
    public VersamentoExtAggAllPrsr(VersamentoExtAggAll vers, RispostaWSAggAll risp) {
        versamento = vers;
        rispostaWs = risp;
    ...
    public void parseXML(SyncFakeSessn sessione) {
        ...
        DocumentoVersPrsr myDocumentoVersPrsr = null;
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (parsedUnitaDoc.getConfigurazione() != null) {
                ...
                if (parsedUnitaDoc.getConfigurazione().getTipoConservazione() == TipoConservazioneType.FISCALE) {
                    tmpConfigurazione.setTipoConservazione(TipoConservazioneType.FISCALE.name());
                }

                if (parsedUnitaDoc.getConfigurazione().getTipoConservazione() == TipoConservazioneType.MIGRAZIONE) {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (rispostaControlli.getrLong() != -1) {
                ...
                // build flags
                this.elabFlagSipVDB(myEsito, os);
            } else {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR && myEsito.getUnitaDocumentaria().getEsitoUnitaDocumentaria()
            ...
            // forza
            boolean flForzaAggiuntaDocumento = parsedUnitaDoc.getConfigurazione().isForzaAggiuntaDocumento() != null
            ...
            if (scud == CostantiDB.StatoConservazioneUnitaDoc.IN_ARCHIVIO
                ...
                if (versamento.getStrutturaComponenti().isFlagAbilitaForzaDocAgg() && isWarn) {
                    ...
                    // errore UD-013-001 L'unità documentaria è inserita in almeno una serie;
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            myDocumentoVersPrsr = new DocumentoVersPrsr(versamento, rispostaWs, myEsito.getConfigurazione());
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                ...
                tmpDocumento.setTipoDocumento((String) valueDocVers.getRifDocumento().getTipoDocumento());
                ...
                myDocumentoVersPrsr.parseDocumentoGen(valueDocVers, tmpDocumento);
            }
    ...
    private void elabFlagSipVDB(EsitoVersAggAllegati myEsito, OrgStrut os) {
        ...
        boolean flForzaHashVers = configurationHelper
                .getParamApplicValueAsFl(ParametroApplFl.FL_FORZA_HASH_VERS, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE);
        ...
        //
        versamento.getStrutturaComponenti().setFlagAbilitaForzaDocAgg(configurationHelper
                .getParamApplicValueAsFl(ParametroApplFl.FL_ABILITA_FORZA_DOC_AGG, idStrut, idAmbiente, idTipoUd)
                .equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti()
                .setFlagForzaDocAgg(configurationHelper
                        .getParamApplicValueAsFl(ParametroApplFl.FL_FORZA_DOC_AGG, idStrut, idAmbiente, idTipoUd)
                        .equals(CostantiDB.Flag.TRUE));

        versamento.getStrutturaComponenti()
                .setFlagAccettaDocNeg(configurationHelper
                        .getParamApplicValueAsFl(ParametroApplFl.FL_ACCETTA_DOC_AGG_NEG, idStrut, idAmbiente, idTipoUd)
                        .equals(CostantiDB.Flag.TRUE));
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_ABILITA_FORZA_1_5)) {
            ...
            boolean flForzaFmt = parsedUnitaDoc.getConfigurazione().isForzaFormatoFile() != null
     ...
     */
    private void buildFlagsOnEsito(EsitoVersAggAllegati myEsito) {
     ...
     * @param myEsito
     */
    private void buildUrnSipOnEsito(EsitoVersAggAllegati myEsito) {
