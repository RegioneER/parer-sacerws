 ...
 */
public class VersamentoSyncSrvlt extends HttpServlet {

    private static final long serialVersionUID = 1L;
    private static final Logger log = LoggerFactory.getLogger(VersamentoSyncSrvlt.class);
     ...
     *             errore generico
     */
    public VersamentoSyncSrvlt() throws IOException {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            versamentoSync = (VersamentoSync) new InitialContext().lookup("java:app/sacerws-ejb/VersamentoSync");
        } catch (NamingException ex) {
            log.error("Errore nel recupero dell'EJB VersamentoSync ", ex);
            throw new ServletException("Impossibile recuperare l'ejb VersamentoSync ", ex);
        }
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (isMultipart) {
                ...
                try {
                    ...
                    // MEV#27048
                    // TODO: da spostare nel salvaTutto e da rimuovere
                    // if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                    // versamentoSync.gestisciUdInAttesaSched(rispostaWs, myVersamentoExt);
                    // }
                    // end MEV#27048
                    ...
                    log.error("Eccezione FileUploadException nella servlet versamento sync "
                            + "\n TS Apertura sessione: " + format(sessioneFinta.getTmApertura()) + "\n IP chiamante: "
                            + sessioneFinta.getIpChiamante(), e1);
                } catch (Exception e1) {
                    rispostaWs.setSeverity(SeverityEnum.ERROR);
                    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                            "Eccezione generica nella servlet versamento sync " + e1.getMessage());
                    //
                    SrvltHandlingException.handlingSocketErrors(Response405.NomeWebServiceRest.VERSAMENTO_SYNC, e1,
                            rispostaWs);
                    log.error("Eccezione generica nella servlet versamento sync " + "\n TS Apertura sessione: "
                            + format(sessioneFinta.getTmApertura()) + "\n IP chiamante: "
                            + sessioneFinta.getIpChiamante(), e1);
                } finally {
                    if (fileItems != null) {
                ...
                log.error("Errore nella servlet versamento sync: la chiamata non è multipart/formdata ");
            }
        }
