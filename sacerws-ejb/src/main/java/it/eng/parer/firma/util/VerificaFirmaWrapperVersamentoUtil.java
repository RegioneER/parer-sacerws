...
public class VerificaFirmaWrapperVersamentoUtil {
    ...
    private static final Logger LOG = LoggerFactory
	    .getLogger(VerificaFirmaWrapperVersamentoUtil.class);
     ...
     *
     * TODO: da integrare su WRAPPER (gestione del report/risultato al netto della verifica della
     * firma)
     *
     ...
     *
     *
     * @see it.eng.parer.firma.helper.IBaseWrapperParser#buildECMarcaType(it.eng.parer.
     * firma.xml.VerificaFirmaWrapper)
     */
	...
	tmpMarcaResp.setOrdineMarca(BigInteger.valueOf(
		marcaCompType.getPgMarca() != null ? marcaCompType.getPgMarca().longValue() : 0));
	...
	if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_INFO_FIRME_EXT_OUT)) {
	    tmpMarcaResp.getEsitoMarca()
		    .setDettaglioControlloConformita(marcaCompType.getDsMsgEsitoContrConforme());
	...
	if (esitoContrConf.equals(SacerIndication.POSITIVO.name())) {
	    ...
	    tmpMarcaResp.getEsitoMarca()
		    .setVerificaMarca(new ECMarcaType.EsitoMarca.VerificaMarca());
		...
		//
		if (contrMarcaCompType.getTiContr().name()
			.equals(VFTipoControlloType.CRITTOGRAFICO.name())) {
		    tmpMarcaResp.getEsitoMarca().getVerificaMarca().setControlloCrittografico(
			    ECEsitoControlloType.valueOf(tmpTiEsitoControlloMarca));
			...
			// memorizzo l'anomalia relativa alla marca del componente. alla fine del
			// ciclo
			// firme-marche aggiungerò un warning
		    }
		} else if (contrMarcaCompType.getTiContr().name()
			.equals(VFTipoControlloType.CATENA_TRUSTED.name())) {
		    tmpMarcaResp.getEsitoMarca().getVerificaMarca().setControlloCatenaTrusted(
			    ECEsitoControlloType.valueOf(tmpTiEsitoControlloMarca));
		    if (scriviDettagliContrMarca) {
			tmpMarcaResp.getEsitoMarca().getVerificaMarca()
			...
			.equals(VFTipoControlloType.CERTIFICATO.name())) {
		    tmpMarcaResp.getEsitoMarca().getVerificaMarca().setControlloCertificato(
			    ECEsitoControlloType.valueOf(tmpTiEsitoControlloMarca));
		    if (scriviDettagliContrMarca) {
			tmpMarcaResp.getEsitoMarca().getVerificaMarca()
			...
			.equals(VFTipoControlloType.CRL.name())) {
		    tmpMarcaResp.getEsitoMarca().getVerificaMarca().setControlloCRL(
			    ECEsitoControlloType.valueOf(tmpTiEsitoControlloMarca));
		    if (scriviDettagliContrMarca) {
			tmpMarcaResp.getEsitoMarca().getVerificaMarca()
				.setDettaglioControlloCRL(messaggioEsitoCMarca);
		    }
		    if (!tmpTiEsitoControlloMarca.equals(SacerIndication.POSITIVO.name())
			    && !tmpTiEsitoControlloMarca
				    .equals(SacerIndication.NON_NECESSARIO.name())) {
			...
			.equals(VFTipoControlloType.OCSP.name())
			&& versamento.getModificatoriWSCalc().contains(
				ModificatoriWS.TAG_FIRMA_1_5)/* tag visibile a partire da v1.5 */) {
		    tmpMarcaResp.getEsitoMarca().getVerificaMarca().setControlloOCSP(
			    ECEsitoControlloType.valueOf(tmpTiEsitoControlloMarca));
		    if (scriviDettagliContrMarca) {
			tmpMarcaResp.getEsitoMarca().getVerificaMarca()
				.setDettaglioControlloOCSP(messaggioEsitoCMarca);
		    }
	    ...
	    tmpMarcaResp.getEsitoMarca()
		    .setControlloConformita(ECEsitoControlloType.FORMATO_NON_CONOSCIUTO);
	...
	tmpFirmatarioResp.setOrdineFirma(BigInteger.valueOf(
		firmaCompType.getPgFirma() != null ? firmaCompType.getPgFirma().longValue() : 0));
	// compilo il cognome e nome, se presente, oppure riporto l'intero campo DN
	if (firmaCompType.getNmCognomeFirmatario() != null
		&& firmaCompType.getNmFirmatario() != null) {
	    tmpFirmatarioResp.setCognomeNome(
		    firmaCompType.getNmCognomeFirmatario() + " " + firmaCompType.getNmFirmatario());
	} else {
	...
	//
	tmpFirmatarioResp
		.setTipoRiferimentoTemporaleUsato(firmaCompType.getTipoRiferimentoTemporaleUsato());
	...
	if (esitoContrConf.equals(SacerIndication.FORMATO_NON_CONOSCIUTO.name())) {
	    tmpFirmatarioResp.getEsitoFirma()
		    .setControlloConformita(ECEsitoControlloType.FORMATO_NON_CONOSCIUTO);
	    firmaVerificabile = false; // nota bene: SOLO in questo caso, viene considerato come NON
				       // FIRMATO
	    ...
	    tmpFirmatarioResp.getEsitoFirma()
		    .setControlloConformita(ECEsitoControlloType.FORMATO_NON_CONFORME);
	    ...
	    tmpFirmatarioResp.getEsitoFirma()
		    .setControlloConformita(ECEsitoControlloType.NON_AMMESSO_DELIB_45_CNIPA);
	...
	if (firmaVerificabile) {
	    statoComponente.componenteFirmato = true; // c'è almeno una firma utilizzabile nel
						      // componente
	    tmpFirmatarioResp.getEsitoFirma()
		    .setVerificaFirma(new ECFirmatarioType.EsitoFirma.VerificaFirma());
		...
		final boolean esitoControlloPositivo = esitoControlloFirma
			.equals(SacerIndication.POSITIVO.name());

		if (contrFirmaCompType.getTiContr().name()
			.equals(VFTipoControlloType.CRITTOGRAFICO.name())) {
		    // imposto il valore e l'eventuale messaggio di dettaglio nell'esito
		    tmpFirmatarioResp.getEsitoFirma().getVerificaFirma().setControlloCrittografico(
			    ECEsitoControlloType.valueOf(esitoControlloFirma));
			...
			if (esitoControlloFirma.equals(SacerIndication.NON_ESEGUITO.name())) {
			    ...
			    // se è andato in errore, la firma è in err, il componente è in err e
			    // aggiungo
			    // l'errore alla lista
			    ...
			    LOG.debug(LOG_VALNOTMANAGED_MSG, contrFirmaCompType.getTiContr(),
				    esitoControlloFirma);
			}
		    }
		} else if (contrFirmaCompType.getTiContr().name()
			.equals(VFTipoControlloType.CATENA_TRUSTED.name())) {
		    // imposto il valore e l'eventuale messaggio di dettaglio nell'esito
		    tmpFirmatarioResp.getEsitoFirma().getVerificaFirma().setControlloCatenaTrusted(
			    ECEsitoControlloType.valueOf(esitoControlloFirma));
		    if (scriviDettagliContrFirma) {
			tmpFirmatarioResp.getEsitoFirma().getVerificaFirma()
			...
			.equals(VFTipoControlloType.CERTIFICATO.name())) {
		    // imposto il valore e l'eventuale messaggio di dettaglio nell'esito
		    tmpFirmatarioResp.getEsitoFirma().getVerificaFirma().setControlloCertificato(
			    ECEsitoControlloType.valueOf(esitoControlloFirma));
		    if (scriviDettagliContrFirma) {
			tmpFirmatarioResp.getEsitoFirma().getVerificaFirma()
			...
			if (esitoControlloFirma.equals(SacerIndication.NON_ESEGUITO.name())) {
			    ...
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.CERTIFICATO_SCADUTO.name())
				|| esitoControlloFirma
					.equals(SacerIndication.REVOCHE_NON_CONSISTENTI.name())
				|| esitoControlloFirma
					.equals(SacerIndication.CERTIFICATO_REVOCATO.name())) {
			    statoComponente.ctrlFirmeCertificatoScadRev = true;
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.CERTIFICATO_NON_VALIDO.name())) {
			    statoComponente.ctrlFirmeCertificatoNoValid = true;
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.CERTIFICATO_ERRATO.name())) {
			...
			// verifico lo stato:
			if (esitoControlloFirma.equals(SacerIndication.NON_ESEGUITO.name())
				|| esitoControlloFirma.equals(
					SacerIndication.CERTIFICATO_SCADUTO_3_12_2009.name())) {
				...
				.equals(SacerIndication.CRL_NON_VALIDA.name())) {
			    statoComponente.ctrlFirmeCRLNoValid = true;
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.CRL_NON_SCARICABILE.name())) {
			...
			if (esitoControlloFirma.equals(SacerIndication.NON_ESEGUITO.name())
				|| esitoControlloFirma
					.equals(SacerIndication.OCSP_NON_AGGIORNATO.name())) {
			    statoComponente.ctrlFirmeOCSPWarning = true;
			    firmaWarn = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.OCSP_SCONOSCIUTO.name())) {
			    statoComponente.ctrlFirmeOCSPNoValid = true;
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.OCSP_REVOCATO.name())) {
			    statoComponente.ctrlFirmeOCSPRev = true;
			    firmaErr = true;
			} else if (esitoControlloFirma
				.equals(SacerIndication.OCSP_NON_SCARICABILE.name())) {
