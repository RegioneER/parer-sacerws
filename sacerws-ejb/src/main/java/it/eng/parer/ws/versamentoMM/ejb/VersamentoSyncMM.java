...
public class VersamentoSyncMM extends VersamentoSyncBase {
    ...
    public void parseXMLIndiceMM(SyncFakeSessn sessione, RispostaWS rispostaWs,
	    VersamentoMMExt versamento) {
    ...
    public void verificaCoerenzaComponenti(RispostaWS rispostaWs, VersamentoMMExt versamento,
	    SyncFakeSessn sessione, String path) {
	...
	if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
	    // verifica che il payload del versamento possa essere contenuto nel filesystem di
	    // destinazione
    ...
    public void pulisciZipTemp(RispostaWS rispostaWs, VersamentoMMExt versamento,
	...
	if (versamento.isContainerZip() && versamento.getPathLocaleContenutoZip() != null) {
		...
		log.error("Eccezione nella fase di rimozione della directory temporanea per lo ZIP",
			ex);
	    }
	}
    }
