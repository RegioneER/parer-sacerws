...
public class AllineaFileComponenti {
    ...
    public void verificaCoerenza(String pathFileTemporanei, VersamentoMMExt versamento,
            RispostaWS rispostaWs, SyncFakeSessn sessioneFinta) throws IOException {
        ComponenteVers tmpComponenteVers;
        for (ComponenteMM componenteMM : versamento.getStrutturaIndiceMM().getComponenti()
                .values()) {
            tmpComponenteVers = versamento.getStrutturaComponenti().getFileAttesi()
                    .get(componenteMM.getId());
            boolean erroriComp = false;

            if (tmpComponenteVers == null) {
                erroriComp = true;
                // Componente Multimedia <ID> {0}: Non \u00e8 stato trovato un componente di
                // versamento corrispondente
                versamento.listErrAddErrorUd(componenteMM.getMyComponenteMM().getID(),
                        MessaggiWSBundle.MM_COMP_006_001, componenteMM.getMyComponenteMM().getID());
                // inutile proseguire: se i due xml sono incoerenti a questo livello tutti gli altri
                // test sono privi di
                // senso.
            } else {
                if (tmpComponenteVers.getTipoSupporto() != ComponenteVers.TipiSupporto.FILE) {
                    erroriComp = true;
                    // Componente Multimedia <ID> {0}: Il componente di versamento deve essere di
                    // tipo FILE
                    versamento.listErrAddErrorUd(componenteMM.getMyComponenteMM().getID(),
                            MessaggiWSBundle.MM_COMP_007_001,
                            componenteMM.getMyComponenteMM().getID());
                }
                if (componenteMM.isForzaFirmeFormati()
                        && tmpComponenteVers.getMySottoComponente() != null) {
                    erroriComp = true;
                    // Componente Multimedia <ID> {0}: La forzatura di firme o formati \u00e8
                    // possibile solo sui
                    // componenti
                    versamento.listErrAddErrorUd(componenteMM.getMyComponenteMM().getID(),
                            MessaggiWSBundle.MM_COMP_008_001,
                            componenteMM.getMyComponenteMM().getID());
                }
                if (componenteMM.isForzaFirmeFormati()
                        && tmpComponenteVers.getMyComponente().getSottoComponenti() != null
                        && tmpComponenteVers.getMyComponente().getSottoComponenti()
                                .getSottoComponente().size() > 0) {
                    erroriComp = true;
                    // Componente Multimedia <ID> {0}: La forzatura di firme o formati \u00e8
                    // possibile solo sui
                    // componenti privi di sottocomponenti
                    versamento.listErrAddErrorUd(componenteMM.getMyComponenteMM().getID(),
                            MessaggiWSBundle.MM_COMP_009_001,
                            componenteMM.getMyComponenteMM().getID());
                }
            }
            //
        ...
        for (ComponenteVers componenteVers : versamento.getStrutturaComponenti().getFileAttesi()
                .values()) {
            if (componenteVers.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE
                    && componenteVers.getRifComponenteMM() == null) {
                // Componente di versamento <ID> {0}: Non \u00e8 stato trovato un componente
                // Multimedia corrispondente
                versamento.listErrAddErrorUd(componenteVers.getId(),
                        MessaggiWSBundle.MM_COMP_010_001, componenteVers.getId());
            }
        }
        //
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (versamento.isContainerZip()) {
                ...
                tmpDirectoryPerZip = Files
                        .createTempDirectory(FileSystems.getDefault().getPath(pathFileTemporanei),
                                "tempZip_", attribute)
                        .toFile();
            ...
            for (ComponenteVers componente : versamento.getStrutturaComponenti().getFileAttesi()
                ...
                if (componente.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE) {
                    ...
                    tmpFileBinario
                            .setId(componente.getRifComponenteMM().getMyComponenteMM().getID());
                    ...
                    if (componente.getRifComponenteResp() != null) {
                        componente.getRifComponenteResp()
                                .setDimensioneFile(BigInteger.valueOf(tmpFile.length()));
                    }
