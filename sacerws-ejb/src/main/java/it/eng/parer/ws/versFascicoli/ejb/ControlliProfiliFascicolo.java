...
public class ControlliProfiliFascicolo {
    ...
    public RispostaControlli verificaProfiloArchivistico(VersFascicoloExt versamento) {
        ...
        try {
            String queryStr = "select count(t) from DecUsoModelloXsdFasc t "
                    ...
                    + "and t.decModelloXsdFascicolo.dtSoppres > :dataDiOggiIn "; // da notare
            // STRETTAMENTE
            // MAGGIORE ;
            ...
            if (conta > 0) {
                ...
                if (parsedIndiceFasc.getParametri()
                        .getVersioneProfiloArchivisticoFascicolo() != null) {
                    versione = parsedIndiceFasc.getParametri()
                            .getVersioneProfiloArchivisticoFascicolo();
                    // la versione è stata indicata
                    queryStr = "select t from DecModelloXsdFascicolo t "
                            + "join t.decUsoModelloXsdFascs uf "
                            + "where t.tiModelloXsd = :segnatura "
                            + "and t.tiUsoModelloXsd = :usoversamento " + "and t.cdXsd = :cdXsd "
                            + "and t.dtIstituz <= :dataDiOggiIn "
                    ...
                    if (dmxfs.isEmpty()) {
                        ...
                        rispostaControlli.setDsErr(
                                MessaggiWSBundle.getString(MessaggiWSBundle.FAS_PF_ARCH_001_002,
                    ...
                    queryStr = "select t from DecModelloXsdFascicolo t "
                            + "join t.decUsoModelloXsdFascs uf "
                            + "where t.tiModelloXsd = :segnatura "
                            + "and t.tiUsoModelloXsd = :usoversamento "
                            + "and uf.flStandard = :flDefault "
                            + "and t.dtIstituz <= :dataDiOggiIn "
                            + "and t.dtSoppres > :dataDiOggiIn " // da notare STRETTAMENTE MAGGIORE
                            + "and uf.decAaTipoFascicolo.decTipoFascicolo.idTipoFascicolo = :idTipoFascicolo "
                            + "and uf.decAaTipoFascicolo.aaIniTipoFascicolo <= :annoIn "
                    ...
                    if (dmxfs.isEmpty()) {
                        ...
                        rispostaControlli.setDsErr(
                                MessaggiWSBundle.getString(MessaggiWSBundle.FAS_PF_ARCH_001_001,
                ...
                if (rc.isrBoolean()) {
                    ...
                    rispostaControlli.setDsErr(
                            MessaggiWSBundle.getString(MessaggiWSBundle.FAS_PF_ARCH_002_001,
                                    svf.getUrnPartChiaveFascicolo(), rc.getDsErr()));
                    return rispostaControlli;
                }
                DatiXmlProfiloArchivistico dxpa = null;
                if (prosegui) {
                    dxpa = this.recuperaDatiDaXmlPA(
                            parsedIndiceFasc.getProfiloArchivistico().getValue());
                    if (dxpa == null) {
                        rispostaControlli.setCodErr(MessaggiWSBundle.FAS_PF_ARCH_002_001);
                        rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                                MessaggiWSBundle.FAS_PF_ARCH_002_001,
                                svf.getUrnPartChiaveFascicolo(),
    ...
    private DatiXmlProfiloArchivistico recuperaDatiDaXmlPA(ProfiloArchivisticoType pat) {
        ...
        try {
            ...
            //
            XPathExpression expr = xpath.compile("//SegnaturaArchivistica" + "/Classificazione"
                    + "/IndiceClassificazione/text()");
            ...
            expr = xpath.compile("//SegnaturaArchivistica" + "/Classificazione"
            ...
            if (tmpnode != null && tmpnode.getNodeType() == Node.ELEMENT_NODE) {
                tmpDatiXml.setChiaveFascicoloDiAppartenenza(
                        this.recuperaCsChiaveSottoFascDaNodo(tmpnode));
            }
            //
    ...
    public RispostaControlli verificaProfiloGenerale(VersFascicoloExt versamento,
        ...
        try {
            ...
            if (parsedIndiceFasc.getParametri().getVersioneProfiloGeneraleFascicolo() != null) {
                ...
                // la versione è stata indicata
                String queryStr = "select t from DecModelloXsdFascicolo t "
                        + "join t.decUsoModelloXsdFascs uf " + "where t.tiModelloXsd = :segnatura "
                        + "and t.tiUsoModelloXsd = :usoversamento " + "and t.cdXsd = :cdXsd "
                        + "and t.dtIstituz <= :dataDiOggiIn " + "and t.dtSoppres > :dataDiOggiIn " // da
                        // notare
                        // STRETTAMENTE
                        // MAGGIORE
                        + "and uf.decAaTipoFascicolo.decTipoFascicolo.idTipoFascicolo = :idTipoFascicolo "
                ...
                if (dmxfs.isEmpty()) {
                    versamento.listErrAddError(svf.getUrnPartChiaveFascicolo(),
                            MessaggiWSBundle.FAS_PF_GEN_001_001, svf.getUrnPartChiaveFascicolo(),
                            svf.getTipoFascicoloNonverificato());
                    return rispostaControlli;
                }
            }
            ...
            if (prosegui) {
                ...
                fascicoloResp.setDataApertura(
                        XmlDateUtility.dateToXMLGregorianCalendar(dxpg.getDataApertura()));
                fascicoloResp.setDataChiusura(
                        XmlDateUtility.dateToXMLGregorianCalendar(dxpg.getDataChiusura()));
                ...
                if (!this.verificaDocumentiProfiloGen(versamento, dxpg)) {
                    // se ci sono errori sui documenti proseguo, ma so di aver fallito il
                    // versamento.
                    // questo metodo scrive autonomamente gli errori nella lista errori
                    prosegui = false;
                }
                ...
                if (!this.verificaResponsabiliProfiloGen(versamento, dxpg)) {
                    // se ci sono errori sui responsabile proseguo, ma so di aver fallito il
                    // versamento.
                    // questo metodo scrive autonomamente gli errori nella lista errori
                    prosegui = false;
                }
    ...
    private boolean verificaDocumentiProfiloGen(VersFascicoloExt versamento,
            DatiXmlProfiloGenerale dxpg) {
        boolean retVal = true;
        StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
        if (dxpg.getPrimoDocumento() != null) {
            RispostaControlli rc = controlliSemantici.checkChiave(dxpg.getPrimoDocumento(),
                    svf.getIdStruttura(), ControlliSemantici.TipiGestioneUDAnnullate.CARICA);
            if (rc.getrLong() == -1 || (svf.getUnitaDocElencate() != null
                    && !svf.getUnitaDocElencate().contains(rc.getrLong()))) {
                ...
                String descChiaveUdColl = MessaggiWSFormat
                        .formattaUrnPartUnitaDoc(dxpg.getPrimoDocumento());
                versamento.listErrAddError(svf.getUrnPartChiaveFascicolo(),
                        MessaggiWSBundle.FAS_PF_GEN_003_003, svf.getUrnPartChiaveFascicolo(),
                        descChiaveUdColl);
                retVal = false;
            } else {
        ...
        if (dxpg.getUltimoDocumento() != null) {
            RispostaControlli rc = controlliSemantici.checkChiave(dxpg.getUltimoDocumento(),
                    svf.getIdStruttura(), ControlliSemantici.TipiGestioneUDAnnullate.CARICA);
            if (rc.getrLong() == -1 || (svf.getUnitaDocElencate() != null
                    && !svf.getUnitaDocElencate().contains(rc.getrLong()))) {
                // se non ha trovato la chiave tra quelle dichiarate
                // registro l'errore
                String descChiaveUdColl = MessaggiWSFormat
                        .formattaUrnPartUnitaDoc(dxpg.getUltimoDocumento());
                versamento.listErrAddError(svf.getUrnPartChiaveFascicolo(),
                        MessaggiWSBundle.FAS_PF_GEN_003_004, svf.getUrnPartChiaveFascicolo(),
                        descChiaveUdColl);
                retVal = false;
            } else {
    ...
    private boolean verificaDateProfiloGen(VersFascicoloExt versamento,
            DatiXmlProfiloGenerale dxpg) {
        StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
        ...
        if (versamento.getVersamento().getParametri()
    ...
    private boolean verificaResponsabiliProfiloGen(VersFascicoloExt versamento,
            DatiXmlProfiloGenerale dxpg) {
    ...
    private DatiXmlProfiloGenerale recuperaDatiDaXmlPG(ProfiloGeneraleType pgt) {
        ...
        try {
            ...
            //
            expr = xpath.compile("//ProfiloGeneraleFascicolo" + "/AmministrazioniPartecipanti"
            ...
            //
            expr = xpath.compile(
                    "//ProfiloGeneraleFascicolo" + "/SoggettiCoinvolti" + "/SoggettoCoinvolto");
            nodes = (NodeList) expr.evaluate(tmpDati, XPathConstants.NODESET);
            for (int i = 0; i < nodes.getLength(); i++) {
                // SoggettoCoinvolto
                tmpnode = nodes.item(i);
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                /*
                 * Nota: per il momento è una lista di tipo String (come da XSD) essendo un unico
                 * campo TODO/EVO: eventualmente wrappare su oggetto
                 */
                tmpString = nodes.item(i).getNodeValue();
