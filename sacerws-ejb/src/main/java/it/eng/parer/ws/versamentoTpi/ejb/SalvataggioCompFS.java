...
public class SalvataggioCompFS {
    ...
    public RispostaControlli verificaDirTPIDataViaDb(StrutturaVersamento versamento,
        ...
        Date tmpTestData = DateUtils.truncate(convert(versamento.getDataVersamento()), Calendar.DAY_OF_MONTH);
        // genero il nome della directory corrispondente da usare per il test e la creazione
        statoCreaCartelle.setPathCartellaData(MessageFormat.format("{0}/{1}/{2}", versamento.getTpiRootTpiDaSacer(),
        ...
        try {
            ...
            String queryStr = "select t.ID_DT_VERS " + "from vrs_dt_vers t " + "where t.DT_VERS = ?";

            /*
             * nota bene: il codice strano che segue ha senso poiché apparentemente la scrittura della tabella da parte
             * di un eventuale altro nodo del cluster non viene registrata immediatamente dagli altri nodi. Questo
             * consente di verificare se il record ricercato esiste, dando il tempo alla cache di aggiornarsi.
             */
            for (int volte = 0; volte < numeroTentativi; volte++) {
                ...
                if (!tmpList.isEmpty()) {
                    ...
                    log.debug("verificaDirTPIDataViaDb : non ho trovato riga DT_VERS Data {}", tmpTestData);
    ...
    public RispostaControlli verificaDirTPIVersatoreViaDb(StrutturaVersamento versamento,
        ...
        // genero il nome della directory corrispondente da usare per il test e la creazione
        statoCreaCartelle.setPathCartellaVersatore(
                MessageFormat.format("{0}/{1}/{2}/{3}", versamento.getTpiRootTpiDaSacer(), versamento.getTpiRootVers(),
        ...
        try {
            ...
            // prima da un altro nodo del cluster e questa scrittura potrebbe non essere stata
            // ancora recepita.
            String queryStr = "select count(*) " + "from VRS_PATH_DT_VERS t " + "where t.ID_DT_VERS = ? "
                    + "and t.DL_PATH = ?";
            ...
            for (int volte = 0; volte < numeroTentativi; volte++) {
                log.debug("verificaDirTPIVersatoreViaDb : cerco riga VRS_PATH_DT_VERS IdDtVers {} subpath {}",
    ...
    public RispostaControlli verificaDirSuFs(String testDir) {
        ...
        rispostaControlli.setrBoolean(false);
        /*
         * nota bene: il codice strano che segue ha senso dal momento che la share NFS su cui vengono depositati i file
         * non aggiorna subito il contenuto a tutti i sistemi verso cui viene esportata. La semplice invocazione del
         * metodo di verifica file eseguita pochi millisecondi dopo la creazione, come avviene nel caso di versamenti
         * paralleli in produzione, fornisce quasi sempre esito negativo. Il ciclo for... che segue consente di
         * ritentare più volte la verifica con pause di circa 2 secondi per fornire al file system il tempo di
         * registrare la creazione della directory.
         */
    ...
    public RispostaControlli generaDirDataTPIBlock(StrutturaVersamento versamento,
        ...
        /*
         * lock sulla funzione di creazione directory TPI, per evitare problemi di concorrenza tra i vari application
         * server
         */
        ...
        try {
            ...
            if (rc.isrBoolean()) {
                if (statoCreaCartelle.isCreaCartellaData()) {
                    log.info("SalvataggioCompFS.generaDirDataTPIBlock: devo creare la cartella Data");
                    ...
                    //
                    xadConn = xadCf.getConnection();
                    WsXAUtil.createDirectory(xadConn, new File(statoCreaCartelle.getPathCartellaData()));
                    entityManager.flush();
                    //
                    // memorizzo l'id della riga creata.
                    // nel caso isCreaCartellaData fosse stato negativo, l'id sarebbe già stato memorizzato
            ...
            if (ExceptionUtils.indexOfThrowable(e, java.sql.SQLIntegrityConstraintViolationException.class) > -1) {
                // se ho una violazione di integrità della constraint, vuol dire che sto cercando di inserire
                // un record che la query precedente non ha trovato ma che in realtà c'è.
                // in questo caso rendo una risposta positiva ma lascio lo stato creazione inalterato
                // (in particolare manca l'ID del record di VrsDtVers, non trovato e non creato da me)
                rispostaControlli.setrLong(1);
                rispostaControlli.setrBoolean(true);
                log.warn(
        ...
        try {
            ...
            if (rc.isrBoolean()) {
                if (statoCreaCartelle.isCreaCartellaVersatore()) {
                    log.info("SalvataggioCompFS.generaDirVersatoreTPIBlock: devo creare la cartella Versatore");
                    VrsDtVers vrsDtVers = entityManager.find(VrsDtVers.class, statoCreaCartelle.getIdDtVers());
                    //
                    ...
                    //
                    xadConn = xadCf.getConnection();
                    WsXAUtil.createDirectory(xadConn, new File(statoCreaCartelle.getPathCartellaVersatore()));
                    entityManager.flush();
                    //
    ...
    public RispostaControlli generaDirUDTPI(StrutturaVersamento versamento, StatoCreaCartelle statoCreaCartelle) {
        RispostaControlli rispostaControlli;
        rispostaControlli = new RispostaControlli();
        rispostaControlli.setrLong(-1);
        rispostaControlli.setrBoolean(false);
        ...
        try {
            ...
            testDir = MessageFormat.format("{0}/{1}/{2}/{3}/{4}", versamento.getTpiRootTpiDaSacer(),
                    versamento.getTpiRootVers(), versamento.getSubPathDataVers(), versamento.getSubPathVersatoreArk(),
                    versamento.getSubPathUnitaDocArk());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli copiaCompSuFS(StrutturaVersamento versamento, ComponenteVers componenteVers) {
        RispostaControlli rispostaControlli;
        rispostaControlli = new RispostaControlli();
        rispostaControlli.setrLong(-1);
        rispostaControlli.setrBoolean(false);
        ...
        String tmpFilePath = MessaggiWSFormat.formattaFilePathArk(versamento.getTpiRootTpiDaSacer(),
                versamento.getTpiRootVers(), versamento.getSubPathDataVers(), versamento.getSubPathVersatoreArk(),
                versamento.getSubPathUnitaDocArk(), componenteVers.getNomeFileArk());

        try {
            xadConn = xadCf.getConnection();
            WsXAUtil.copyFile(xadConn, componenteVers.getRifFileBinario().getFileSuDisco(), new File(tmpFilePath));
            rispostaControlli.setrLong(1);
            rispostaControlli.setrBoolean(true);
        } catch (Exception e) {
            rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
