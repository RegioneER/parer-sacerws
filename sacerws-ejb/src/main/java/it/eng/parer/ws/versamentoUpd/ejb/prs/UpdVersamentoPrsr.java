...
public class UpdVersamentoPrsr extends UpdVersamentoBasePrsr {
    ...
    public void parseXML(SyncFakeSessn sessione, UpdVersamentoExt versamento,
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            versamento.addControlloOkOnGenerali(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_XMLVSXSD));

            RispostaControlli rispostaControlli = rapportoVersBuilder
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            try {
                ...
                parsedIndiceUpd = (IndiceSIPAggiornamentoUnitaDocumentaria) unmarshaller
                        .unmarshal(tmpReader);
                ...
                // sessione errata.
                tagCSVersatore.setAmbiente(parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                        .getVersatore().getAmbiente());
                tagCSVersatore.setEnte(parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                        .getVersatore().getEnte());
                tagCSVersatore.setStruttura(parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                ...
                versamento.getStrutturaUpdVers().setDescTipologiaUnitaDocumentariaNonVerificata(
                        parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                                .getTipologiaUnitaDocumentaria());

                // memorizzo la chiave in una variabile di appoggio per usarla in diverse parti
                // dell'elaborazione
                tagCSChiave.setAnno(new BigDecimal(parsedIndiceUpd.getUnitaDocumentaria()
                        .getIntestazione().getChiave().getAnno()).longValue());
                tagCSChiave.setNumero(parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                        .getChiave().getNumero());
                tagCSChiave.setTipoRegistro(parsedIndiceUpd.getUnitaDocumentaria().getIntestazione()
                        .getChiave().getTipoRegistro());
                 ...
                 * NEW URN
                 */
                versamento.getStrutturaUpdVers().setUrnPartVersatore(
                        MessaggiWSFormat.formattaUrnPartVersatore(tagCSVersatore));
                ...
                if (parsedIndiceUpd.getUnitaDocumentaria().getDocumentoPrincipale() != null) {
                    versamento.getStrutturaUpdVers().setDescTipoDocPrincipaleNonVerificato(
                            parsedIndiceUpd.getUnitaDocumentaria().getDocumentoPrincipale()
                                    .getTipoDocumento());
                }
                ...
                versamento.addEsitoControlloOnGeneraliBundle(
                        ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_XMLVSXSD),
                        SeverityEnum.ERROR, TipiEsitoErrore.NEGATIVO, MessaggiWSBundle.XSD_001_002,
                        msg);

                if (ex instanceof JAXBException) {
                    // se non riesco a convalidare l'XML magari posso provare a leggerlo
                    // in modo meno preciso e tentare di riscostruire la struttura, la chiave a il
                    // tipo documento
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            if (versamento.getVersamento().getParametriAggiornamento() != null) {
                // tipo aggiornamento
                strutturaUpdVers.setTipoAggiornamento(versamento.getVersamento()
                        .getParametriAggiornamento().getTipoAggiornamento());

                // note aggiornamento
                myEsito.getParametriAggiornamento().setTipoAggiornamento(versamento.getVersamento()
                        .getParametriAggiornamento().getTipoAggiornamento());// valore
                strutturaUpdVers.setNoteAggiornamento(versamento.getVersamento()
                        .getParametriAggiornamento().getNoteAggiornamento());

                if (versamento.getVersamento().getParametriAggiornamento()
                        .isForzaAggiornamento() != null) {
                    // risposta
                    myEsito.getParametriAggiornamento().setForzaAggiornamento(versamento
                            .getVersamento().getParametriAggiornamento().isForzaAggiornamento());// valore
                    // comunicato

                    // struttura
                    strutturaUpdVers.setFlForzaAggiornamento(versamento.getVersamento()
                            .getParametriAggiornamento().isForzaAggiornamento());
                }
            ...
            //
            myEsito.setIdentificativoRapportoVersamento(MessaggiWSFormat
                    .formattaUrnPartRappVersUpd(tmpUrn, Costanti.UrnFormatter.URN_RAPP_VERS_V2));
            // SIP
            SIPType mySip = new SIPType();
            myEsito.setSIP(mySip);
            mySip.setDataVersamento(
                    XmlDateUtility.dateToXMLGregorianCalendar(sessione.getTmApertura()));
            mySip.setURNIndiceSIP(MessaggiWSFormat.formattaUrnIndiceSipUpd(tmpUrn,
            ...
            if (versamento.getVersamento().getUnitaDocumentaria().getConfigurazione() != null) {
                ...
                if (versamento.getVersamento().getUnitaDocumentaria().getConfigurazione()
                    ...
                    strutturaUpdVers
                            .setTipoConservazione(versamento.getVersamento().getUnitaDocumentaria()
                                    .getConfigurazione().getTipoConservazione().name());
                }

                // imposta il flag globale di simulazione scrittura
                Boolean simulaScrittura = versamento.getVersamento().getUnitaDocumentaria()
                        .getConfigurazione().isSimulaSalvataggioDatiInDB();
                if (simulaScrittura != null && simulaScrittura) {
                    versamento.setSimulaScrittura(true);
                    IntestazioneType intestazione = parsedIndiceUpd.getUnitaDocumentaria()
                            .getIntestazione();
            ...
            tmpVersatoreType.setUserID(versamento.getVersamento().getUnitaDocumentaria()
            ...
            myEsito.setTipologiaUnitaDocumentaria(versamento.getVersamento().getUnitaDocumentaria()
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            if (prosegui) {
                prosegui = this.controllaStrutturaVersatore(versamento, rispostaWs, tagCSVersatore,
                        descChiaveUD);
            }

            // nota: comunque vada, da qui in poi la sessione non puï¿½ essere piï¿½ DUBBIA
            // (puï¿½ essere solo OK, FALLITA oppure ERRATA) perchï¿½ ho tutti gli elementi per
            // memorizzare
            ...
            // controlliXSD = POSITVO && controlliIntestazione = POSITVO (controlli generali) ->
            // prosegui
            ...
            if (prosegui) {
                prosegui = super.checkTipoUDAndAbilUpdMeta(myEsito, versamento, rispostaWs,
                        descChiaveUD);
            }
            ...
            if (prosegui && versamento.hasProfiloUnitaDocumentariaToUpd()) {
                prosegui = super.controllaProfiloUnitaDocumentaria(versamento, rispostaWs,
                        descChiaveUD);
            }

            // se trovato profilo UD
            if ((prosegui && versamento.hasProfiloNormativoToUpd()) && versamento
                    .getModificatoriWSCalc().contains(ModificatoriWS.TAG_PROFILI_UPD_1_6)) {
            ...
            if (prosegui && versamento.hasPAFascicoliSecondariToUp()
                    && !versamento.hasPAFascicoloPrincipaleToUpd()) {
    ...
    private void initElementiDaAggiornare(UpdVersamentoExt versamento) {
        ...
        versamento.setHasPAFascicoloPrincipaleToUpd(versamento.hasProfiloArchivisticoToUpd()
                && versamento.getVersamento().getUnitaDocumentaria().getProfiloArchivistico()
                        .getFascicoloPrincipale() != null);
        // ProfiloArchivistico -> FascicoliSecondari
        versamento.setHasPAFascicoliSecondariToUp(versamento.hasProfiloArchivisticoToUpd()
                && versamento.getVersamento().getUnitaDocumentaria().getProfiloArchivistico()
                        .getFascicoliSecondari() != null);
        ...
        versamento.setHasProfiloArchivisticoToUpdNull(versamento.hasProfiloArchivisticoToUpd()
                && !versamento.hasPAFascicoloPrincipaleToUpd()
                && !versamento.hasPAFascicoliSecondariToUp());
        // ProfiloUnitaDocumentaria
        versamento.setHasProfiloUnitaDocumentariaToUpd(versamento.getVersamento()
        ...
        versamento.setHasDatiSpecificiMigrazioneToUpd(versamento.getVersamento()
                .getUnitaDocumentaria().getDatiSpecificiMigrazione() != null);
        // DocumentiCollegati
        ...
        versamento.setHasDocPStrutturaOriginaleToUpd(versamento.hasDocumentoPrincipaleToUpd()
                && versamento.getVersamento().getUnitaDocumentaria().getDocumentoPrincipale()
                        .getStrutturaOriginale() != null);

        // Allegati
        versamento.setHasAllegatiToUpd(
                versamento.getVersamento().getUnitaDocumentaria().getAllegati() != null);
        // Annessi
        versamento.setHasAnnessiToUpd(
                versamento.getVersamento().getUnitaDocumentaria().getAnnessi() != null);
        // Annotazioni
        versamento.setHasAnnotazioniToUpd(
                versamento.getVersamento().getUnitaDocumentaria().getAnnotazioni() != null);
    ...
    private void aggiornamentiUnitaDoc(StrutturaUpdVers strutturaUpdVers,
        ...
        if (versamento.hasProfiloArchivisticoToUpd()) {
            if (versamento.hasPAFascicoloPrincipaleToUpd()) {
                versamento.addAggiornamentoUnitaDocumentaria(
                        AggiornamentoEffettuato.HASPAFASCICOLOPRINCIPALETOUPD);
            }
            if (versamento.hasPAFascicoliSecondariToUp()) {
                versamento.addAggiornamentoUnitaDocumentaria(
                        AggiornamentoEffettuato.HASPAFASCICOLISECONDARITOUP);
            }
        }
        //
        if (versamento.hasProfiloUnitaDocumentariaToUpd()) {
            versamento.addAggiornamentoUnitaDocumentaria(
                    AggiornamentoEffettuato.HASPROFILOUNITADOCUMENTARIATOUPD);
        }
        //
        if (versamento.hasProfiloNormativoToUpd()) {
            versamento.addAggiornamentoUnitaDocumentaria(
                    AggiornamentoEffettuato.HASPROFILONORMATIVOTOUPD);
        }
        //
        if (versamento.hasDocumentiCollegatiToUpd()) {
            versamento.addAggiornamentoUnitaDocumentaria(
                    AggiornamentoEffettuato.HASDOCUMENTICOLLEGATITOUPD);
        }
        //
        if (versamento.hasDatiSpecificiToBeUpdated()) {
            versamento.addAggiornamentoUnitaDocumentaria(
                    AggiornamentoEffettuato.HASDATISPECIFICITOBEUPDATED);
        }
        if (versamento.hasDatiSpecificiMigrazioneToUpd()) {
            versamento.addAggiornamentoUnitaDocumentaria(
                    AggiornamentoEffettuato.HASDATISPECIFICIMIGRAZIONETOUPD);
        }
        //
        ...
        if (!strutturaUpdVers.getComponentiAttesi().isEmpty()) {
            versamento
                    .addAggiornamentoUnitaDocumentaria(AggiornamentoEffettuato.HASCOMPONENTITOUPD);
        }
    }

    private void aggiornamentiDocumenti(StrutturaUpdVers strutturaUpdVers,
            CategoriaDocumento catDoc, UpdVersamentoExt versamento) {
        for (UpdDocumentoVers documento : strutturaUpdVers.getDocumentiAttesi().stream()
            ...
            if (documento.getRifUpdDocumento().getDatiSpecificiMigrazione() != null) {
                versamento.addAggiornamentoDocumenti(AggiornamentoEffettuato.HASDOCDATISPECMIGRAZ,
                        catDoc, documento.getRifUpdDocumento().getIDDocumento());
            }
            //
            if (!documento.getUpdComponentiAttesi().isEmpty()) {
                versamento.addAggiornamentoDocumenti(AggiornamentoEffettuato.HASDOCCOMPONENTI,
                        catDoc, documento.getRifUpdDocumento().getIDDocumento());
            }
            //
            for (UpdComponenteVers componente : documento.getUpdComponentiAttesi()) {
                //
                versamento.addAggiornamentoCompDocumenti(AggiornamentoEffettuato.HASCOMPONENTE,
                        catDoc, componente.getKeyCtrl());
                //
                if (componente.getDatiSpecifici() != null) {
                    versamento.addAggiornamentoCompDocumenti(
                            AggiornamentoEffettuato.HASCOMPONENTEDATISPEC, catDoc,
                            componente.getKeyCtrl());
                }
                //
                if (componente.getDatiSpecificiMigrazione() != null) {
                    versamento.addAggiornamentoCompDocumenti(
                            AggiornamentoEffettuato.HASCOMPONENTEDATISPECMIGRAZ, catDoc,
                            componente.getKeyCtrl());
                }
            }
        }
    ...
    private void buildUrnSipOnEsito(CompRapportoUpdVers myEsito, UpdVersamentoExt versamento) {
        ...
        if (versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_URN_SIP_1_5)) {
            ...
            String urnSip = MessaggiWSFormat.formattaUrnSip(tmpUrn,
                    Costanti.UrnFormatter.URN_SIP_UPD_V2);
