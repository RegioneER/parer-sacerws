...
public class EidasFirmaBuilder extends EidasBaseWrapperResult
	implements IEidasBuilderVFObj<VFFirmaCompType> {
    ...
    @Override
    public VFFirmaCompType build(EidasWSReportsDTOTree eidasReportsDto,
	...
	// test
	EidasWrapperResultControl.fieldCannotBeNull(signatureW, "signingCertificate",
		Optional.empty());
	...
	EidasWrapperResultControl.fieldCannotBeNull(xmlcertificate, "notBefore",
		Optional.ofNullable(xmlcertificate.getId()));
	certifFirmatarioType.setDtIniValCertifFirmatario(
	...
	EidasWrapperResultControl.fieldCannotBeNull(xmlcertificate, "notAfter",
		Optional.ofNullable(xmlcertificate.getId()));
	certifFirmatarioType.setDtFinValCertifFirmatario(
		XmlDateUtility.dateToXMLGregorianCalendar(xmlcertificate.getNotAfter()));
	...
	certifCaType.setDtIniValCertifCa(
		XmlDateUtility.dateToXMLGregorianCalendar(xmlcertificateCA.getNotBefore()));

	// test
	EidasWrapperResultControl.fieldCannotBeNull(xmlcertificateCA, "notAfter",
		Optional.ofNullable(xmlcertificateCA.getId()));
	certifCaType.setDtFinValCertifCa(
		XmlDateUtility.dateToXMLGregorianCalendar(xmlcertificateCA.getNotAfter()));
	...
	int urlIdx = 1;
	// check NON empty URL
	List<String> crlFiltered = xmlcertificateCA.getCRLDistributionPoints().stream()
	...
	Iterable<String> ocspCombinedUrls = CollectionUtils
		.union(xmlcertificateCA.getOCSPAccessUrls(), xmlcertificate.getOCSPAccessUrls())
	...
	if (signatureW.getDigestAlgorithm() != null
		&& signatureW.getEncryptionAlgorithm() != null) {
	...
	XmlBasicBuildingBlocks bbb = reports.getDetailedReport()
		.getBasicBuildingBlockById(signatureW.getId());
	if (bbb == null) {
	...
	if (signatureW.isCounterSignature()) {
	    bbbParent = reports.getDetailedReport()
		    .getBasicBuildingBlockById(signatureW.getParent().getId());
	    if (bbbParent == null) {
		// TOFIX: cosa fare in questi casi ?!
		LOG.warn("Parent BasicBuildingBlockById not found SIGNATURE ID = {}",
			signatureW.getParent().getId());
	    }
	}
	// CONTROLLI FIRMA
	// global result
	boolean isSignaturePassed = reports.getSimpleReport().isValid(signatureW.getId());
	buildContrFirmaComp(firmaCompType, isSignaturePassed, signatureW, xmlcertificate, bbb,
		bbbParent);
	...
	// CRL + CONTR
	buildFirmaCompCERwithCRL(xmlcertificate, certifCaType, firmaCompType, dataDiRiferimento,
		bbb);
    ...
    private void buildContrFirmaComp(VFFirmaCompType firmaCompType, boolean isSignaturePassed,
	    SignatureWrapper signature, CertificateWrapper xmlcertificate,
	    XmlBasicBuildingBlocks bbb, XmlBasicBuildingBlocks bbbParent) {
	...
	if (bbbParent != null) {
	    ...
	    isFormatCompliant = formatCompliantConstraint != null && formatCompliantConstraint
		    .getConclusion().getIndication().equals(Indication.PASSED);
	} else {
	    formatCompliantConstraint = bbb.getFC();
	    isFormatCompliant = bbb != null && formatCompliantConstraint != null
		    && formatCompliantConstraint.getConclusion().getIndication()
	...
	//
	firmaCompType.setDsMsgEsitoContrConforme(
		esitoContrConforme.equals(SacerIndication.POSITIVO) ? esitoContrConforme.message()
			: generateErrorContrMsgEsito(esitoContrConforme.message(),
				formatCompliantConstraint.getConclusion().getIndication(),
				Optional.empty()));
	//
	SacerIndication esitoVerifFirma = isSignaturePassed ? SacerIndication.POSITIVO
	...
	if (!controlliAbilitati.get(ParametroApplFl.FL_ABILITA_CONTR_CRITTOG_VERS).booleanValue()) {
		...
		if (!cv.getConclusion().getIndication().equals(Indication.PASSED)
		    ...
		    // log eidas message
		    logEidasConclusion(xmlcertificate, cv,
			    VFTipoControlloType.CRITTOGRAFICO.name());

		    contrFirmaCompType.setTiEsitoContrFirma(esito.name());
		    contrFirmaCompType.setDsMsgEsitoContrFirma(generateErrorContrMsgEsito(
	...
	if (!controlliAbilitati.get(ParametroApplFl.FL_ABILITA_CONTR_TRUST_VERS).booleanValue()) {
	    ...
	    //
	    SacerIndication esito = signature.isTrustedChain() || isSignaturePassed
		    ? SacerIndication.POSITIVO
		    : SacerIndication.NEGATIVO;
	    //
	    contrFirmaCompType.setTiEsitoContrFirma(esito.name());
	    contrFirmaCompType.setDsMsgEsitoContrFirma(esito.message());
	...
	if (!controlliAbilitati.get(ParametroApplFl.FL_ABILITA_CONTR_CERTIF_VERS).booleanValue()) {
		...
		if ((!isc.getConclusion().getIndication().equals(Indication.PASSED)
			|| !sav.getConclusion().getIndication().equals(Indication.PASSED))
			&& !isSignaturePassed) {
		    // log eidas message
		    XmlConstraintsConclusion constraint = !isc.getConclusion().getIndication()
			    .equals(Indication.PASSED) ? isc : sav;
		    logEidasConclusion(xmlcertificate, constraint,
		    ...
		    contrFirmaCompType.setTiEsitoContrFirma(esito.name());
		    contrFirmaCompType.setDsMsgEsitoContrFirma(generateErrorContrMsgEsito(
			    esito.message(), constraint.getConclusion().getIndication(),
			    Optional.of(
			...
			case REVOKED:
				...
				detailedMsg.append((subXCV.get().getRevocationInfo() != null
					&& subXCV.get().getRevocationInfo()
						.getRevocationDate() != null ? "revocato in data "
    ...
    private void buildFirmaCompCERwithCRL(CertificateWrapper xmlcertificate,
	    VFCertifCaType certifCaType, VFFirmaCompType firmaCompType,
	...
	// CRL
	RevocationWrapper crl = findRevocationByType(xmlcertificate, dataDiRiferimento,
		RevocationType.CRL);
	if (hasOcsp || crl == null) {
	    //
    ...
    private void buildFirmaCompCAwithCRL(CertificateWrapper xmlcertificateCA,
	    VFCertifCaType certifCaType, VFFirmaCompType firmaCompType,
	    ZonedDateTime dataDiRiferimento) throws NoSuchMethodException, IllegalAccessException,
	    IllegalArgumentException, InvocationTargetException {
	...
	// CRL
	RevocationWrapper crl = findRevocationByType(xmlcertificateCA, dataDiRiferimento,
		RevocationType.CRL);
	if (hasOcsp || crl == null) {
	    return;
	}
    ...
    private void buildFirmaCompCAwithOCSP(Reports reports, CertificateWrapper xmlcertificateCA,
	    ...
	    InvocationTargetException {
	// OCSP
	RevocationWrapper ocsp = findRevocationByType(xmlcertificateCA, dataDiRiferimento,
		RevocationType.OCSP);
	if (ocsp == null) {
	    return;
	} // no ocsp
    ...
    // No CRL
    private void buildContrCRLNotFoundFirmaComp(VFFirmaCompType firmaCompType,
	    CertificateWrapper xmlcertificate, boolean hasOcsp) {
	buildContrCRLFirmaComp(firmaCompType, xmlcertificate, null, null, hasOcsp);
    }

    private void buildContrCRLFirmaComp(VFFirmaCompType firmaCompType,
	    CertificateWrapper xmlcertificate, RevocationWrapper crl, XmlBasicBuildingBlocks bbb,
	    boolean hasOcsp) {
	...
	if (!controlliAbilitati.get(ParametroApplFl.FL_ABILITA_CONTR_REVOCA_VERS).booleanValue()) {
		...
		if (!bbbFounded || bbb.getXCV().getSubXCV().stream()
			.noneMatch(c -> c.getId().equals(xmlcertificate.getId()))) {
		    SacerIndication esito = SacerIndication.NEGATIVO;
		    //
		    contrFirmaCompType.setTiEsitoContrFirma(esito.name());
		    contrFirmaCompType.setDsMsgEsitoContrFirma(esito.message());
		    ...
		    XmlSubXCV subXvc = bbb.getXCV().getSubXCV().stream()
			    .filter(c -> c.getId().equals(xmlcertificate.getId()))
			...
			contrFirmaCompType.setDsMsgEsitoContrFirma(esito.message());
			    ...
			    .equals(Indication.PASSED)) {
			// log eidas message
			logEidasConclusion(xmlcertificate, xmlRacs.get(0),
    ...
    private void buildContrOCSPNotFoundFirmaComp(VFFirmaCompType firmaCompType,
	    CertificateWrapper xmlcertificate, boolean hasCrl) {
	buildContrOCSPFirmaComp(firmaCompType, xmlcertificate, null, null, hasCrl);
    }

    private void buildContrOCSPFirmaComp(VFFirmaCompType firmaCompType,
	    CertificateWrapper xmlcertificate, CertificateRevocationWrapper ocsp,
	...
	if (!controlliAbilitati.get(ParametroApplFl.FL_ABILITA_CONTR_REVOCA_VERS).booleanValue()) {
		...
		contrFirmaCompType.setTiEsitoContrFirma(esito.name());
		contrFirmaCompType.setDsMsgEsitoContrFirma(esito.message()
			+ ": controllo non necessario in quanto avviene tramite CRL");
