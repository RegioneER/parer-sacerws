...
public class DocumentoVersVFirmeHash {
    ...
    public boolean verificaDocumentoGenFirma(DocumentoVers documento, long idTipoUd,
	...
	for (ComponenteVers tmpCV : documento.getFileAttesi()) {
	    ...
	    if (tmpCV.getMySottoComponente() == null
		    && tmpCV.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE
		...
		myAvanzamentoWs.setFase("inizio verifica componente firma")
			...
			.setComponente(
				Long.toString(tmpCV.getMyComponente().getOrdinePresentazione()))
			.logAvanzamento();
		...
		if (proseguiElaborazione) {
			...
			tmpCV.withAcdEntity().setTmRifTempVers(
				XmlDateUtility.xmlGregorianCalendarToZonedDateTime(
					tmpCV.getMyComponente().getRiferimentoTemporale()));
		    }

		    if (tmpCV.getMyComponente().isUtilizzoDataFirmaPerRifTemp() != null) {
			if (tmpCV.getMyComponente().isUtilizzoDataFirmaPerRifTemp()
				.booleanValue()) {
		    ...
		    tmpCV.withAcdEntity().setIdStrut(
			...
			// riscorro ogni componente: deve essere un sottocomponente, essere mio
			// figlio e
			// di tipo FILE
			if (tmpCV2.getMySottoComponente() != null
				&& tmpCV2.getRifComponenteVersPadre().getMyComponente().getID()
					.equals(tmpCV.getId())
				&& tmpCV2.getTipoSupporto() == ComponenteVers.TipiSupporto.FILE) {
			    // creo l'istanza di AroCompDoc
			    tmpCV2.withAcdEntity().setIdStrut(new BigDecimal(
				...
				break;
			    default:
				// se è un sottocomponente generico... viene passato alla libreria
				// per la
		    ...
		    RispostaControlli rispostaControlli = this
			    .elabCompDocMockWithVerificaFirmaAndFormatoResult(tmpCV,
				    sottoComponentiFirma, sottoComponentiMarca,
				    sottoComponentiAltri, idTipoUd, versamento,
				    esitoConfigurazione);

		    if (rispostaControlli.getrLong() == -1) { // se il valore è -1, allora codErr è
							      // un codice di errore
			...
			if (rispostaControlli.getCodErr().equals(MessaggiWSBundle.ERR_666)) {
			    ...
			    // aggiunge l'errore che poi potrà essere "forzato" attraverso appositi
			    // flag
			    if (isForzaAccett) {
				versamento.addWarningDoc(documento, tmpCV.getChiaveComp(),
					rispostaControlli.getCodErr(),
					rispostaControlli.getDsErr());
			    } else {
				versamento.addErrorDoc(documento, tmpCV.getChiaveComp(),
					rispostaControlli.getCodErr(),
					rispostaControlli.getDsErr());
			    }
			}
		...
		if (proseguiElaborazione) {
		    //
		    VerificaFirmaWrapperVersamentoUtil.buildECFirmatarioMarcaType(versamento, tmpCV,
			    statoCompCorrente);
			...
			if (statoCompCorrente.compEsitoVerFirmeHasErr) {
			    ...
			    tmpCV.withAcdEntity()
				    .setDsMsgEsitoVerifFirme(SacerIndication.NEGATIVO.message());
			    ...
			    tmpCV.withAcdEntity()
				    .setDsMsgEsitoVerifFirme(SacerIndication.WARNING.message());
			    tmpCV.getRifComponenteResp().getEsitoComponente()
			    ...
			    tmpCV.withAcdEntity()
				    .setDsMsgEsitoVerifFirme(SacerIndication.POSITIVO.message());
			    tmpCV.getRifComponenteResp().getEsitoComponente()
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloNonConformita()
				|| isForzaAccett && esitoConfigurazione.isAccettaMarcaSconosciuta()
					.booleanValue()) {
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloNonConformita()
				|| isForzaAccett && esitoConfigurazione.isAccettaFirmaSconosciuta()
					.booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloNonConformita()
				|| isForzaAccett && esitoConfigurazione.isAccettaFirmaNonConforme()
					.booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloNonConformita()
				|| isForzaAccett && esitoConfigurazione.isAccettaFirmaNoDelibera45()
					.booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloCrittografico()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCrittograficoNegativo().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_002_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloTrust()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloTrustNegativo().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_003_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloCertificato()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCertificatoScaduto().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_004_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloCertificato()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCertificatoNoValido().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_004_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti()
				.isConfigFlagForzaControlloCertificato()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCertificatoNoFirma().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_004_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCRLNegativo().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_005_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCRLScaduta().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_005_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCRLNoValida().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_005_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloCRLNoScaric().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_005_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloOCSPNoValido().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_007_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloOCSPNegativo().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_007_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
			...
			if (versamento.getStrutturaComponenti().isConfigFlagForzaControlloRevoca()
				|| isForzaAccett && esitoConfigurazione
					.isAccettaControlloOCSPNoScaric().booleanValue()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FIRMA_007_001, tmpCV.getChiaveComp());
			    tmpComponenteWarn = true;
			} else {
	...
	if (proseguiElaborazione) {
	    // MEV#18660
	    RispostaControlli rispostaControlli = this.verificaFormatiCompDoc(documento,
		    statoDocumento, versamento, esitoConfigurazione);

	    if (rispostaControlli.getrLong() == -1) { // se il valore è -1, allora codErr è un
						      // codice di errore 666
		proseguiElaborazione = false;
		rispostaWs.setSeverity(SeverityEnum.ERROR);
		rispostaWs.setEsitoWsError(rispostaControlli.getCodErr(),
			rispostaControlli.getDsErr());
	...
	if (proseguiElaborazione) {
		...
		if (statoDocumento.verFirmeCompCodiceEsitoNeg) {
		    ...
		    esitoDoc.getEsitoDocumento()
			    .setVerificaFirmeDocumento(ECEsitoPosNegWarType.NEGATIVO);
		    ...
		    esitoDoc.getEsitoDocumento()
			    .setVerificaFirmeDocumento(ECEsitoPosNegWarType.WARNING);
		} else {
		    ...
		    esitoDoc.getEsitoDocumento()
			    .setVerificaFirmeDocumento(ECEsitoPosNegWarType.POSITIVO);
		}
	    }
    ...
    ///
    public void verificaDocumentoGenHash(DocumentoVers documento, AbsVersamentoExt versamento,
	...
	for (ComponenteVers tmpCV : documento.getFileAttesi()) {
		...
		if (versamento.getStrutturaComponenti().isFlagVerificaHash()
			&& !tmpCV.isHashForzato() && !tmpCV.isHashVersNonDefinito()) {
			...
			if (versamento.getStrutturaComponenti().isFlagAccettaHashErrato()) {
			    versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
				    ...
				    tmpCV.getHashVersatoEncoding(),
				    tmpCV.getHashVersatoAlgoritmo());
			    tmpComponenteWarn = true;
			} else {
			    versamento.listErrAddErrorDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.HASH_004_001, tmpCV.getChiaveComp(),
    ...
    private RispostaControlli verificaFormatiCompDoc(DocumentoVers documento,
	    StatoDocumento statoDoc, AbsVersamentoExt versamento,
	    ECConfigurazioneType esitoConfigurazione) {
	//
	...
	for (ComponenteVers tmpCV : documento.getFileAttesi()) {
		...
		// MEV#14881
		boolean isForzaFormato = versamento.getStrutturaComponenti()
			.isConfigFlagForzaFormato();
		...
		if ((mock.getTiEsitoContrFormatoFile().equals(SacerIndication.POSITIVO.name())
		    ...
		    tagEsitoRiconoscimentoFormato = ECEsitoPosNegWarDisType
			    .valueOf(mock.getTiEsitoContrFormatoFile());
			...
			statoDoc.docEsitoCompHasWar = true; // propago la condizione di warning del
							    // componente
			versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
		...
		} else if (mock.getTiEsitoContrFormatoFile()
			.equals(SacerIndication.NEGATIVO.name())) {
		    // caso negativo
			...
			if (isForzaFormato || isAccettFormNeg && isForzaAccettazione) { // se
											// ammette
											// errori
			    if (this.isTipoFileAmmesso(mock.getDsFormatoRapprCalc(),
				    versamento.getStrutturaComponenti().getIdStruttura(),
				    tmpCV.getIdTipoComponente())) { // se ammette errori e se il
								    // formato calcolato è
				// ammesso
				tagEsitoRiconoscimentoFormato = ECEsitoPosNegWarDisType.WARNING;
				tagEsitoComponente = ECEsitoPosNegWarType.WARNING;
				statoDoc.docEsitoCompHasWar = true; // propago la condizione di
								    // warning del componente
				versamento.listErrAddWarningDoc(documento, tmpCV.getChiaveComp(),
					MessaggiWSBundle.FORMATO_001_001, tmpCV.getChiaveComp(),
					tagMessRiconoscimentoFormato);
				...
				statoDoc.docEsitoCompHasNeg = true; // propago la condizione di
			    ...
			    tagEsitoRiconoscimentoFormato = ECEsitoPosNegWarDisType.NEGATIVO;
			    tagEsitoComponente = ECEsitoPosNegWarType.NEGATIVO;
			    statoDoc.docEsitoCompHasNeg = true; // propago la condizione di errore
								// del componente
			    versamento.listErrAddErrorDoc(documento, tmpCV.getChiaveComp(),
				    MessaggiWSBundle.FORMATO_001_001, tmpCV.getChiaveComp(),
				    tagMessRiconoscimentoFormato);
			...
			// formato standard (vedi caso del TSR con formato P7M da estrazione
			// formato)
			// superfluo verificare ANCHE che non sia associato un formato (già ottenuto
			// l'errore sul formato vedi 606)
			// attraverso i flag sarà possibile decidere se l'esito dovrà essere
			// negativo o
			// warning
			if (tmpCV.getTipoUso().equals("MARCA")) { // se è una marca
			    if (isForzaAccettFormato /*
						      * && tmpTabCD.getDecFormatoFileStandard() ==
						      * null
						      */
				    && esitoConfigurazione.isAccettaMarcaSconosciuta()
					    .booleanValue()) {
				tagEsitoRiconoscimentoFormato = ECEsitoPosNegWarDisType.WARNING;
				tagEsitoComponente = ECEsitoPosNegWarType.WARNING;
				statoDoc.docEsitoCompHasWar = true; // propago la condizione di
			    ...
			    if (isForzaAccettFormato /*
						      ...
						      * null
						      */
				    && esitoConfigurazione.isAccettaFirmaSconosciuta()
					    .booleanValue()) {
				tagEsitoRiconoscimentoFormato = ECEsitoPosNegWarDisType.WARNING;
				tagEsitoComponente = ECEsitoPosNegWarType.WARNING;
				statoDoc.docEsitoCompHasWar = true; // propago la condizione di
			...
			.equals(SacerIndication.DISABILITATO.name())) {
		    // caso disattivato
		...
		if (isComponente) { // componente
		    if (versamento.getModificatoriWSCalc()
			    .contains(ModificatoriWS.TAG_VERIFICA_FORMATI_1_25)) { // versione
			// WS
			// successiva
			// alla
			// 1.2
			tmpCV.getRifComponenteResp().setFormatoRappresentazione(tagFormatRapp);
			tmpCV.getRifComponenteResp()
				.setFormatoRappresentazioneEsteso(tagFormatRappEsteso);
		    ...
		    } else if (versamento.getModificatoriWSCalc()
			    .contains(ModificatoriWS.TAG_VERIFICA_FORMATI_OLD)) { // versione
			// ws
			// 1.2
			// e
			// precedenti
			if (tagEsitoRiconoscimentoFormato == ECEsitoPosNegWarDisType.POSITIVO
				...
				|| tagEsitoRiconoscimentoFormato == ECEsitoPosNegWarDisType.WARNING) {
			    tmpCV.getRifComponenteResp().getEsitoComponente()
				    .setVerificaRiconoscimentoFormato(
					    tagEsitoRiconoscimentoFormato);
			}
		    }

		    /*
		     * se il precente esito - impostato dalla verifica firma - è positivo, allora
		     * l'esito prende il valore della verifica formato. se il precente esito è
		     * warning, l'esito prende il valore della verifica formato solo se questa non è
		     * positiva. altrimenti il valore resta inalterato, cioè negativo
		     */
		    if (tmpCV.getRifComponenteResp().getEsitoComponente()
			    ...
			    || tmpCV.getRifComponenteResp().getEsitoComponente()
				    ...
				    && tagEsitoComponente != ECEsitoPosNegWarType.POSITIVO) {
			tmpCV.getRifComponenteResp().getEsitoComponente()
				.setCodiceEsito(tagEsitoComponente);
		    }
			...
			tmpCV.getRifSottoComponenteResp()
				.setFormatoRappresentazioneEsteso(tagFormatRappEsteso);
			...
			if (tagEsitoRiconoscimentoFormato == ECEsitoPosNegWarDisType.POSITIVO
			    ...
			    tmpCV.getRifSottoComponenteResp().getEsitoSottoComponente()
				    .setVerificaRiconoscimentoFormato(
					    tagEsitoRiconoscimentoFormato);
			} else if (tagEsitoRiconoscimentoFormato == ECEsitoPosNegWarDisType.WARNING) { // l'esito
												       // del
			    // sottocomp non
			    // prevede il
			    // WARNING nel ws
		    ...
		    /*
		     * se il precedente esito - impostato dalla verifica firma - è positivo, allora
		     * l'esito prende il valore della verifica formato. se il precente esito è
		     * warning, l'esito prende il valore della verifica formato solo se questa non è
		     * positiva. altrimenti il valore resta inalterato, cioè negativo
		     */
	...
	try {
	    ...
	    rc.setrLong(-1); // se in uscita vale -1, allora il chiamante lo interpreta come
			     // eccezione
	    rc.setCodErr(MessaggiWSBundle.ERR_666);
	    rc.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
    ...
    private RispostaControlli elabCompDocMockWithVerificaFirmaAndFormatoResult(
	    ComponenteVers componente, List<ComponenteVers> sottoComponentiFirma,
	    List<ComponenteVers> sottoComponentiMarca, List<ComponenteVers> sottoComponentiAltri,
	    long idTipoUd, AbsVersamentoExt versamento, ECConfigurazioneType esitoConfigurazione) {
	...
	if (componente.getRifComponenteMM() != null
		&& componente.getRifComponenteMM().isForzaFirmeFormati()) {
	    /*
	     * caso della forzatura di formato del file in un versamento multimedia Lo stato di
	     * verifica firma viene fissato a "non firmato"
	     */
	    if (sottoComponentiFirma.isEmpty() && sottoComponentiMarca.isEmpty()
		    && sottoComponentiAltri.isEmpty()) {
		it.eng.parer.ws.xml.versReqMultiMedia.ComponenteType tmpComponente = componente
			.getRifComponenteMM().getMyComponenteMM();
		CompDocMock mock = componente.withAcdEntity();
		...
		if (tmpComponente.getForzaFormato() != null) {
		    ...
		    rs = controlliPerFirme.confrontaFormati(
			    componente.getRifComponenteMM().getIdFormatoFileCalc(),
			    componente.getIdFormatoFileVers());
		    if (rs.getrLong() != -1) { // i controlli del formato potrebbero aver reso
					       // un'eccezione "666"
			...
			mock.setDsFormatoRapprCalc(tmpComponente.getForzaFormato()
				.getFormatoRappresentazioneCompatto());
		...
		log.error(
			"Errore interno: si può forzare il formato solo su componenti privi di sottocomponenti");
	    }
	    ...
	    /*
	     * caso della forzatura di formato del file a causa del tipo struttura Lo stato di
	     * verifica firma viene fissato a "non firmato"
	     */
	    if (sottoComponentiFirma.isEmpty() && sottoComponentiMarca.isEmpty()
		    && sottoComponentiAltri.isEmpty()) {
	    ...
	    /*
	     * caso della verifica normale di firme e formati il flag relativo al NON calcolo di
	     * firma e formato viene posto a "0"
	     */
	    ZonedDateTime rifTempVers = componente.withAcdEntity().getTmRifTempVers();
	    if (rifTempVers == null) {
		Instant dataVersamento = versamento.getStrutturaComponenti().getDataVersamento()
			.toInstant();
	    ...
	    // detached
	    rs = firmeFormatiEjb.verifica(componente, sottoComponentiFirma, sottoComponentiMarca,
		...
		rs = firmeFormatiEjb.verifica(tmpComponenteVers, tmpEmptyComponenteVerses,
			tmpEmptyComponenteVerses, rifTempVers, idTipoUd, versamento);
		if (rs.getrLong() == -1) {
		    return rs; // error
		}
