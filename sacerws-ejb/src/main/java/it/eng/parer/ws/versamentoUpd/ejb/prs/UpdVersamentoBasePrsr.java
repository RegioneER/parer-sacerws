...
public abstract class UpdVersamentoBasePrsr {
    ...
    @EJB
    protected ControlliWS controlliEjb;
    // singleton ejb di gestione cache dei parser jaxb dei fascicoli
    @EJB
    protected XmlUpdVersCache xmlUpdVersCache;
    // stateless ejb: crea il rapporto di versamento e canonicalizza (C14N) il SIP
    @EJB
    protected RapportoVersBuilder rapportoVersBuilder;
    // controlli su versamento
    @EJB
    protected ControlliUpdVersamento updVersamentoControlli;
    // classe di supporto per la verifica e l'estrazione dei dati specifici
    @EJB
    protected UpdGestioneDatiSpec gestioneDatiSpec; // allocata nel costruttore
    // singleton ejb di gestione configurazioni applicative
    @EJB
    protected ConfigurationHelper configurationHelper;
    //
    @EJB
    protected UpdDocumentiPrsr updDocumentiPrsr;
    //
    @EJB
    protected UpdDocumentiIdCountPrsr updDocumentiIdCountPrsr;
    //
    @EJB
    protected UpdCollegamentiPrsr updCollegamentiPrsr;

    protected boolean controllaUnivocitaDocIDs(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        //
        updDocumentiIdCountPrsr.eseguiControlli(versamento);
        ...
        if (versamento.hasDocumentiToUpd()) {
            if (strutturaUpdVers.isTrovatiIdDocDuplicati()) {
                ...
                rispostaWs.setEsitoWsErrBundle(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_XSD_UNIVOCITADOCUMENTI),
        ...
        if (strutturaUpdVers.isCorrAllegatiDichiarati() != null) {
            if (!strutturaUpdVers.isCorrAllegatiDichiarati()) {
                ...
                rispostaWs.setEsitoWsErrBundle(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_XSD_CORRISPONDENZAALLEGATI),
        ...
        if (strutturaUpdVers.isCorrAnnessiDichiarati() != null) {
            if (!strutturaUpdVers.isCorrAnnessiDichiarati()) {
                ...
                rispostaWs.setEsitoWsErrBundle(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_XSD_CORRISPONDENZAANNESSI),
        ...
        if (strutturaUpdVers.isCorrAnnotazioniDichiarati() != null) {
            if (!strutturaUpdVers.isCorrAnnotazioniDichiarati()) {
                ...
                rispostaWs.setEsitoWsErrBundle(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_XSD_CORRISPONDENZAANNOTAZIONI),
    ...
    protected boolean controllaTipoAggiornamento(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        ...
        if (StringUtils.isNotBlank(versamento.getStrutturaUpdVers().getTipoAggiornamento())) {
            if (!versamento.getStrutturaUpdVers().getTipoAggiornamento()
                    .equals(TipoAggiornamento.METADATI.name())) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                // esito generale
                rispostaWs.setEsitoWsErrBundle(
                        ControlliWSBundle.getControllo(
                                ControlliWSBundle.CTRL_XSD_CONTROLLOTIPOAGGIORNAMENTO),
                ...
                // aggiunta su controlli generali
                versamento.addControlloOkOnGenerali(ControlliWSBundle
                        .getControllo(ControlliWSBundle.CTRL_XSD_CONTROLLOTIPOAGGIORNAMENTO));
            }
        } else {
            // default METADATI
            versamento.getStrutturaUpdVers()
                    .setTipoAggiornamento(TipoAggiornamento.METADATI.name());
        }
        return true; // prosegui controllo successivo
    }

    protected boolean controllaVersioneXSD(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        if (!versamento.getVersamento().getUnitaDocumentaria().getIntestazione().getVersione()
            ...
            // esito generale
            rispostaWs.setEsitoWsErrBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_INTS_VERSIONEXSD),
                    MessaggiWSBundle.UD_001_013, versamento.getVersamento().getUnitaDocumentaria()
                            .getIntestazione().getVersione());

            // aggiunta su controlli generali
            versamento.addEsitoControlloOnGeneraliBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_INTS_VERSIONEXSD),
                    rispostaWs.getSeverity(), TipiEsitoErrore.NEGATIVO, MessaggiWSBundle.UD_001_013,
                    versamento.getVersamento().getUnitaDocumentaria().getIntestazione()
                            .getVersione());
    ...
    protected boolean controllaStrutturaVersatore(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, CSVersatore tagCSVersatore, String descChiaveUD) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        // verifica la struttura versante e versatore
        boolean utenteOk = true;
        boolean strutturaOk = true;
        ...
        if (rispostaControlli.getrLong() < 1) {
            ...
            // esito generale
            rispostaWs.setEsitoWsError(ControlliWSBundle.getControllo(cdControllo),
                    rispostaControlli.getCodErr(), rispostaControlli.getDsErr());
        ...
        if (!versamento.getVersamento().getUnitaDocumentaria().getIntestazione().getVersatore()
                .getUserID().equals(versamento.getLoginName())) {
            ...
            rispostaWs.setEsitoWsErrBundle(
                    ...
                    MessaggiWSBundle.UD_001_005, versamento.getVersamento().getUnitaDocumentaria()
                            .getIntestazione().getVersatore().getUserID());
        ...
        if (strutturaOk && utenteOk) {
            versamento.getUtente()
                    .setIdOrganizzazioneFoglia(new BigDecimal(strutturaUpdVers.getIdStruttura()));
            rispostaControlli = controlliEjb.checkAuthWS(versamento.getUtente(),
                    versamento.getDescrizione(), TipiWSPerControlli.AGGIORNAMENTO_VERSAMENTO);
            if (!rispostaControlli.isrBoolean()) {
                ...
                rispostaWs.setEsitoWsError(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_INTS_ABILITAZIONEUTENTE),
                        rispostaControlli.getCodErr(), rispostaControlli.getDsErr());
        ...
        if (strutturaOk) {
            ...
            rispostaControlli = controlliSemantici.checkTipologiaUD(UpdCostanti.TIPOUD_SCONOSCIUTO,
                    descChiaveUD, strutturaUpdVers.getIdStruttura());
            // tipologia REG "Sconosciuto"
            if (rispostaControlli.isrBoolean()) {
                // registro ID TIPOUD
                strutturaUpdVers.setIdTipoUDUnknown(rispostaControlli.getrLong());
                strutturaUpdVers.setDescTipologiaUnitaDocumentariaUnknown(
            ...
            rispostaControlli = controlliSemantici.checkTipoRegistro(
                    UpdCostanti.TIPOREG_SCONOSCIUTO, descChiaveUD,
            ...
            rispostaControlli = controlliSemantici.checkTipoDocumento(
                    UpdCostanti.TIPDOC_PRINCIPALE_SCONOSCIUTO, strutturaUpdVers.getIdStruttura(),
                    true, descChiaveUD);
    ...
    protected boolean controlloChiaveECdKeyNorm(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, CSChiave tagCSChiave, String descChiaveUD) {
        boolean prosegui = controllaChiave(versamento, rispostaWs, tagCSChiave, descChiaveUD);
        // se esito precedente positivo - verifico se calcolare chiave normalizzata (se
        // esiste già su sistema restituisco errore)
        if (prosegui) {
            prosegui = controllaCdKeyNormalized(versamento, rispostaWs);
        }
        if (prosegui) {
            // aggiunta su controlli generali
            versamento.addControlloOkOnGenerali(
    ...
    private boolean controllaChiave(UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs,
            CSChiave tagCSChiave, String descChiaveUD) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        ...
        if (rispostaControlli.isrBoolean()) {
            ...
            // controlla registro
            rispostaControlli = controlliSemantici.checkTipoRegistro(tagCSChiave.getTipoRegistro(),
                    descChiaveUD, strutturaUpdVers.getIdStruttura());
            ...
            if (versamento.hasDocumentoPrincipaleToUpd()) {
                rispostaControlli = controlliSemantici.checkTipoDocumento(
                        ...
                        strutturaUpdVers.getIdStruttura(), true, "dummy doc");
                if (!rispostaControlli.isrBoolean()) {
                    // raccolgo esito NEGATIVO solo per errori "gravi"
                    if (rispostaControlli.getCodErr().contains(MessaggiWSBundle.ERR_666)) {
                        rispostaWs.setSeverity(SeverityEnum.ERROR);
            ...
            // forza collegamento
            strutturaUpdVers.setFlForzaCollegamento(((String) rispostaControlli.getrMap()
                    .get(RispostaControlli.ValuesOnrMap.FL_FORZA_COLL.name())).equals("1"));
    ...
    protected boolean controllaHashSIP(SyncFakeSessn sessione, UpdVersamentoExt versamento,
        ...
        RispostaControlli rispostaControlli = updVersamentoControlli.checkUltimoSIPWithHashBinary(
                descChiaveUD, versamento.getStrutturaUpdVers().getIdUd(),
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            strutturaUpdVers
                    .setEncodingHashIndiceSIP(CostantiDB.TipiEncBinari.HEX_BINARY.descrivi());
    ...
    protected boolean checkTipoUDAndAbilUpdMeta(CompRapportoUpdVers myEsito,
            UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs, String descChiaveUD) {
        FlControlliUpd flControlliUpd = null;
        boolean prosegui = true;
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        /*
         * TODO: prima del controllo per struttura, il sistema controlla che la unità doc
         * determinata in precedenza abbia tipo di unità doc pari al valore del tag
         * “TipologiaUnitaDocumentaria“ su SIP aggiornamento il tag in questione non è presente
         */
        if (!versamento.getVersamento().getUnitaDocumentaria().getIntestazione()
                .getTipologiaUnitaDocumentaria().equalsIgnoreCase(
            ...
            rispostaWs.setEsitoWsErrBundle(
                    ...
                    MessaggiWSBundle.UD_015_001, descChiaveUD,
                    versamento.getVersamento().getUnitaDocumentaria().getIntestazione()
            ...
            versamento.addEsitoControlloOnControlliUnitaDocumentariaBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_TIPO),
                    rispostaWs.getSeverity(), TipiEsitoErrore.NEGATIVO, MessaggiWSBundle.UD_015_001,
                    descChiaveUD, versamento.getVersamento().getUnitaDocumentaria()
            ...
            strutturaUpdVers
                    .setIdTipologiaUnitaDocumentaria(strutturaUpdVers.getIdTipoUDNonVerificata()); // da
            // controllaChiave
            ...
            if (!rispostaControlli.isrBoolean()) {
                ...
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(rispostaControlli.getCodErr(),
                        rispostaControlli.getDsErr());
            ...
            if (!flControlliUpd.isFlAbilitaUpdMeta()) {
                ...
                // aggiunta OK
                versamento.addControlloOkOnControlliUnitaDocumentaria(ControlliWSBundle
                        .getControllo(ControlliWSBundle.CTRL_UD_ABILAGGIORNAMENTO));
            }

            // aggiunta flag su riposta
            myEsito.getParametriAggiornamento()
                    .setAbilitaAggiornamento(flControlliUpd.isFlAbilitaUpdMeta());
            //
            myEsito.getParametriAggiornamento()
                    .setForzaAggiornamento(flControlliUpd.isFlForzaUpdMetaInark());
            //
            myEsito.getParametriAggiornamento()
    ...
    protected boolean controllaAbilUserRegTipoUD(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        // verifica abilitazione utente per registro/tipo UD
        RispostaControlli rispostaControlli = updVersamentoControlli
                .checkTipoUDRegIamUserOrganizzazione(strutturaUpdVers.getChiaveNonVerificata(),
                        strutturaUpdVers.getDescTipologiaUnitaDocumentaria(),
    ...
    protected boolean controllaStatoConservazione(UpdVersamentoExt versamento,
        ...
        boolean isWarn = flControlliUpd.isFlAccettaUpdMetaInark()
                && flControlliUpd.isFlForzaUpdMetaInark();
        //
        CostantiDB.StatoConservazioneUnitaDoc scud = strutturaUpdVers
        ...
        if (scud == CostantiDB.StatoConservazioneUnitaDoc.PRESA_IN_CARICO
            ...
            if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
                ...
                rispostaWs.setEsitoWsWarnBundle(
                        ControlliWSBundle
                                .getControllo(ControlliWSBundle.CTRL_UD_STATOCONSERVAZIONE),
            ...
            versamento.addEsitoControlloOnControlliUnitaDocumentariaBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_STATOCONSERVAZIONE),
                    SeverityEnum.WARNING, TipiEsitoErrore.WARNING, MessaggiWSBundle.UD_013_003,
                    descChiaveUD, scud);
    ...
    protected boolean controllaCollegamenti(CompRapportoUpdVers myEsito,
            UpdVersamentoExt versamento, RispostaWSUpdVers rispostaWs) {
        //
        return updCollegamentiPrsr.eseguiControlli(versamento, rispostaWs, myEsito);
    }

    protected boolean controllaProfiloNormativo(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        // recupera e controlla fascicolo princ
        RispostaControlli rispostaControlli = updVersamentoControlli.checkProfiloNormativo(
                versamento, TipiEntitaSacer.UNI_DOC,
                versamento.getStrutturaUpdVers().getIdTipologiaUnitaDocumentaria(),
                versamento.getStrutturaUpdVers().getUrnPartChiaveUd(),
                versamento.getVersamento().getUnitaDocumentaria().getIntestazione()
                        .getTipologiaUnitaDocumentaria(),
                versamento.getStrutturaUpdVers().getIdUd());

        if (!rispostaControlli.isrBoolean()) {
            rispostaWs.setSeverity(SeverityEnum.ERROR);
            // esito generale
            rispostaWs.setEsitoWsError(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_PROFILONORMATIVO),
                    rispostaControlli.getCodErr(), rispostaControlli.getDsErr());
    ...
    protected boolean controllaProfiloUnitaDocumentaria(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, String descChiaveUD) {
        ...
        if (strutturaUpdVers.getFlControlliUpd().isFlProfiloUdObbOggetto() && (versamento
                .getVersamento().getUnitaDocumentaria().getProfiloUnitaDocumentaria() == null
            ...
            versamento.addEsitoControlloOnControlliUnitaDocumentariaBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_PROFILOUD),
                    rispostaWs.getSeverity(), TipiEsitoErrore.NEGATIVO, MessaggiWSBundle.UD_014_001,
                    descChiaveUD);
        ...
        if (strutturaUpdVers.getFlControlliUpd().isFlProfiloUdObbData() && (versamento
                .getVersamento().getUnitaDocumentaria().getProfiloUnitaDocumentaria() == null
                || versamento.getVersamento().getUnitaDocumentaria().getProfiloUnitaDocumentaria()
                        .getData() == null)) {
            ...
            versamento.addEsitoControlloOnControlliUnitaDocumentariaBundle(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_UD_PROFILOUD),
                    rispostaWs.getSeverity(), TipiEsitoErrore.NEGATIVO, MessaggiWSBundle.UD_014_002,
                    descChiaveUD);
    ...
    protected boolean controllaDatiSpecifici(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, String descChiaveUD) {
        //
    ...
    protected boolean controllaDatiSpeificiMig(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs, String descChiaveUD) {
        ...
        if (StringUtils.isNotBlank(strutturaUpdVers.getTipoConservazione())
            ...
            // costruttore
            // verifico gli eventuali dati specifici e di migrazione
            RispostaControlliAttSpec tmpControlliAttSpec = gestioneDatiSpec.parseDatiSpecMig(
                    TipiEntitaSacer.UNI_DOC,
            ...
            if (tmpControlliAttSpec.isrBoolean()) {
                ...
                // aggiunto esito positivo
                versamento.addControlloOkOnControlliUnitaDocumentaria(ControlliWSBundle
                        .getControllo(ControlliWSBundle.CTRL_UD_DATISPECIFICIMIGRAZ));
            } else {
                // è un errore grave ...
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                // esito generale
    ...
    protected boolean controllaDocumenti(CompRapportoUpdVers myEsito, UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        return updDocumentiPrsr.eseguiControlli(versamento, rispostaWs, myEsito);
    }

    protected boolean controllaFascicoliSecondari(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        // recupera e controlla fascicolo princ
        RispostaControlli rispostaControlli = updVersamentoControlli
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            rispostaWs.setEsitoWsError(
                    ControlliWSBundle
                            .getControllo(ControlliWSBundle.CTRL_UD_FASCICOLISECONDARIVSPRINCIPALE),
                    rispostaControlli.getCodErr(), rispostaControlli.getDsErr());
    ...
    private boolean controllaCdKeyNormalized(UpdVersamentoExt versamento,
            RispostaWSUpdVers rispostaWs) {
        StrutturaUpdVers strutturaUpdVers = versamento.getStrutturaUpdVers();
        boolean prosegui = true;
        // recupero parametro DB per verifica calcolo URN normalizzato
        RispostaControlli rispostaControlli = controlliSemantici.getDtCalcInizioNuoviUrn();
        ...
        if (prosegui) {
            ...
            rispostaControlli = controlliSemantici.checkUniqueCdKeyNormalized(
                    strutturaUpdVers.getChiaveNonVerificata(), strutturaUpdVers.getIdRegistro(),
                    strutturaUpdVers.getIdUd(), strutturaUpdVers.getCdKeyNormalized(),
