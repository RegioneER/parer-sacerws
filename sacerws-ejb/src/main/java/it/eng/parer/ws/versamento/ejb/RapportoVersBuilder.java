...
public class RapportoVersBuilder {
    ...
    private RispostaControlli canonicalizzaDaSalvareIndiceSip(SyncFakeSessn sessione, boolean unPrettyPrint) {
        ...
        try {
            ...
            tmpControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.XSD_001_002, e.getMessage()));
        }
    ...
    //
    public RispostaControlli produciEsitoVecchioRapportoVers(VersamentoExt versamento, long idElemento,
        ...
        try {
            ...
            if (tmpRisp.getrLong() > 0) {
                ...
                if (versamento.getModificatoriWSCalc().contains(Costanti.ModificatoriWS.TAG_RAPPORTO_VERS_OUT)) {
                    esito.setRapportoVersamento(xml);
                } else {
                    esito.setXMLVersamento(xml);
                }
            } else {
                esito.setXMLVersamento("La sessione di versamento per l'UD da versare non è presente.");
            }

        } catch (Exception e) {
            tmpControlli.setCodErr("ERR");
            tmpControlli
                    .setDsErr("Errore nella generazione del Rapporto di Versamento UD duplicata: " + e.getMessage());
    ...
    public RispostaControlli produciEsitoVecchioRapportoVers(VersamentoExtAggAll versamento, long idElemento,
            String urnPartDoc, EsitoVersAggAllegati esito) {
        RispostaControlli tmpControlli = new RispostaControlli();
        ...
        try {
            ...
            if (tmpRisp.getrLong() > 0) {
                ...
                if (tmpRisp.getrString() != null) {
                    ...
                    String tmpIdDocumento = versamento.getStrutturaComponenti().getDocumentiAttesi().get(0)
                    ...
                    xml = (MessageFormat.format(
                            ...
                            tmpIdDocumento, versamento.getStrutturaComponenti().getUrnPartChiaveUd(),
                            MessaggiWSFormat.formattaUrnDocUniDoc(tmpUrn)));
                }
                if (versamento.getModificatoriWSCalc().contains(Costanti.ModificatoriWS.TAG_RAPPORTO_VERS_OUT)) {
                    esito.setRapportoVersamento(xml);
                ...
                esito.setXMLVersamento("La sessione di versamento per il Documento da versare non è presente.");
            }
        } catch (Exception e) {
            tmpControlli.setCodErr("ERR");
            tmpControlli.setDsErr(
                    "Errore nella generazione del Rapporto di Versamento Documento duplicato: " + e.getMessage());
            log.error("Errore nella generazione del Rapporto di Versamento Documento duplicato ", e);
        }
        return tmpControlli;
    }

    //
    public RispostaControlli produciEsitoNuovoRapportoVers(VersamentoExt versamento, SyncFakeSessn sessione,
            EsitoVersamento esito) {
        RispostaControlli tmpControlli = new RispostaControlli();
        ...
        try {
            ...
            aggiungiDocumentoSip(tmpDoc, esito.getUnitaDocumentaria().getDocumentoPrincipale(), versamento);
            //
            if (esito.getUnitaDocumentaria().getAllegati() != null) {
                for (ECDocumentoType tmpEsitoDoc : esito.getUnitaDocumentaria().getAllegati().getAllegato()) {
            ...
            if (esito.getUnitaDocumentaria().getAnnessi() != null) {
                for (ECDocumentoType tmpEsitoDoc : esito.getUnitaDocumentaria().getAnnessi().getAnnesso()) {
            ...
            if (esito.getUnitaDocumentaria().getAnnotazioni() != null) {
                for (ECDocumentoType tmpEsitoDoc : esito.getUnitaDocumentaria().getAnnotazioni().getAnnotazione()) {
            ...
            Marshaller tmpMarshaller = xmlVersCache.getVersRespCtxforRapportoVersamento().createMarshaller();
            ...
            tmpControlli.setCodErr("ERR");
            tmpControlli.setDsErr("Errore nella generazione del Rapporto di Versamento: " + e.getMessage());
            log.error("Errore nella generazione del Rapporto di Versamento ", e);
        }
        return tmpControlli;
    }

    public RispostaControlli produciEsitoNuovoRapportoVers(VersamentoExtAggAll versamento, SyncFakeSessn sessione,
            EsitoVersAggAllegati esito) {
        RispostaControlli tmpControlli = new RispostaControlli();
    ...
    private RapportoVersamento inizializzaRapportoVersamento(String tmpUrn, SyncFakeSessn sessione)
        ...
        // #versione V2
        sessione.setUrnIndiceSipXml(
                MessaggiWSFormat.formattaUrnIndiceSip(tmpUrn, Costanti.UrnFormatter.URN_INDICE_SIP_V2));
        sessione.setHashIndiceSipXml(
                hashCalculator.calculateHashSHAX(sessione.getDatiDaSalvareIndiceSip(), TipiHash.SHA_256).toHexBinary());
        sessione.setUrnEsitoVersamento(
        ...
        if (sessione.getDatiPackInfoSipXml() != null) {
            ...
            sessione.setHashPackInfoSipXml(
                    hashCalculator.calculateHashSHAX(sessione.getDatiPackInfoSipXml(), TipiHash.SHA_256).toHexBinary());
        }
        sessione.setUrnRapportoVersamento(
        ...
        rapportoVersamento
                .setDataRapportoVersamento(XmlDateUtility.dateToXMLGregorianCalendar(sessione.getTmChiusura()));
        //
    ...
    private void aggiungiDocumentoSip(DocumentoSIPType docDest, ECDocumentoType esitoDoc, IVersamentoExt versamento) {
        ...
        for (ECComponenteType tmpEsitoComp : esitoDoc.getComponenti().getComponente()) {
            ...
            if (tmpEsitoComp.getSottoComponenti() != null) {
                for (ECSottoComponenteType tmpEsitoSComp : tmpEsitoComp.getSottoComponenti().getSottoComponente()) {
                    ...
                    if (tmpEsitoSComp.getHash() != null && tmpEsitoSComp.getHash().length > 0) {
                        tmpSComponenteSIP.setHash(BinEncUtility.encodeUTF8HexString(tmpEsitoSComp.getHash()));
                    ...
                    if (versamento.getModificatoriWSCalc().contains(Costanti.ModificatoriWS.TAG_RAPPORTO_VERS_1_5)) {
                        ...
                        tmpComponenteSIP.setIDComponenteVersato(tmpEsitoSComp.getIDComponenteVersato());
                    }
                }
            }
        }
