...
@WebServlet(urlPatterns = {
	"/VersamentoSync" }, asyncSupported = true)
public class VersamentoSyncSrvlt extends HttpServlet {
    ...
    @EJB(mappedName = "java:app/sacerws-ejb/VersamentoSync")
    private VersamentoSync versamentoSync;
	...
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
		...
		try {
		    ...
		    // ---> i file sono caricati in locale!!
		    tmpAvanzamento
		    ...
		    /// ---------------> qui i file devono essere in staging
		    // la verifica firme deve essere asyncronous
		    ...
		    log.error("Eccezione FileUploadException nella servlet versamento sync "
			    ...
			    + "\n IP chiamante: " + sessioneFinta.getIpChiamante(), e1);
		} catch (Exception e1) {
		    rispostaWs.setSeverity(SeverityEnum.ERROR);
		    rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
			    "Eccezione generica nella servlet versamento sync " + e1.getMessage());
		    ...
		    SrvltHandlingException.handlingSocketErrors(
			    Response405.NomeWebServiceRest.VERSAMENTO_SYNC, e1, rispostaWs);
		...
		log.error(
			"Errore nella servlet versamento sync: la chiamata non è multipart/formdata ");
	    }
	}
