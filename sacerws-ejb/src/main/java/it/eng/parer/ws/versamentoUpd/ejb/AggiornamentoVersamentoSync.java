...
public class AggiornamentoVersamentoSync {
    ...
    public void init(RispostaWSUpdVers rispostaWs, AvanzamentoWs avanzamento, UpdVersamentoExt versamento) {
        ...
        myEsitoComposito.getEsitoGeneralePositivo().setCodiceEsito(CodiceEsitoType.POSITIVO);// TODO: ancora da
                                                                                             // definire a
                                                                                             // livello di
                                                                                             // controlli

        myEsitoComposito.setEsitoGeneraleNegativo(new EsitoGeneraleNegativoType());
        myEsitoComposito.getEsitoGeneraleNegativo().setCodiceEsito(CodiceEsitoType.NON_ATTIVATO);// TODO: ancora da
                                                                                                 // definire a
                                                                                                 // livello di
                                                                                                 // controlli
        ...
        for (ControlloWSResp controllo : ControlliWSBundle
                .getControlliByCdFamiglia(ControlloEseguito.FamigliaControllo.CONTROLLIGENERALI.name())) {
            versamento.addControlloNAOnGenerali(controllo);
        }

        for (ControlloWSResp controllo : ControlliWSBundle
                .getControlliByCdFamiglia(ControlloEseguito.FamigliaControllo.CONTROLLIXSD.name())) {
            versamento.addControlloNAOnGenerali(controllo);
        }

        for (ControlloWSResp controllo : ControlliWSBundle
                .getControlliByCdFamiglia(ControlloEseguito.FamigliaControllo.CONTROLLIINTESTAZIONE.name())) {
            versamento.addControlloNAOnGenerali(controllo);
        }

        for (ControlloWSResp controllo : ControlliWSBundle
                .getControlliByCdFamiglia(ControlloEseguito.FamigliaControllo.CONTROLLIUNITADOC.name())) {
    ...
    public void verificaVersione(String versione, RispostaWSUpdVers rispostaWs, UpdVersamentoExt versamento) {
        ...
        if (!tmpRispostaControlli.isrBoolean()) {
            ...
            rispostaWs.setEsitoWsError(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_ESISTEVERSIONEXSD),
            ...
            versamento.addEsitoControlloOnGenerali(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_ESISTEVERSIONEXSD),
                    rispostaWs.getSeverity(), TipiEsitoErrore.NEGATIVO, tmpRispostaControlli.getCodErr(),
    ...
    public void verificaCredenziali(String loginName, String password, String indirizzoIp, RispostaWSUpdVers rispostaWs,
        ...
        if (!tmpRispostaControlli.isrBoolean()) {
            rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
            // esito generale
            rispostaWs.setEsitoWsError(ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_UTENTE),
                    tmpRispostaControlli.getCodErr(), tmpRispostaControlli.getDsErr());

            // aggiunta su controlli generali
            versamento.addEsitoControlloOnGenerali(
                    ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_UTENTE), rispostaWs.getSeverity(),
                    TipiEsitoErrore.NEGATIVO, tmpRispostaControlli.getCodErr(), tmpRispostaControlli.getDsErr());
        ...
        if (tmpRispostaControlli.isrBoolean()) {
            ...
            // passato controllo versione (e ws)
            versamento.addControlloOkOnGenerali(ControlliWSBundle.getControllo(ControlliWSBundle.CTRL_GENERALI_UTENTE));
        }
     ...
     * UPD (sempre sulla base del comunicato + reperito da UD : in pratica sempre TUTTO dell'UD //TODO : anche la parte
     * sui documenti/annessi/etc.? ) 3. save or not su ERR / KO
     */

    public void salvaTutto(SyncFakeSessn sessione, RispostaWSUpdVers rispostaWs, UpdVersamentoExt versamento) {
        ...
        myEsito.setControlliFallitiUlteriori(versamento.produciControlliFallitiUlteriori(rispostaWs));
        // *************************************************************************************
        ...
        if (!versamento.isSimulaScrittura()
            ...
            if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR
                ...
                if (!salvataggioUpdVersamento.salvaAggiornamento(rispostaWs, versamento, sessione)) {
            ...
            if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.ERROR && !rispostaWs.isErroreDiSistema()) {
             ...
             */
            if ((rispostaWs.getStatoSessioneVersamento() == IRispostaWS.StatiSessioneVersEnum.ERRATA
                    || rispostaWs.getStatoSessioneVersamento() == IRispostaWS.StatiSessioneVersEnum.FALLITA)
                    && rispostaWs.isErroreDiSistema()) {
                tmpAvanzamentoWs.setCheckPoint(AvanzamentoWs.CheckPoints.SalvataggioSessioneWS).setFase("inizio")
                        .logAvanzamento();
    ...
    private void salvaSessioneErrataSysError(SyncFakeSessn sessione, RispostaWSUpdVers rispostaWs,
        ...
        versamento.addEsitoControlloOnSistema(rispostaWs.getControlloWs(), rispostaWs.getErrorCode(),
                rispostaWs.getErrorMessage());

        updLogSessione.registraSessioneErrata(rispostaWs, versamento, sessione);

        // Nota : se fallisce anche la persistenza su sessione errata verrà resituito l'errore al client
    }

    private void buildControlliDocs(UpdVersamentoExt versamento, CompRapportoUpdVers myEsito) {
        if (versamento.hasDocumentiCollegatiToUpd() && myEsito.getUnitaDocumentaria().getDocumentiCollegati() != null) {
            // per ogni documento collegato (normalizzato) .....
            for (DocumentoCollegato doccollegato : myEsito.getUnitaDocumentaria().getDocumentiCollegati()
        ...
        if (versamento.hasDocumentoPrincipaleToUpd()
            ...
            if (!versamento.getControlliDocPrincipale().isEmpty()) {
                ...
                myEsito.getUnitaDocumentaria().getDocumentoPrincipale()
                        .setControlliDocumento(versamento.produciControlliDocPrincipale(
                                myEsito.getUnitaDocumentaria().getDocumentoPrincipale().getIDDocumento()));
            }
            // componenti
            if (!versamento.getControlliComponentiDocPrincipale().isEmpty()) {
                int idx = 0;
                for (ComponenteType componente : myEsito.getUnitaDocumentaria().getDocumentoPrincipale()
                        .getStrutturaOriginale().getComponenti().getComponente()) {
                    // calcolo ctrl key
                    String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Principale.name(),
                            componente.getOrdinePresentazione(), idx);
                    // aggiunta su controlli componenti
                    componente.setControlliComponente(versamento.produciControlliComponenteDocPrincipale(keyCtrl));
                    idx++;
                }
            }
        }
        ...
        if (versamento.hasAllegatiToUpd() && myEsito.getUnitaDocumentaria().getAllegati() != null) {
            ...
            for (DocumentoType allegato : myEsito.getUnitaDocumentaria().getAllegati().getAllegato()) {
                if (!versamento.getControlliAllegati().isEmpty()) {
                    allegato.setControlliDocumento(versamento.produciControlliAllegato(allegato.getIDDocumento()));

                }
                // componenti su allegato
                if (!versamento.getControlliComponentiAllegati().isEmpty()) {
                    int idx = 0;
                    for (ComponenteType componente : allegato.getStrutturaOriginale().getComponenti().getComponente()) {
                        // calcolo ctrl key
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Allegato.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setControlliComponente(versamento.produciControlliComponenteAllegati(keyCtrl));
                        idx++;
                    }
                }
            }
        ...
        if (versamento.hasAnnessiToUpd() && myEsito.getUnitaDocumentaria().getAnnessi() != null) {
            ...
            for (DocumentoType annesso : myEsito.getUnitaDocumentaria().getAnnessi().getAnnesso()) {
                if (!versamento.getControlliAnnessi().isEmpty()) {
                    annesso.setControlliDocumento(versamento.produciControlliAnnesso(annesso.getIDDocumento()));
                }
                // componenti
                if (!versamento.getControlliComponentiAnnessi().isEmpty()) {
                    int idx = 0;
                    for (ComponenteType componente : annesso.getStrutturaOriginale().getComponenti().getComponente()) {
                        // calcolo ctrl key
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Annesso.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setControlliComponente(versamento.produciControlliComponenteAnnessi(keyCtrl));
                        idx++;
                    }
                }
        ...
        // Annotazioni
        if (versamento.hasAnnotazioniToUpd() && myEsito.getUnitaDocumentaria().getAnnotazioni() != null) {
            for (DocumentoType annotazione : myEsito.getUnitaDocumentaria().getAnnotazioni().getAnnotazione()) {
                ...
                if (!versamento.getControlliComponentiAnnotazioni().isEmpty()) {
                    int idx = 0;
                    for (ComponenteType componente : annotazione.getStrutturaOriginale().getComponenti()
                            .getComponente()) {
                        // calcolo ctrl key
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Annotazione.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setControlliComponente(versamento.produciControlliComponenteAnnotazioni(keyCtrl));
                        idx++;
                    }
                }
            }
    ...
    private void buildAggiornamentiDaEffettuare(UpdVersamentoExt versamento, CompRapportoUpdVers myEsito) {

        // *************************************************************************************
        // unità documentaria
        // *************************************************************************************
        myEsito.getUnitaDocumentaria().setAggiornamentiEffettuati(versamento.produciAggiornamentiUnitaDocumentaria());
        // *************************************************************************************
        // documenti
        // *************************************************************************************
        // DocPrincipale
        if (versamento.hasDocumentoPrincipaleToUpd()
            ...
            if (!versamento.getAggiornamentiDocPrincipale().isEmpty()) {
                // aggiunta su controlli doc principale
                myEsito.getUnitaDocumentaria().getDocumentoPrincipale()
                        .setAggiornamentiEffettuati(versamento.produciAggiornamentiDocPrincipale(
                                myEsito.getUnitaDocumentaria().getDocumentoPrincipale().getIDDocumento()));
            }
            // componenti
            if (!versamento.getAggiornamentiCompDocPrincipale().isEmpty()) {
                ...
                for (ComponenteType componente : myEsito.getUnitaDocumentaria().getDocumentoPrincipale()
                    ...
                    String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Principale.name(),
                            componente.getOrdinePresentazione(), idx);
                    // aggiunta su controlli componenti
                    componente.setAggiornamentiEffettuati(versamento.produciAggiornamentiCompDocPrincipale(keyCtrl));
                    idx++;
                }
            }
        }
                ...
                if (!versamento.getAggiornamentiCompAllegati().isEmpty()) {
                    ...
                    for (ComponenteType componente : allegato.getStrutturaOriginale().getComponenti().getComponente()) {
                        ...
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Allegato.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setAggiornamentiEffettuati(versamento.produciAggiornamentiCompAllegato(keyCtrl));
                        idx++;
                    }
                }
            }
                ...
                if (!versamento.getAggiornamentiCompAnnessi().isEmpty()) {
                    ...
                    for (ComponenteType componente : annesso.getStrutturaOriginale().getComponenti().getComponente()) {
                        ...
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Annesso.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setAggiornamentiEffettuati(versamento.produciAggiornamentiCompAnnesso(keyCtrl));
                        idx++;
                    }
                }
        ...
        if (versamento.hasAnnotazioniToUpd() && myEsito.getUnitaDocumentaria().getAnnotazioni() != null) {
            for (DocumentoType annotazione : myEsito.getUnitaDocumentaria().getAnnotazioni().getAnnotazione()) {
                if (!versamento.getAggiornamentiAnnotazioni().isEmpty()) {
                    // aggiunta su controlli annotazioni
                    annotazione.setAggiornamentiEffettuati(
                            versamento.produciAggiornamentiAnnotazione(annotazione.getIDDocumento()));
                }
                // componenti
                if (!versamento.getAggiornamentiCompAnnotazioni().isEmpty()) {
                    ...
                    for (ComponenteType componente : annotazione.getStrutturaOriginale().getComponenti()
                        ...
                        String keyCtrl = UpdDocumentiUtils.buildComponenteKeyCtrl(CategoriaDocumento.Annotazione.name(),
                                componente.getOrdinePresentazione(), idx);
                        // aggiunta su controlli componenti
                        componente.setAggiornamentiEffettuati(versamento.produciAggiornamentiCompAnnotazione(keyCtrl));
                        idx++;
                    }
                }
            }
