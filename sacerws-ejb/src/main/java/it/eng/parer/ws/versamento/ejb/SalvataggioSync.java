...
public class SalvataggioSync {
	...
	try {
	    ...
	    if (!this.verificaCreaDirectoryFS(tmpRispostaControlli,
	    ...
	    if (prosegui && !this.salvaUnitaDoc(versamento.getStrutturaComponenti(),
		    versamento.getVersamento(), rispostaWS, nomeAgente)) {
		prosegui = false;
	    }

	    // SALVO UNITA' DOCUMENTARIA DA INSERIRE NELL'ELENCO DI VERSAMENTO
	    if (prosegui && !this.salvaUdElencoVers(versamento.getStrutturaComponenti(),
		    versamento.getVersamento())) {
		rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
	    ...
	    if (prosegui && !this.salvaFascicoliSecondari(versamento.getStrutturaComponenti(),
		    versamento.getVersamento())) {
		rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
	    ...
	    if (prosegui && !this.salvaDocumentiCollegati(versamento.getStrutturaComponenti(),
		    versamento.getVersamento())) {
		rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
		...
		tmpConfigPerDoc.setForzaAccettazione(booleanToFlag(
			rispostaWS.getIstanzaEsito().getConfigurazione().isForzaAccettazione()));
		tmpConfigPerDoc.setForzaConservazione(booleanToFlag(
			versamento.getStrutturaComponenti().isConfigFlagForzaConservazione()));
		tmpConfigPerDoc.setTipoConservazione(
			rispostaWS.getIstanzaEsito().getConfigurazione().getTipoConservazione());
	    ...
	    if (prosegui && !this.salvaComponenti(tmpRispostaControlli,
		    versamento.getStrutturaComponenti(),
		...
		rispostaWS.setEsitoWsErrBundle(
			...
			"Eccezione nella persistenza dei componenti "
				+ tmpRispostaControlli.getDsErr());
		prosegui = false;
	    }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public boolean salvaDatiVersamento(VersamentoExtAggAll versamento,
	...
	try {
		...
		String userid = versamento.getVersamento().getIntestazione().getVersatore()
			.getUserID();
		String utente = versamento.getVersamento().getIntestazione().getVersatore()
			.getUtente();
		String nomeAgente = "userid: " + userid + "; utente: " + utente;
		boolean result = this.aggiornaUnitaDoc(versamento.getStrutturaComponenti(),
	    ...
	    if (prosegui) {
		boolean result = this
			.aggiornaPregressoDocumenti(versamento.getStrutturaComponenti());
		if (!result) {
		    rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		    rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
	    ...
	    if (prosegui && !this.salvaDocElencoVers(versamento.getStrutturaComponenti(),
		    versamento.getVersamento())) {
		rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
    ...
    /*
     * ************************************************************************** salvataggio dati e
     * metadati per versamento completo
     * **************************************************************************
     */
    private boolean salvaUnitaDoc(StrutturaVersamento strutV, UnitaDocumentaria ud,
	    RispostaWS rispostaWS, String nomeAgente) {
	...
	tmpTabUD.setDecRegistroUnitaDoc(
	...
	tmpTabUD.setDecTipoUnitaDoc(entityManager.find(DecTipoUnitaDoc.class,
		strutV.getIdTipologiaUnitaDocumentaria()));
	...
	Integer numeroAnnotazioni = ud.getNumeroAnnotazioni() != null ? ud.getNumeroAnnotazioni()
		: 0;
	...
	// aggiunti da Paolo
	tmpTabUD.setFlUnitaDocFirmato(booleanToFlag(
		rispostaWS.getIstanzaEsito().getUnitaDocumentaria().isFirmatoDigitalmente()));
	...
	// configurazione
	tmpTabUD.setFlForzaAccettazione(booleanToFlag(
		rispostaWS.getIstanzaEsito().getConfigurazione().isForzaAccettazione()));
	tmpTabUD.setFlForzaConservazione(booleanToFlag(strutV.isConfigFlagForzaConservazione()));
	tmpTabUD.setTiConservazione(
		rispostaWS.getIstanzaEsito().getConfigurazione().getTipoConservazione());
	tmpTabUD.setNmSistemaMigraz(
		rispostaWS.getIstanzaEsito().getConfigurazione().getSistemaDiMigrazione() != null
			? rispostaWS.getIstanzaEsito().getConfigurazione().getSistemaDiMigrazione()
			: "");

	tmpTabUD.setFlForzaCollegamento(booleanToFlag(
		rispostaWS.getIstanzaEsito().getConfigurazione().isForzaCollegamento()));
	...
	if (ud.getProfiloArchivistico() != null) {
	    tmpTabUD.setDsClassifPrinc(
		    ud.getProfiloArchivistico().getFascicoloPrincipale().getClassifica());
	    if (ud.getProfiloArchivistico().getFascicoloPrincipale().getFascicolo() != null) {
		tmpTabUD.setCdFascicPrinc(ud.getProfiloArchivistico().getFascicoloPrincipale()
			.getFascicolo().getIdentificativo());
		String value = JAXBUtils.getStringValFromJAXBElement(ud.getProfiloArchivistico()
		...
		tmpTabUD.setCdSottofascicPrinc(ud.getProfiloArchivistico().getFascicoloPrincipale()
	...
	if (strutV.getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE
		&& strutV.isTpiAbilitato()) {
	    tmpTabUD.setTiStatoUdElencoVers(
		    TipiStatoElementoVersato.IN_ATTESA_MEMORIZZAZIONE.name());
	} else {
	...
	try {
	    ...
	    entityManager.flush();
	    /*
	     * memorizza l'id dell'UD appena creata, mi servirà come FK nei salvataggi delle altre
	     * tabelle. Nota che nel caso dell'aggiunta documento, questa informazione è caricata
	     * durante i controlli.
	     */
	...
	query.setParameter("idAaRegistroUnitaDocIn",
		strutV.getConfigRegAnno().getIdAaRegistroUnitaDoc());
	query.setParameter("aaRegistroUnitaDocIn",
	...
	if (numWarningAggiornati == 0) {
	    ...
	    tmpDecWarnAaRegistroUd.setAaRegistroUnitaDoc(
		    BigDecimal.valueOf(ud.getIntestazione().getChiave().getAnno()));
	    tmpDecWarnAaRegistroUd
		    .setDecAaRegistroUnitaDoc(entityManager.find(DecAaRegistroUnitaDoc.class,
			    strutV.getConfigRegAnno().getIdAaRegistroUnitaDoc()));
	...
	/*
	 * questa tabella viene impiegata per la verifica di contiguità nella conservazione di
	 * documenti fiscali. Dal momento che il database è partizionato per anno è necessario
	 * effetture le ricerche sulla tabella AroUnitaDoc sapendo già su che anni operare per
	 * evitare crolli di performance. --- prima conto se il record nella tabella registri è già
	 * presente. In questo modo evito di usare un lock esclusivo se non è necessario
	 */
	Long numAaUdReg;
	...
	if (numAaUdReg == 0) {
	    ...
	    try {
		tmpRegistroUnitaDoc = entityManager.find(DecRegistroUnitaDoc.class,
			strutV.getIdRegistroUnitaDoc(), LockModeType.PESSIMISTIC_WRITE, properties);
	    } catch (Exception re) {
		/// logga l'errore e blocca tutto
		log.error(ERROR_PERSISTENZA_UD, re);
		tmpReturn = false;
	    ...
	    if (tmpReturn) {
		/*
		 * rieseguo il conteggio per sicurezza (potrei avere un errore di chiave duplicata
		 * in inserimento in alcune sfigatissime condizioni di concorrenza)
		 */
		...
		if (numAaUdReg == 0) {
		    ...
		    tmpAaUnitaDocRegistro.setAaUnitaDocRegistro(
			    BigDecimal.valueOf(ud.getIntestazione().getChiave().getAnno()));
		    tmpAaUnitaDocRegistro.setDecRegistroUnitaDoc(entityManager
			    .find(DecRegistroUnitaDoc.class, strutV.getIdRegistroUnitaDoc()));

		    /*
		     * inserisco il registro. Sono sicuro che non è presente in tabella e che
		     * nessuno sta eseguendo la stessa operazione in contemporanea
		     */
		    try {
		...
		/*
		 * rilascio il lock. Da notare che questo in apparenza non funziona ed il lock viene
		 * rilasciato solo dopo il termine della transazione
		 */
    ...
    private boolean riparaCollegamentiUdNonRisolti(StrutturaVersamento strutV,
	...
	javax.persistence.Query query = entityManager.createQuery(queryStr);
	query.setParameter("aroUnitaDocLinkIn",
	...
	query.setParameter("cdRegistroKeyUnitaDocLinkIn",
		ud.getIntestazione().getChiave().getTipoRegistro());
	query.setParameter("aaKeyUnitaDocLinkIn",
		BigDecimal.valueOf(ud.getIntestazione().getChiave().getAnno()));
	...
	/*
	 * rimuovo l'eventuale record della tabella delle UD non versate per errore che corrisponde
	 * a quella che si sta versando (la cerco per la tupla che definisce la chiave)
	 */
	long numUdNonVersRimosse;
	String queryStr = "delete from VrsUnitaDocNonVer al "
	...
	query.setParameter("cdRegistroKeyUnitaDocIn",
		ud.getIntestazione().getChiave().getTipoRegistro());
	query.setParameter("aaKeyUnitaDocIn",
		BigDecimal.valueOf(ud.getIntestazione().getChiave().getAnno()));
	...
	if (ud.getProfiloArchivistico() != null
		&& ud.getProfiloArchivistico().getFascicoliSecondari() != null) {
		...
		if (tmpFs.getFascicolo() != null
			&& tmpFs.getFascicolo().getIdentificativo() != null) {
		...
		if (tmpFs.getSottoFascicolo() != null
			&& tmpFs.getSottoFascicolo().getIdentificativo() != null) {
		    tmpArr[3] = tmpFs.getSottoFascicolo().getIdentificativo();
		}
		if (tmpFs.getSottoFascicolo() != null
			&& tmpFs.getSottoFascicolo().getOggetto() != null
	...
	for (CamiciaFascicoloType tmpFascicolo : tmpFascicoliUnivoci.values()) {
		...
		String value = JAXBUtils
			.getStringValFromJAXBElement(tmpFascicolo.getFascicolo().getOggetto());
		...
		String value = JAXBUtils
			.getStringValFromJAXBElement(tmpFascicolo.getSottoFascicolo().getOggetto());
	...
	if (ud.getDocumentiCollegati() != null) {
		...
		tmpTabLUD.setAaKeyUnitaDocLink(
			BigDecimal.valueOf(tmpUnitaDocColl.getChiave().getAnno()));
		...
		if (tmpUnitaDocColl.getIdUnitaDocLink() != -1) {
		    tmpTabLUD.setAroUnitaDocLink(entityManager.find(AroUnitaDoc.class,
			    tmpUnitaDocColl.getIdUnitaDocLink()));
		}
	...
	if (tmpReturn) {
	    tmpReturn = this.salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec.MIGRAZ,
		    TipiEntitaSacer.UNI_DOC, strutturaVersIn.getIdUnitaDoc(),
		    strutturaVersIn.getIdStruttura(),
		    strutturaVersIn.getIdRecXsdDatiSpecMigrazione(),
		    strutturaVersIn.getDatiSpecificiMigrazione(), strutturaVersIn.getIdUnitaDoc());
	}
    ...
    /*
     * ************************************************************************** salvataggio dati e
     * metadati per aggiunta documenti
     * **************************************************************************
     */
	...
	try {
	    ...
	    //
	    tmpTabUD = entityManager.find(AroUnitaDoc.class, strutV.getIdUnitaDoc(),
	...
	if (tmpReturn) {
	    ...
	    if (strutV.getDocumentiAttesi().get(0)
		    .getCategoriaDoc() == CategoriaDocumento.Allegato) {
		tmpTabUD.setNiAlleg(tmpTabUD.getNiAlleg().add(BigDecimal.valueOf(1)));
	    }
	    if (strutV.getDocumentiAttesi().get(0)
		    .getCategoriaDoc() == CategoriaDocumento.Annesso) {
		tmpTabUD.setNiAnnessi(tmpTabUD.getNiAnnessi().add(BigDecimal.valueOf(1)));
	    }
	    if (strutV.getDocumentiAttesi().get(0)
		    .getCategoriaDoc() == CategoriaDocumento.Annotazione) {
		...
		/*
		 * se l'unità doc presente non ha un risultato di verifca firme, aggiorno con quello
		 * appena calcolato
		 */
		if (tmpTabUD.getTiEsitoVerifFirme() == null
			|| tmpTabUD.getTiEsitoVerifFirme().equals("")) {
		    tmpTabUD.setTiEsitoVerifFirme(strutV.getTiEsitoVerifFirme());
		    tmpTabUD.setDsMsgEsitoVerifFirme(strutV.getDsMsgEsitoVerifica());
		}

		/*
		 * se il doc appena aggiunto è in errore, aggiorno stato e messaggio nell'unità doc.
		 * già presente in ogni caso con il valore NEGATIVO.
		 *
		 */
		...
		/*
		 * se il doc appena aggiunto è in warning, mentre l'unità doc già presente è
		 * POSITIVO, aggiorna l'unità doc a WARNING
		 */
		if (strutV.getTiEsitoVerifFirme().equals(SacerIndication.WARNING.name()) && tmpTabUD
			.getTiEsitoVerifFirme().equals(SacerIndication.POSITIVO.name())) {
		    tmpTabUD.setTiEsitoVerifFirme(strutV.getTiEsitoVerifFirme());
		    tmpTabUD.setDsMsgEsitoVerifFirme(strutV.getDsMsgEsitoVerifica());
		}
		/*
		 * in tutti gli altri casi, non aggiorno l'unità doc, dal momento cha ha già un
		 * risultato di verifica firme uguale o peggiore di quello che si è appena calcolato
		 */
	    }
	...
	try {
		...
		// per ogni componente
		for (Iterator<AroStrutDoc> it = tmpAroDoc.getAroStrutDocs().iterator(); it
			.hasNext();) {
    ...
    public boolean aggiornaDataCreazione(StrutturaVersamento strutV, ZonedDateTime tmChiusura,
	...
	try {
		...
		AroDoc tmpTabDoc = entityManager.find(AroDoc.class,
			valueDocVers.getIdRecDocumentoDB());
	    ...
	    rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
	    rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
		    "Eccezione nella persistenza della data di creazione versamento UD: "
			    + e.getMessage());
	...
	try {
	    Date dataChiusura = DateUtilsConverter.convert(tmChiusura);
	    for (DocumentoVers valueDocVers : strutV.getDocumentiAttesi()) {
		ElvDocAggDaElabElenco tmpElv = retrieveElvDocAggDaElabElenco(
			valueDocVers.getIdRecDocumentoDB());
		if (tmpElv != null) {
		    tmpElv.setDtCreazione(dataChiusura);
		}
	    ...
	    rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
	    rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
		    "Eccezione nella persistenza della data di creazione aggiunta documento: "
			    + e.getMessage());
	...
	Query q = entityManager.createQuery(
		"SELECT e FROM ElvUdVersDaElabElenco e WHERE e.aroUnitaDoc.idUnitaDoc = :idUnitaDoc");
	...
	Query q = entityManager
		.createQuery("SELECT e FROM ElvDocAggDaElabElenco e WHERE e.aroDoc.idDoc = :idDoc");
	...
	if (listaDoc != null) {
		...
		public int compare(AroDoc doc1, AroDoc doc2) {
		    int comparazionePerData = doc1.getDtCreazione()
			    .compareTo(doc2.getDtCreazione());
    ...
    private void salvaURNComponente(StrutturaVersamento strutV, AroDoc tmpAroDoc,
	...
	for (TiUrn tmpTiUrn : tiUrnToCalculate) {
	    ...
	    long count = new ArrayList<AroCompUrnCalc>(tmpAroCompDoc.getAroAroCompUrnCalcs())
		    .stream().filter(c -> c.getTiUrn().equals(tmpTiUrn)).count();
	    // se non esiste
	    if (count == 0) {
		// DOCXXXXXX
		String tmpUrnPartDoc = MessaggiWSFormat.formattaUrnPartDocumento(
			CategoriaDocumento.Documento, tmpAroDoc.getNiOrdDoc().intValue(), true,
	...
	if (strutV.getDtVersMax().before(strutV.getDtInizioCalcoloNewUrn())
	    ...
	    try {
		AroUnitaDoc tmpTabUD = entityManager.find(AroUnitaDoc.class,
	...
	for (VrsVLisXmlUdUrnDaCalc vrs : vrsVLisXmlUdUrnDaCalcs) {
	    ...
	    VrsXmlDatiSessioneVers xmlDatiSessioneVers = entityManager
	    ...
	    String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnUnitaDoc(
		    strutV.getUrnPartVersatoreNormalized(), strutV.getUrnPartChiaveUdNormalized());
	...
	for (AroDoc aroDoc : tmpAroUnitaDoc.getAroDocs()) {
		...
		//
		VrsXmlDatiSessioneVers xmlDatiSessioneVers = entityManager.find(
			VrsXmlDatiSessioneVers.class, vrs.getIdXmlDatiSessioneVers().longValue());

		// calcolo parte urn ORIGINALE
		// DOCXXXXXX
		String tmpUrnPartDoc = MessaggiWSFormat.formattaUrnPartDocumento(
			CategoriaDocumento.Documento, aroDoc.getNiOrdDoc().intValue(), true,
			Costanti.UrnFormatter.DOC_FMT_STRING_V2,
			Costanti.UrnFormatter.PAD5DIGITS_FMT);

		String tmpUrn = MessaggiWSFormat.formattaBaseUrnDoc(svf.getUrnPartVersatore(),
			svf.getUrnPartChiaveUd(), tmpUrnPartDoc,
			Costanti.UrnFormatter.URN_DOC_FMT_STRING_V2);

		// calcolo urn NORMALIZZATO
		String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnDoc(
			svf.getUrnPartVersatoreNormalized(), svf.getUrnPartChiaveUdNormalized(),
	...
	//
	List<AroUpdUnitaDoc> aroUpdUnitaDocs = this
		.retrieveAroUpdUnitaDocByUd(tmpAroUnitaDoc.getIdUnitaDoc());
	...
	for (AroUpdUnitaDoc updUnitaDoc : aroUpdUnitaDocs) {
		...
		String tmpUrn = MessaggiWSFormat.formattaBaseUrnUpdUnitaDoc(
			svf.getUrnPartVersatore(), svf.getUrnPartChiaveUd(),
	...
	tmpTab.setAroDoc(entityManager.find(AroDoc.class,
		strutV.getDocumentiAttesi().get(0).getIdRecDocumentoDB()));
	tmpTab.setAaKeyUnitaDoc(BigDecimal.valueOf(ud.getIntestazione().getChiave().getAnno()));
	tmpTab.setDtCreazione(convert(strutV.getDataVersamento()));
	tmpTab.setIdStrut(BigDecimal.valueOf(strutV.getIdStruttura()));
	// nel caso il salvataggio sia su filesystem/Tivoli,
    ...
    /*
     * ************************************************************************** parte comune +
     * TIVOLI - verifica percorsi nel caso la persistenza sia su filesystem
     * **************************************************************************
     */
    private boolean verificaCreaDirectoryFS(RispostaControlli risposta,
	...
	if (strutV.getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE
		...
		if (tmpControlli.isrBoolean() && statoCreaCartelle.isCreaCartellaData()) {
		    tmpControlli = salvataggioCompFS.generaDirDataTPIBlock(strutV,
			    statoCreaCartelle);
		}
		...
		if (tmpControlli.isrBoolean() && statoCreaCartelle.isCreaCartellaData()) {
		    tmpControlli = salvataggioCompFS.verificaDirTPIDataViaDb(strutV,
			    statoCreaCartelle, SalvataggioCompFS.MAX_NUMERO_TENTATIVI_TEST);
		}

		// se continua a mancare la riga, allora termino in errore
		if (tmpControlli.isrBoolean() && statoCreaCartelle.isCreaCartellaData()) {
		    tmpControlli.setrBoolean(false);
		    tmpControlli.setDsErr("Il record di VrsDtVers relativo alla data "
			    + format(strutV.getDataVersamento())
		...
		if (tmpControlli.isrBoolean()) {
		    tmpControlli = salvataggioCompFS
			    .verificaDirSuFs(statoCreaCartelle.getPathCartellaData());
		}

		if (tmpControlli.isrBoolean()) {
		    // verifica se devo creare la cartella data, via database
		    tmpControlli = salvataggioCompFS.verificaDirTPIVersatoreViaDb(strutV,
			    statoCreaCartelle, 1);
		} else {
			...
			tmpControlli.setDsErr(
				"La directory corrispondentre alla data non è presente sul file system "
		...
		if (tmpControlli.isrBoolean() && statoCreaCartelle.isCreaCartellaVersatore()) {
		    tmpControlli = salvataggioCompFS.generaDirVersatoreTPIBlock(strutV,
			    statoCreaCartelle);
		}
		...
		if (tmpControlli.isrBoolean() && statoCreaCartelle.isCreaCartellaVersatore()) {
		    tmpControlli.setrBoolean(false);
		    tmpControlli.setDsErr("Il record di VrsPathDtVers relativo alla data "
			    + strutV.getSubPathVersatoreArk()
			    + "non risulta in database ma non è scrivibile. Impossibile proseguire");
		}
		...
		if (tmpControlli.isrBoolean()) {
		    tmpControlli = salvataggioCompFS
			    .verificaDirSuFs(statoCreaCartelle.getPathCartellaVersatore());
		}
		...
		if (tmpControlli.isrBoolean()) {
		    ...
		    if (tmpControlli.getDsErr() == null || tmpControlli.getDsErr().isEmpty()) {
			tmpControlli.setDsErr(
				"La directory corrispondentre al versatore non è presente sul file system "
					+ "mentre risulta registrata nel database");
		    }
		}
		...
		/// logga l'errore e blocca tutto
		log.error(
			"Eccezione nella verifica/creazione directory per salvataggio su filesystem ",
			re);
     ...
     * **************************************************************************
     */
    private boolean salvaDocumenti(StrutturaVersamento strutV, ConfigPerDoc configPerDoc,
	    ChiaveType chiaveDoc, IRispostaVersWS rispostaWS) {
	boolean tmpReturn = true;
	Calendar tmpCal;
	...
	for (DocumentoVers valueDocVers : strutV.getDocumentiAttesi()) {
		...
		if (strutV.getTipoSalvataggioFile() == CostantiDB.TipoSalvataggioFile.FILE
			&& strutV.isTpiAbilitato()) {
		    tmpTabD.setTiStatoDocElencoVers(
			    TipiStatoElementoVersato.IN_ATTESA_MEMORIZZAZIONE.name());
		} else {
		    tmpTabD.setTiStatoDocElencoVers(
			    TipiStatoElementoVersato.IN_ATTESA_SCHED.name());
		}
	    }
	    ...
	    tmpTabD.setNmSistemaMigraz(configPerDoc.getSistemaDiMigrazione() != null
		    ? configPerDoc.getSistemaDiMigrazione()
		    : "");
		...
		tmpTabD.setDlDoc(
			valueDocVers.getRifDocumento().getProfiloDocumento().getDescrizione());
		tmpTabD.setDsAutoreDoc(
			valueDocVers.getRifDocumento().getProfiloDocumento().getAutore());
	    }
	    ...
	    if (!tmpReturn) {
		rispostaWS.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
			"Eccezione nella persistenza del documento ");
		rispostaWS.setSeverity(IRispostaWS.SeverityEnum.ERROR);
		break;
	    }
	}
    ...
    private boolean salvaDatiAccessoriDocumenti(DocumentoVers documentoVersIn,
	    StrutturaVersamento strutV) {
	AroStrutDoc tmpTabSD = null;

	boolean tmpReturn = this.salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec.VERS,
		TipiEntitaSacer.DOC, documentoVersIn.getIdRecDocumentoDB(), strutV.getIdStruttura(),
		documentoVersIn.getIdRecXsdDatiSpec(), documentoVersIn.getDatiSpecifici(),
		strutV.getIdUnitaDoc());

	if (tmpReturn) {
	    tmpReturn = this.salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec.MIGRAZ,
		    TipiEntitaSacer.DOC, documentoVersIn.getIdRecDocumentoDB(),
		    strutV.getIdStruttura(), documentoVersIn.getIdRecXsdDatiSpecMigrazione(),
		    documentoVersIn.getDatiSpecificiMigrazione(), strutV.getIdUnitaDoc());
	}
    ...
    private boolean rimuoviSessVersDocErrate(DocumentoVers documentoVersIn,
	    StrutturaVersamento strutV, ChiaveType chiaveUd) {
	boolean tmpReturn = true;
	long numUdNonVersRimosse;
    ...
    private boolean salvaComponenti(RispostaControlli risposta, StrutturaVersamento strutV,
	    ChiaveType chiave, String nomeWs) {
	boolean tmpReturn = true;
	...
	for (DocumentoVers valueDocVers : strutV.getDocumentiAttesi()) {
	    ...
	    tmpAroStrutDoc = entityManager.find(AroStrutDoc.class,
		    valueDocVers.getIdRecStrutturaDB());
		...
		if (tmpCV.getMySottoComponente() == null) {
		    ...
		    tmpTabCDComponente.setNiOrdCompDoc(
			    BigDecimal.valueOf(tmpCV.getMyComponente().getOrdinePresentazione()));
		    tmpTabCDComponente.setTiSupportoComp(
			    tmpCV.getMyComponente().getTipoSupportoComponente().toString());
		    tmpTabCDComponente.setDecTipoCompDoc(
			    entityManager.find(DecTipoCompDoc.class, tmpCV.getIdTipoComponente()));
		    tmpTabCDComponente
			    .setDsNomeCompVers(tmpCV.getMyComponente().getNomeComponente());
		    tmpTabCDComponente.setDlUrnCompVers(tmpCV.getMyComponente().getUrnVersato());
		    tmpTabCDComponente
			    .setDsIdCompVers(tmpCV.getMyComponente().getIDComponenteVersato());
		    tmpTabCDComponente.setDecTipoRapprComp(entityManager.find(
			    DecTipoRapprComp.class, tmpCV.getIdTipoRappresentazioneComponente()));

		    switch (tmpCV.getTipoSupporto()) {
		    case FILE:
			tmpTabCDComponente.setNiSizeFileCalc(
				BigDecimal.valueOf(tmpCV.getRifFileBinario().getDimensione()));
			tmpTabCDComponente.setDecFormatoFileDoc(entityManager
				.find(DecFormatoFileDoc.class, tmpCV.getIdFormatoFileVers()));
			break;
		    case METADATI:
			// nulla da fare
			break;
		    case RIFERIMENTO:
			tmpTabCDComponente.setAroUnitaDoc(
				entityManager.find(AroUnitaDoc.class, tmpCV.getIdUnitaDocRif()));
			break;
		    default:
		    }
		    ...
		    tmpTabCDComponente.setDsRifTempVers(
			    tmpCV.getMyComponente().getDescrizioneRiferimentoTemporale());
		    ...
		    //
		    tmpTabCDComponente
			    .setTiEsitoContrFormatoFile(mock.getTiEsitoContrFormatoFile());
		    tmpTabCDComponente.setTiEsitoVerifFirme(mock.getTiEsitoVerifFirme());
		    tmpTabCDComponente
			    .setTiEsitoVerifFirmeDtVers(mock.getTiEsitoVerifFirmeDtVers());
		    //
		    ...
		    tmpTabCDComponente
			    .setDsEsitoVerifFirmeDtVers(mock.getDsEsitoVerifFirmeDtVers());
		    tmpTabCDComponente.setDsFormatoRapprCalc(mock.getDsFormatoRapprCalc());
		    tmpTabCDComponente
			    .setDsFormatoRapprEstesoCalc(mock.getDsFormatoRapprEstesoCalc());
		    //
			...
			tmpTabCDComponente.setDecFormatoFileStandard(
				entityManager.find(DecFormatoFileStandard.class,
		    ...
		    String tmpUrn = MessaggiWSFormat.formattaBaseUrnDoc(
			    strutV.getUrnPartVersatore(), strutV.getUrnPartChiaveUd(),
			    tmpCV.getUrnPartComponenteNiOrdDoc(),
			    Costanti.UrnFormatter.URN_COMP_FMT_STRING);
			...
			tmpReturn = salvataggioFirmaManager.salvaBustaCrittografica(risposta,
				tmpCV.getId(), tmpCV.getVfWrapper(), tmpTabCDComponente, null);
			// se ci sono problemi nel salvataggio, blocco tutto
			if (!tmpReturn) {
			    return tmpReturn;
			}
			// per il componente "dichiarato" come firmato salvo i dati relativi al
			// report
			// della firma
		    ...
		    tmpTabCDSottoComp.setNiOrdCompDoc(BigDecimal
			    .valueOf(tmpCV.getMySottoComponente().getOrdinePresentazione()));
		    tmpTabCDSottoComp.setTiSupportoComp(
			    tmpCV.getMySottoComponente().getTipoSupportoComponente().toString());
		    tmpTabCDSottoComp.setDecTipoCompDoc(
			    entityManager.find(DecTipoCompDoc.class, tmpCV.getIdTipoComponente()));
		    tmpTabCDSottoComp
			    .setDsNomeCompVers(tmpCV.getMySottoComponente().getNomeComponente());
		    tmpTabCDSottoComp
			    .setDlUrnCompVers(tmpCV.getMySottoComponente().getUrnVersato());
		    tmpTabCDSottoComp
			    .setDsIdCompVers(tmpCV.getMySottoComponente().getIDComponenteVersato());

		    switch (tmpCV.getTipoSupporto()) {
		    case FILE:
			tmpTabCDSottoComp.setNiSizeFileCalc(
				BigDecimal.valueOf(tmpCV.getRifFileBinario().getDimensione()));
			tmpTabCDSottoComp.setDecFormatoFileDoc(entityManager
				.find(DecFormatoFileDoc.class, tmpCV.getIdFormatoFileVers()));
			break;
		    case METADATI:
			// nulla da fare
			break;
		    case RIFERIMENTO:
			tmpTabCDSottoComp.setAroUnitaDoc(
				entityManager.find(AroUnitaDoc.class, tmpCV.getIdUnitaDocRif()));
			break;
		    default:
		    }
		    ...
		    // salvo l'ordine di presentazione nella struttura documento, per recuperarlo in
		    // seguito
		    tmpCV.setOrdinePresentazione(
			    tmpCV.getMySottoComponente().getOrdinePresentazione());
		    ...
		    // TODO : da verificare
		    if (mock.getIdDecFormatoFileStandard() != null) {
			tmpTabCDSottoComp.setDecFormatoFileStandard(
				entityManager.find(DecFormatoFileStandard.class,
					mock.getIdDecFormatoFileStandard().longValue()));
		    }
		    // init bi-directionals
			...
			// per ogni componente salvo i dati relativi alla firma (se esistono)
			tmpReturn = salvataggioFirmaManager.salvaBustaCrittografica(risposta,
				tmpCV.getId(), tmpCV.getVfWrapper(), tmpTabCDComponente,
				tmpTabCDSottoComp);
			// se ci sono problemi nel salvataggio, blocco tutto
			if (!tmpReturn) {
			    return tmpReturn;
		...
		case BLOB:
		    tmpReturn = this.salvaBlobStrutDoc(tmpAroStrutDocMerged, valueDocVers, risposta,
			    strutV, chiave, WriteCompBlbOracle.TabellaBlob.ARO_CONTENUTO_COMP,
			    nomeWs);
		    break;
		case FILE:
		    if (strutV.isTpiAbilitato()) {
			tmpReturn = this.salvaFileStrutDoc(tmpAroStrutDocMerged, valueDocVers,
				risposta, strutV);
		    } else {
			/*
			 * se il TPI non è stato installato, vuol dire che tutta la gestione
			 * asincrona del versamento basata su TIVOLI è inutilizabile. In questo caso
			 * lo storage dei documenti avviene su una tabella di blob dedicata chiamata
			 * ARO_FILE_COMP con struttura identica a ARO_CONTENUTO_COMP
			 */
			tmpReturn = this.salvaBlobStrutDoc(tmpAroStrutDocMerged, valueDocVers,
				risposta, strutV, chiave,
    ...
    /**
     * Rispetta la firma degli altri metodi privati, di fatto restituisce sempre true, non esistono
     * infatti casistiche per cui debba fallire
     *
     * @param aroCompDoc
     * @param tmpUrn
     * @param tiUrn
    ...
    private boolean salvaBlobStrutDoc(AroStrutDoc aroStrutDoc, DocumentoVers valueDocVers,
	    RispostaControlli risposta, StrutturaVersamento strutV, ChiaveType chiave,
	    WriteCompBlbOracle.TabellaBlob tabellaBlob, String nomeWs) {
	// ricarico i componenti appena salvati per recuperare gli ID e conservarli in
	// memoria
	// inoltre preparo e salvo i record di contenuto componente relativi ai file
	for (AroCompDoc tmpCompDoc : aroStrutDoc.getAroCompDocs()) {
		...
		if (tmpCV.getOrdinePresentazione() == tmpCompDoc.getNiOrdCompDoc().longValue()) {
		    ...
		    if (!this.aggiungiBlobSuBlob(risposta, tmpCV, tmpCompDoc, strutV, chiave,
			    tabellaBlob, nomeWs)) {
			return false;
		    }
		}
    ...
    private boolean salvaFileStrutDoc(AroStrutDoc aroStrutDoc, DocumentoVers valueDocVers,
	    RispostaControlli risposta, StrutturaVersamento strutV) {
	// ricarico i componenti appena salvati per recuperare gli ID e conservarli in
	// memoria
	// inoltre preparo e salvo i record di contenuto componente relativi ai file
	for (AroCompDoc tmpCompDoc : aroStrutDoc.getAroCompDocs()) {
    ...
    private boolean aggiungiBlobSuBlob(RispostaControlli risposta, ComponenteVers componente,
	    AroCompDoc compntEntity, StrutturaVersamento strutV, ChiaveType chiave,
	    WriteCompBlbOracle.TabellaBlob tabellaBlob, String nomeWs) {
	boolean tmpReturn = true;
	FileBinario tmpFb;
	byte[] hash;
	...
	if (tmpReturn) {
		...
		if (componente.getHashVersatoAlgoritmo() != null) {
		    compntEntity
			    .setDsAlgoHashFileVers(componente.getHashVersatoAlgoritmo().descrivi());
		}
		if (componente.getHashVersatoEncoding() != null) {
		    compntEntity.setCdEncodingHashFileVers(
			    componente.getHashVersatoEncoding().descrivi());
		}
		...
		if (componente.isHashForzato()) {
			...
			compntEntity.setTiEsitoContrHashVers(
				CostantiDB.TipiEsitoVerificaHash.NEGATIVO.name());
		    } else {
			compntEntity.setTiEsitoContrHashVers(
				CostantiDB.TipiEsitoVerificaHash.NON_EFFETTUATO.name());
		    }
		} else if (componente.getHashCalcolato() != null
			&& componente.getHashCalcolato().length > 0) {
		    ...
		    hash = new HashCalculator().calculateSHAX(fis, TipiHash.SHA_256)
			    .getHashCalcolato();
			...
			compntEntity.setTiEsitoContrHashVers(
				CostantiDB.TipiEsitoVerificaHash.DISABILITATO.name());
		    }
		}
		...
		compntEntity
			.setCdEncodingHashFileCalc(CostantiDB.TipiEncBinari.HEX_BINARY.descrivi());
		...
		if (componente.getMySottoComponente() == null) {
		    ...
		    componente.getRifComponenteResp()
			    .setAlgoritmoHash(compntEntity.getDsAlgoHashFileCalc());
		    componente.getRifComponenteResp()
			    .setEncoding(compntEntity.getCdEncodingHashFileCalc());
		} else {
		    ...
		    componente.getRifSottoComponenteResp()
			    .setAlgoritmoHash(compntEntity.getDsAlgoHashFileCalc());
		    componente.getRifSottoComponenteResp()
			    .setEncoding(compntEntity.getCdEncodingHashFileCalc());
		}
	...
	if (tmpReturn) {
		...
		long idTipoUnitaDoc = docRif.getAroUnitaDoc().getDecTipoUnitaDoc()
			.getIdTipoUnitaDoc().longValue();
		BackendStorage backendComponenti = objectStorageService
			.lookupBackendByServiceName(idTipoUnitaDoc, nomeWs);
		...
		if (backendComponenti.isDataBase()) {
		    ...
		    RispostaControlli tmpControlli = writeCompBlbOracle
			    .salvaStreamSuBlobComp(datiAccessori, tmpFb);
		    if (!tmpControlli.isrBoolean()) {
			risposta.setDsErr(tmpControlli.getDsErr());
			tmpReturn = false;
		    }
		} else { // Backend Object storage
		    ObjectStorageResource componenteSuOS = objectStorageService
			    .createOrCopyResourceInComponenti(strutV, compntEntity,
				    backendComponenti.getBackendName(), tmpFb);
		    log.debug(
			    "Salvato il componente su Object storage nel bucket {} con chiave {}! ",
    ...
    private boolean aggiungiBlobSuFile(RispostaControlli risposta, ComponenteVers componente,
	    AroCompDoc compntEntity, StrutturaVersamento strutV) {
	boolean tmpReturn = true;
	FileBinario tmpFb;
	byte[] hash;
	...
	if (tmpReturn) {
		...
		String tmpFilePath = MessaggiWSFormat.formattaFilePathArk(strutV.getTpiRootTpi(),
			strutV.getTpiRootArkVers(), strutV.getSubPathDataVers(),
			strutV.getSubPathVersatoreArk(), strutV.getSubPathUnitaDocArk(),
			componente.getNomeFileArk());
	...
	if (tmpReturn) {
		...
		RispostaControlli tmpControlli = salvataggioCompFS.copiaCompSuFS(strutV,
			componente);
		if (!tmpControlli.isrBoolean()) {
		    risposta.setDsErr(tmpControlli.getDsErr());
		    tmpReturn = false;
	...
	for (DocumentoVers valueDocVers : strutturaVersIn.getDocumentiAttesi()) {
		...
		tmpReturn = this.salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec.VERS, tmpEntitaSacer,
			tmpCV.getIdRecDB(), strutturaVersIn.getIdStruttura(),
			tmpCV.getIdRecXsdDatiSpec(), tmpCV.getDatiSpecifici(),
			strutturaVersIn.getIdUnitaDoc());
		if (!tmpReturn) {
		    return false;
		}

		tmpReturn = this.salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec.MIGRAZ, tmpEntitaSacer,
			tmpCV.getIdRecDB(), strutturaVersIn.getIdStruttura(),
			tmpCV.getIdRecXsdDatiSpecMigrazione(), tmpCV.getDatiSpecificiMigrazione(),
			strutturaVersIn.getIdUnitaDoc());
		if (!tmpReturn) {
		    return false;
		}
    ...
    private boolean salvaDatiSpecGen(CostantiDB.TipiUsoDatiSpec tipoUso, TipiEntitaSacer tipoEntity,
	    long idEntity, long idStrut, long idXsd, Map<String, DatoSpecifico> datiSpecifici,
	    long idAroUnitaDoc) {
	boolean tmpReturn = true;
	...
	if (datiSpecifici != null) {
	    ...
	    case UNI_DOC:
		tmpAroUsoXsdDatiSpec
			.setAroUnitaDoc(entityManager.find(AroUnitaDoc.class, idEntity));
		break;
	    case DOC:
    ...
    /*
     * il sistema determina le versioni serie correnti con stato = CONTROLLATA o
     * VALIDAZIONE_IN_CORSO o VALIDATA o DA_FIRMARE o FIRMATA o IN_CUSTODIA, oppure con stato =
     * DA_CONTROLLARE e contenuto effettivo con stato = CONTROLLO_CONSIST_IN_CORSO, nel cui
     * contenuto di tipo EFFETTIVO sia presente l’unità documentaria in aggiornamento
     *
     ...
     * cui contenuto e’ presente l’unità documentaria in aggiornamento
     *
