...
public class VerificaVersione {
    ...
    public class EsitiVerfica {
        ...
        /**
         * @param flagConfigVersNonSupp
         *            the flagConfigVersNonSupp to set
         */
    ...
    public void verifica(RispostaWS rispostaWS, VersamentoExt versamento) {
        ...
        if (TipoConservazioneType.MIGRAZIONE.name().equals(myEsito.getConfigurazione().getTipoConservazione())) {
        ...
        if (parseOk && parsedUnitaDoc.getAllegati() != null) {
            Iterator<? extends DocumentoType> tmpEnumDoc = parsedUnitaDoc.getAllegati().getAllegato().iterator();
        ...
        if (parseOk && parsedUnitaDoc.getAnnessi() != null) {
            Iterator<? extends DocumentoType> tmpEnumDoc = parsedUnitaDoc.getAnnessi().getAnnesso().iterator();
        ...
        if (parseOk && parsedUnitaDoc.getAnnotazioni() != null) {
            Iterator<? extends DocumentoType> tmpEnumDoc = parsedUnitaDoc.getAnnotazioni().getAnnotazione().iterator();
            while (tmpEnumDoc.hasNext()) {
                DocumentoType tmpDocumentoType = (DocumentoType) tmpEnumDoc.next();
                this.verificaDocumento(tmpDocumentoType);
            }
        ...
        if (parseOk && parsedUnitaDoc.getConfigurazione() != null
                && !versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_ABILITA_FORZA_1_5)
        ...
        if (parseOk) {
            ...
            if (!profiliUdXsd && parsedUnitaDoc.getProfiloNormativo() != null) {
                ...
                tmpEsitiVerfica.getParams().addAll(Arrays.asList(TipiEntitaSacer.UNI_DOC.descrivi(),
                        parsedUnitaDoc.getIntestazione().getTipologiaUnitaDocumentaria(), "<ProfiloNormativo>"));
            }
        }
        //
        if (parseOk && !versamento.getModificatoriWSCalc().contains(ModificatoriWS.TAG_ESTESI_1_5_OUT)
    ...
    private void verificaDocumento(DocumentoType doc) {
        ...
        if (parseOk) {
            Iterator<? extends ComponenteType> tmpEnumCompo = doc.getStrutturaOriginale().getComponenti()
            ...
            while (tmpEnumCompo.hasNext()) {
                ...
                if (parseOk && tmpComponente.getSottoComponenti() != null) {
                    ...
                    while (tmpEnumSottoCompo.hasNext()) {
                        ...
                        if (tmpSottoComponente.getDatiSpecificiMigrazione() != null && migrazione == false) {
                            parseOk = false;
                            tmpEsitiVerfica.setDatiMigrazioneVersNonSupp(true);
                        }
                    }
