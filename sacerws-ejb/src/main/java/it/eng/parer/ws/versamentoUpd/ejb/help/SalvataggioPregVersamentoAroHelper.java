...
public class SalvataggioPregVersamentoAroHelper extends SalvataggioUpdVersamentoBaseHelper {
    ...
    public RispostaControlli salvaCdKeyNormUnitaDocumentaria(AroUnitaDoc tmpAroUnitaDoc,
            StrutturaUpdVers svf) {
        RispostaControlli tmpRispostaControlli = new RispostaControlli();
        tmpRispostaControlli.setrBoolean(true); // default true solo se effettua effettivamente
        // l'update può fallire
        ...
        try {
            ...
            getLogger().error(
                    "Errore interno nella fase di aggiornamento dati pregressi chiave normalizzata ud",
                    e);
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public RispostaControlli aggiornaPregCompDocUrnSip(AroUnitaDoc tmpAroUnitaDoc,
            StrutturaUpdVers svf) {
        RispostaControlli tmpRispostaControlli = new RispostaControlli();
        tmpRispostaControlli.setrBoolean(true); // default true solo se effettua effettivamente
        // l'update può fallire
        ...
        if (svf.getDtVersMax().before(svf.getDtInizioCalcoloNewUrn())
            ...
            try {
                ...
                tmpRispostaControlli.setCodErr(MessaggiWSBundle.ERR_666P);
                tmpRispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666P,
                        "Errore interno nella fase di aggiornamento dati pregressi "
                                + e.getMessage()));
                tmpRispostaControlli.setrBoolean(false);
                getLogger().error(
                        "Errore interno nella fase di aggiornamento dati pregressi sip ud/doc/upd",
                        e);
            }
        }
    ...
    private void salvaURNComponente(StrutturaUpdVers svf, AroDoc tmpAroDoc,
        ...
        for (TiUrn tmpTiUrn : tiUrnToCalculate) {
            ...
            if (count == 0) {
                ...
                switch (tmpTiUrn) {
                case ORIGINALE:
                    tmpUrn = MessaggiWSFormat.formattaBaseUrnDoc(svf.getUrnPartVersatore(),
                            svf.getUrnPartChiaveUd(), tmpUrnDoc,
                            Costanti.UrnFormatter.URN_COMP_FMT_STRING);
                    break;
                case NORMALIZZATO:
                    String urnPartVersatoreNorm = MessaggiWSFormat.formattaUrnPartVersatore(
                            svf.getVersatoreNonverificato(), true,
                            Costanti.UrnFormatter.VERS_FMT_STRING);
                    String urnPartChiaveUdNorm = MessaggiWSFormat.formattaUrnPartUnitaDoc(
                            svf.getChiaveNonVerificata(), true,
        ...
        for (AroDoc aroDoc : tmpAroUnitaDoc.getAroDocs()) {
            ...
            for (VrsVLisXmlDocUrnDaCalc vrs : vrsVLisXmlDocUrnDaCalc) {
                ...
                String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnDoc(
                        MessaggiWSFormat.formattaUrnPartVersatore(svf.getVersatoreNonverificato(),
                                true, Costanti.UrnFormatter.VERS_FMT_STRING),
                        MessaggiWSFormat.formattaUrnPartUnitaDoc(svf.getChiaveNonVerificata(), true,
                                Costanti.UrnFormatter.UD_FMT_STRING),
