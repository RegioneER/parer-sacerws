...
public class IndiceMMPrsr {
    ...
    public void parseXML(SyncFakeSessn sessione, VersamentoMMExt versamento,
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            RispostaControlli rispostaControlli = rapportoVersBuilder
                    .canonicalizzaPackInfoSipXml(sessione);
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            try {
                ...
                Unmarshaller tmpUnmarshaller = xmlVersCache.getVersReqMMCtxforIndiceMM()
                        .createUnmarshaller();
                ...
                myEsito.getEsitoXSD().setControlloStrutturaXML(
                        "Errore: XML malformato nel blocco di dati indice multimedia. Eccezione: "
                                + e.getMessage());
                ...
                myEsito.getEsitoXSD().setCodiceEsito(ECEsitoPosNegType.NEGATIVO);
                myEsito.getEsitoXSD().setControlloStrutturaXML(
                        "Errore di validazione del blocco di dati indice multimedia. Eccezione: "
                                + e.getMessage());
            } catch (Exception e) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.MM_XSD_001_001, e.getMessage());
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            RispostaControlli rispostaControlli = controlliMM.caricaRootPath(
                    parsedIndice.getApplicativoChiamante(), ControlliMM.TipiRootPath.In);
            if (rispostaControlli.isrBoolean()) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            File tmpFile;
            if (parsedIndice.getPathArchivioZip() != null
                    && parsedIndice.getPathArchivioZip().length() > 0) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            HashMap<String, ComponenteMM> tmpMap = versamento.getStrutturaIndiceMM()
                    .getComponenti();
            ...
            for (ComponenteType tmpComponente : parsedIndice.getComponenti().getComponente()) {
                if (tmpComponente.getID() == null || tmpComponente.getID().length() == 0
                    ...
                    if (tmpComponente.isVerificaFirmeFormati()
                            && tmpComponente.getForzaFormato() != null) {
                        // Componente Multimedia <ID> {0}: Tag <ForzaFormato> presente ma tag
                        // <VerificaFirmeFormati> =
                        // true
                        versamento.listErrAddErrorUd(tmpComponente.getID(),
                                MessaggiWSBundle.MM_COMP_001_001, tmpComponente.getID());
                    }
                    //
                    if (tmpComponente.isCalcolaHash() && tmpComponente.getForzaHash() != null) {
                        ...
                        // <CalcolaHash> = true
                        versamento.listErrAddErrorUd(tmpComponente.getID(),
                                MessaggiWSBundle.MM_COMP_002_001, tmpComponente.getID());
                    }
                    //
                    ...
                    if (tmpComponente.getForzaFormato() != null) {
                        RispostaControlli rispostaControlli = controlliSemantici
                                .checkFormatoFileStandard(
                                        tmpComponente.getForzaFormato().getFormatoStandard());
                        if (rispostaControlli.isrBoolean() == false) {
                            // Componente Multimedia <ID> {0}: Il formato [{1}] non \u00e8 presente
                            // nel DB
                            versamento.listErrAddErrorUd(tmpComponente.getID(),
                                    MessaggiWSBundle.MM_COMP_003_001, tmpComponente.getID(),
                                    tmpComponente.getForzaFormato().getFormatoStandard());
                        } else {
                    ...
                    if (tmpComponente.getForzaHash() != null) {
                        ...
                        try {
                            tmpComponenteMM.setHashForzato(Hex.decodeHex(
                                    tmpComponente.getForzaHash().getHash().toCharArray()));
                        } catch (DecoderException e) {
                            hashForzatoErr = true;
                        }
                        if (hashForzatoErr) {
                            // Componente Multimedia <ID> {0}: L'hash forzato non \u00e8 valido
                            // (sono ammessi solo hash
                            // SHA-256 in formato hexBinary)
                            versamento.listErrAddErrorUd(tmpComponente.getID(),
                                    MessaggiWSBundle.MM_COMP_004_001, tmpComponente.getID());
                        }
                    }
                    //
                    String tmpFilePath = tmpComponente.getURNFile().trim();
                    if (tmpFilePath == null || tmpFilePath.length() == 0) {
                        // Componente Multimedia <ID> {0}: Il tag <URNFile> obbligatorio e non
                        // valorizzato
                        versamento.listErrAddErrorUd(tmpComponente.getID(),
                                MessaggiWSBundle.MM_COMP_005_001, tmpComponente.getID());
                    }
                }
            }
            if (trovatoDuplicati) {
        ...
        // .....................verifica che il file dichiarato nel campo URNFile esista nello zip
        // dichiarato
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                ...
                for (ComponenteType tmpComponente : parsedIndice.getComponenti().getComponente()) {
                    ...
                    if (tmpFilePath != null && tmpFilePath.length() > 0) {
                        if (zipFilePath == null) {
                            ...
                            if (!tmpFile.isFile() || !tmpFile.canRead()) {
                                // Il file {0} riferito nel componente Multimedia <ID> {1} non
                                // esiste o non \u00e8
                                // raggiungibile
                                versamento.listErrAddErrorUd(tmpComponente.getID(),
                                        MessaggiWSBundle.MM_FILE_002_001, tmpFilePath,
                                        tmpComponente.getID());
                            }
                        } else {
                            if (!this.verificaPresenzaFileInZip(zipFilePath, tmpFilePath)) {
                                ...
                                // ZIP {2}
                                versamento.listErrAddErrorUd(tmpComponente.getID(),
                                        MessaggiWSBundle.MM_FILE_003_001, tmpFilePath,
                                        tmpComponente.getID(), zipFilePath);
                            }
                        }
                    }
                }
    ...
    private boolean verificaPresenzaFileInZip(String zipPathName, String fileInZip)
            throws IOException {
