...
public class ControlliWSTest {
    ...
    public static Archive<?> createTestArchive() {
        final JavaArchive sacerWSJavaArchive = createSacerWSJavaArchive(
                Arrays.asList("it.eng.spagoLite.security.exception", "it.eng.spagoLite.security",
                        "org.apache.commons.codec.binary", "org.apache.commons.codec", "it.eng.spagoCore.error"),
                ControlliWS.class, ControlliWSTest.class, WsIdpLogger.class, ControlliSemantici.class,
                ConfigurationHelper.class, AppServerInstance.class, LogDto.class, Menu.class, WSLoginHandler.class,
                PwdUtil.class, MenuEntry.class, FrameElement.class, it.eng.parer.idpjaas.logutils.IdpConfigLog.class,
    ...
    public void checkVersione() {
        for (final Costanti.TipiWSPerControlli tipoWS : Costanti.TipiWSPerControlli.values()) {
            ...
            final RispostaControlli rispostaControlli = controlliWS.checkVersione("1.4", versioniWsKey, xmlDefaults,
                    tipoWS);
    ...
    public void checkCredenziali() {
        for (final Costanti.TipiWSPerControlli tipoWS : Costanti.TipiWSPerControlli.values()) {
            final RispostaControlli rispostaControlli = controlliWS.checkCredenziali("admin_generale", "password",
                    "127.0.0.1", tipoWS);
            assertTrue(rispostaControlli.isrBoolean());
    ...
    public void checkAuthWSNoOrg() {
        for (final Costanti.TipiWSPerControlli tipoWS : Costanti.TipiWSPerControlli.values()) {
            final RispostaControlli rispostaControlli = controlliWS.checkAuthWSNoOrg(mockUser(), getIwsDesc(), tipoWS);
            assertTrue(rispostaControlli.isrBoolean());
        }
    }
