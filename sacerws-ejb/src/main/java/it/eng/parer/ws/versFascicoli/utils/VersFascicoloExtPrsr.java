...
import it.eng.parer.ws.versFascicoli.ejb.ControlliCollFascicolo;
import it.eng.parer.ws.versFascicoli.ejb.ControlliFascicoli;
import it.eng.parer.ws.versFascicoli.ejb.ControlliProfiliFascicolo;
...
public class VersFascicoloExtPrsr {
    ...
    // l'istanza dell'indice SIP del fascicolo, decodificata dall'XML di versamento
    IndiceSIPFascicolo parsedIndiceFasc = null;
    //
    private CSVersatore tagCSVersatore = new CSVersatore();
    private CSChiaveFasc tagCSChiave = new CSChiaveFasc();
    private String descChiaveFasc = "";
    ...
    ControlliProfiliFascicolo controlliProfiliFascicolo = null;
    // stateless ejb dei controlli collegamenti dei fascicoli
    ControlliCollFascicolo controlliCollFascicolo = null;
    ...
    XmlFascCache xmlFascCache = null;
    ...
    public VersFascicoloExt getVersamento() {
        return versamento;
    }

    public RispostaWSFascicolo getRispostaWs() {
        return rispostaWs;
    }

    public VersFascicoloExtPrsr(VersFascicoloExt versamento, RispostaWSFascicolo rispostaWs) {
    ...
    public void parseXML(SyncFakeSessn sessione) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            try {
                xmlFascCache = (XmlFascCache) new InitialContext().lookup("java:module/XmlFascCache");
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            try {
                ...
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB dei controlli specifici dei fascicoli  " + ex.getMessage());
                log.error("Errore nel recupero dell'EJB dei controlli specifici dei fascicoli ", ex);
            }
        }
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            try {
                controlliProfiliFascicolo = (ControlliProfiliFascicolo) new InitialContext()
                        .lookup("java:module/ControlliProfiliFascicolo");
            } catch (NamingException ex) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB dei controlli Profili Fascicolo dei fascicoli  "
                                + ex.getMessage());
                log.error("Errore nel recupero dell'EJB dei controlli Profili Fascicolo dei fascicoli ", ex);
            }
        }
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            try {
                controlliCollFascicolo = (ControlliCollFascicolo) new InitialContext()
                        .lookup("java:module/ControlliCollFascicolo");
            } catch (NamingException ex) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB dei controlli collegamenti dei fascicoli  " + ex.getMessage());
                log.error("Errore nel recupero dell'EJB dei controlli collegamenti dei fascicoli ", ex);
            }
        }
        ...
        // con quelli nell'xml
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaVersatore(myControlliFascicolo);
        }

        // se ho passato la verifica strutturale...
        // verifica la struttura versante
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaStruttura(myControlliFascicolo);
        }
        ...
        //
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaTipoFasc(myControlliFascicolo);
        }

        // verifico il tipo fascicolo - abilitato su utente
        //
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaTipoFascUserOrg(myControlliFascicolo);
        }

        // verifico il tipo fascicolo - abilitato su anno
        //
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaTipoFascAnno(myControlliFascicolo);
        }
        ...
        // verifico la chiave
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaChiave(myControlliFascicolo, myEsito);
        }

        // verifico tipo conservazione
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            this.controllaTipoConservazione(myControlliFascicolo, myEsito);
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            boolean prosegui = this.controllaProfiloArchivistico(myEsito);

            if (prosegui) {
                prosegui = this.controllaProfiloSpecifico(myEsito);// TODO: al momento non supportato verificare se va
                                                                   // bene in questo punto
            }
            // viene sempre eseguito
            // blocco controlli profilo generale
            prosegui = this.controllaProfiloGenerale(myEsito, myControlliFascicolo);

            if (prosegui) {
                prosegui = this.controllaSoggettoProd(myEsito);
            }
            // viene sempre eseguito (dati già controllati in precedenza)
            if (prosegui) {
                prosegui = this.controllaFlagStruttura(myEsito);
            }

            if (prosegui) {
                prosegui = this.controllaClassificazione(myEsito);
            }

            if (prosegui) {
                prosegui = this.controllaFormatoNumero(myEsito);
            }

            if (prosegui) {
                prosegui = this.controllaCollegamenti(myEsito);
            }
    ...
    private void controllaTipoConservazione(EsitoControlliFascicolo myControlliFascicolo,
            CompRapportoVersFascicolo myEsito) {
    ...
    private boolean controllaFlagStruttura(CompRapportoVersFascicolo myEsito) {
        ...
        rispostaControlli = controlliFascicoli.checkFlDecAaTipoFascicoloOrgStrutt(svf.getIdAATipoFasc());
    ...
    private boolean controllaClassificazione(CompRapportoVersFascicolo myEsito) {
        ...
        if (svf.getDatiXmlProfiloGenerale() != null && svf.getDatiXmlProfiloArchivistico() != null) {
            ...
            if (!flContrFasc.isFlAbilitaContrClassif()) {
                ...
                rispostaControlli = controlliFascicoli.verificaSIPTitolario(svf);
    ...
    private boolean controllaFormatoNumero(CompRapportoVersFascicolo myEsito) {
        ...
        rispostaControlli = controlliFascicoli.getPartiAANumero(descChiaveFasc, svf.getIdAATipoFasc());
    ...
    private boolean controllaCollegamenti(CompRapportoVersFascicolo myEsito) {
        ...
        if (svf.getDatiXmlProfiloArchivistico() != null
            ...
            rispostaControlli = controlliCollFascicolo.buildCollegamentiFascicolo(versamento, myControlliFascicolo);
    ...
    private void controllaStruttura(ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo) {
        // verifica la struttura versante
        rispostaControlli = controlliSemantici.checkIdStrut(tagCSVersatore, TipiWSPerControlli.VERSAMENTO_FASCICOLO,
    ...
    private void controllaTipoFasc(ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo) {
        // verifico il tipo fascicolo - in caso di errore la sessione è DUBBIA
        //
        rispostaControlli = controlliFascicoli.checkTipoFascicolo(
    ...
    private void controllaTipoFascUserOrg(EsitoControlliFascicolo myControlliFascicolo) {
        ...
        //
        StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
        rispostaControlli = controlliFascicoli.checkTipoFascicoloIamUserOrganizzazione(descChiaveFasc,
    ...
    private void controllaTipoFascAnno(EsitoControlliFascicolo myControlliFascicolo) {
        ...
        //
        StrutturaVersFascicolo svf = versamento.getStrutturaComponenti();
        rispostaControlli = controlliFascicoli.checkTipoFascicoloAnno(descChiaveFasc,
    ...
    private void controllaChiave(ECFascicoloType.EsitoControlliFascicolo myControlliFascicolo,
            CompRapportoVersFascicolo myEsito) {
        rispostaControlli = controlliFascicoli.checkChiave(tagCSChiave, descChiaveFasc,
                versamento.getStrutturaComponenti().getIdStruttura(),
    ...
    private boolean controllaProfiloArchivistico(CompRapportoVersFascicolo myEsito) {
        ...
        rispostaControlli = controlliProfiliFascicolo.verificaProfiloArchivistico(versamento);
        if (!rispostaControlli.isrBoolean()) {
            myControlliFascicolo.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
    ...
    private boolean controllaProfiloGenerale(CompRapportoVersFascicolo myEsito,
        ...
        boolean prosegui = true;
        rispostaControlli = controlliCollFascicolo.verificaUdFascicolo(versamento, myEsito.getFascicolo());
        if (!rispostaControlli.isrBoolean()) {
            myControlliFascicolo.setCodiceEsito(ECEsitoPosNegWarType.NEGATIVO);
