...
public class CryptoWrapperResultStrategy
        implements IVerificaFirmaWrapperResultStrategy<CryptoAroCompDoc> {
    ...
    public void fromVerificaOutOnWrapper(CryptoAroCompDoc result, VerificaFirmaWrapper wrapper) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("I componenti versati sono [{}]",
                    String.join(",", componentiVersati(result)));
        }
    }

    /**
     * Modifica la lista delle firme per allinearle, relativamente a controfirme alla struttura
     * utilizzata dal wrapper
     *
     * @param resultCrypto risultato della verifica del ws crypto
     */
    ...
    /**
     * Nel modello crypto se una firma appare tra le controfirme compare anche tra le firme
     * "normali" creando, quindi, una duplicazione da eliminare
     *
     * @param resultCrypto output del ws crypto
     *
    ...
    private List<CryptoAroFirmaComp> normalizzaFirmeConControfirme(CryptoAroCompDoc resultCrypto) {
        ...
        List<CryptoAroFirmaComp> firmeConControfirma = firmeEsitoCrypto.stream()
                .filter(f -> f.getAroControfirmaFirmaFiglios() != null
                        && !f.getAroControfirmaFirmaFiglios().isEmpty())
    ...
    /**
     * Classe statica utilizzata per poter implementare {@link #equals(java.lang.Object)} e
     * {@link #hashCode()} sulle firme per poter chiamare il metodo
     * {@link List#contains(java.lang.Object) } avendo la certezza che identifichi correttamente il
     * record.
     *
     */
     ...
     *
     * @param resultVerificaCrypto wrapper
     *
    ...
    private VFFirmaCompType buildFirma(CryptoAroFirmaComp cryptoFirma) {
        ...
        if (cryptoFirma.getFirCertifFirmatario() != null) {
            VFCertifFirmatarioType certifFirmatario = buildFirmatario(
                    cryptoFirma.getFirCertifFirmatario());
            firma.setCertifFirmatario(certifFirmatario);
        }

        if (cryptoFirma.getAroControfirmaFirmaFiglios() != null) {
            for (CryptoAroControfirmaFirma cryptoControFirma : cryptoFirma
                    .getAroControfirmaFirmaFiglios()) {
        ...
        for (CryptoAroContrFirmaComp aroContrFirmaComp : cryptoFirma.getAroContrFirmaComps()) {
            ...
            contrFirmaComp
                    .setTiContr(VFTipoControlloType.fromValue(aroContrFirmaComp.getTiContr()));
        ...
        /*
         * Attenzione! la data firma può essere nulla. In quel caso *dovrebbe* essere valorizzato il
         * rif. temporale versato
         */
     ...
     *
     * @param cryptoFirmatario modello del firmatario per la crypto
     *
    ...
    private VFCertifFirmatarioType buildFirmatario(CryptoFirCertifFirmatario cryptoFirmatario) {
        ...
        certifFirmatario.setDsSerialCertifFirmatario(
                String.valueOf(cryptoFirmatario.getNiSerialCertifFirmatario()));
     ...
     *
     * @param cryptoCa Oggetto proveniente dalla Vecchia cryptolibrary.
     *
    ...
    private VFCertifCaType buildCertifCA(CryptoFirCertifCa cryptoCa) {
        ...
        certifCA.setDsSerialCertifCa(String.valueOf(cryptoCa.getNiSerialCertifCa())); // integer->string
        // (needed for
        // db
        // representation)
    ...
    /**
     * Compila la busta relativa alle CRL. <em>ATTENZIONE:</em> il blob delle crl
     * ({@code parerCrl.getEncoded()}) può arrivare a superare i 40MB.
     *
     * @param cryptoCrl Modello di crl restituito da crypto
     *
