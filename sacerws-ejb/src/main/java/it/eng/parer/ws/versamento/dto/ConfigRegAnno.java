 ...
 * @author fioravanti_f
 */
public class ConfigRegAnno {

    private final List<ParteRegAnno> parti;
    private boolean generico;
    private String regExpCalc;
    private String descRegExp;
    long idAaRegistroUnitaDoc;

    public ConfigRegAnno(long idAaRegistroUnitaDoc) {
        this.idAaRegistroUnitaDoc = idAaRegistroUnitaDoc;
        parti = new ArrayList<>();
    }

    public ParteRegAnno aggiungiParte() {
        ParteRegAnno pra = new ParteRegAnno();
        parti.add(pra);
        return pra;
    }
    ...
    public void ElaboraParti() {
        ...
        for (ParteRegAnno pra : parti) {
            ...
            case PARTE_GENERICO:
                ...
                if (pra.isLunghezzaFissa() || pra.getSeparatore() == null || pra.getSeparatore().length() == 0) {
                    tmpSb.append(String.format("(.%s)", pra.quantificatore));
                } else {
                    // altrimenti il contenuto è generico a meno del carattere di separazione
                    // e del carattere di fine linea
                    tmpSb.append(String.format("([^\\n%s]%s)", pra.getSeparatore(), pra.quantificatore));
                    tmpModificatore = String.format(" (tranne il carattere \"%s\")", pra.getSeparatore());
                }
                break;
    ...
    public static void impostaValoriAccettabili(ParteRegAnno parte, String valori) {
    ...
    //
    public List<ParteRegAnno> getParti() {
        return parti;
    }
    ...
    public long getIdAaRegistroUnitaDoc() {
        return idAaRegistroUnitaDoc;
    }

    //
    public enum TipiPadding {

        RIEMPI_0_A_SX_LESS12, STANDARD, NESSUNO
    }

    public class ParteRegAnno {

        private String nomeParte;
        private int numParte;
        private KeyOrdUtility.TipiCalcolo tipoCalcolo;
        private long minLen;
        private long maxLen;
        ...
        public KeyOrdUtility.TipiCalcolo getTipoCalcolo() {
            return tipoCalcolo;
        }

        public void setTipoCalcolo(KeyOrdUtility.TipiCalcolo tipoCalcolo) {
        ...
        public boolean isMatchRegistro() {
            return matchRegistro;
        }

        public void setMatchRegistro(boolean matchRegistro) {
            this.matchRegistro = matchRegistro;
        }
    ...
    public enum TiParte {
        ANNO("ANNO"), PROGR("PROGRESSIVO"), REGISTRO("REGISTRO");
