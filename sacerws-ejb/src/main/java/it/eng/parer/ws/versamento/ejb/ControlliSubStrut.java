...
public class ControlliSubStrut {
    ...
    public RispostaControlli calcolaSubStrut(StrutturaVersamento strutV) {
        ...
        try {
            // trova la regola di calcolo per la substruttura in base al tipo UD e al tipo doc principale
            queryStr = "select tud " + "from OrgRegolaValSubStrut tud "
                    + "where tud.decTipoUnitaDoc.idTipoUnitaDoc = :idTipoUnitaDocIn "
                    + " and tud.decTipoDoc.idTipoDoc = :idTipoDocIn " + " and tud.dtIstituz <= :dataDiOggiIn "
            ...
            query.setParameter("idTipoDocIn", strutV.getDocumentiAttesi().get(0).getIdTipoDocumentoDB());
            ...
            if (orvsses.size() != 1) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.UD_011_001,
                        strutV.getUrnPartChiaveUd(), "la regola di calcolo della substruttura non è stata definita"));
                return rispostaControlli;
            }
            OrgRegolaValSubStrut orvss = orvsses.get(0);
            if (orvss.getOrgCampoValSubStruts() == null || orvss.getOrgCampoValSubStruts().isEmpty()) {
                rispostaControlli.setCodErr(MessaggiWSBundle.UD_011_001);
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.UD_011_001,
                        strutV.getUrnPartChiaveUd(), "la regola di calcolo della substruttura non ha campi definiti"));
                return rispostaControlli;
            }
            ...
            if (orvss.getOrgCampoValSubStruts().size() > 1) {
                rispostaControlli.setCodErr(MessaggiWSBundle.UD_011_001);
                rispostaControlli
                        .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.UD_011_001, strutV.getUrnPartChiaveUd(),
            ...
            case DATO_SPEC_UNI_DOC:
                rispostaControlli = this.calcolaSubStrutDaDatoSpec(strutV, TipiDatiSpecCalcSubStrut.UNITA_DOC,
                        campoValSubStrut.getDecAttribDatiSpec());
                break;
            case DATO_SPEC_DOC_PRINC:
                rispostaControlli = this.calcolaSubStrutDaDatoSpec(strutV,
                        TipiDatiSpecCalcSubStrut.DOCUMENTO_PRINCIPALE, campoValSubStrut.getDecAttribDatiSpec());
                break;
            default:
                rispostaControlli.setCodErr(MessaggiWSBundle.UD_011_001);
                rispostaControlli
    ...
    private RispostaControlli calcolaSubStrutDaDatoSpec(StrutturaVersamento strutV,
        ...
        if (datiSpecifici == null || datiSpecifici.isEmpty()) {
            ...
            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.UD_011_001,
                    strutV.getUrnPartChiaveUd(),
                    "non sono presenti dati specifici per " + descElemento + " utili a determinare la substruttura"));
            return rispostaControlli;
        }
        ...
        if (tmpValSubStrut == null || tmpValSubStrut.isEmpty()) {
            ...
            rispostaControlli.setDsErr(
                    MessaggiWSBundle.getString(coderrDatoSpecNullo, strutV.getUrnPartChiaveUd(), descAttribDatoSpec));
            return rispostaControlli;
        }

        try {
            queryStr = "select tud " + "from OrgSubStrut tud " + "where tud.nmSubStrut = :nmSubStrutIn "
            ...
            if (orgss.isEmpty()) {
                ...
                rispostaControlli.setDsErr(MessaggiWSBundle.getString(codErrDatoSpecErrato, strutV.getUrnPartChiaveUd(),
                        descAttribDatoSpec));
                return rispostaControlli;
            } else {
